...
float validate_classifier_single(char *datacfg, char *filename,
                                 char *weightfile, network *existing_net,
                                 int topk_custom);
...
void train_classifier(char *datacfg, char *cfgfile, char *weightfile, int *gpus,
                      int ngpus, int clear, int dontuse_opencv, int dont_show,
                      int mjpeg_port, int calc_topk, int show_imgs,
                      char *chart_path)
{
  int i;
  ...
  if (classes != l.outputs && (l.type == SOFTMAX || l.type == COST)) {
    printf(
        "\n Error: num of filters = %d in the last conv-layer in cfg-file "
        "doesn't match to classes = %d in data-file \n",
        l.outputs, classes);
  ...
  while (get_current_batch(net) < net.max_batches || net.max_batches == 0) {
    ...
    int calc_topk_for_each =
        iter_topk +
        2 * train_images_num /
            (net.batch * net.subdivisions);  // calculate TOPk for each 2 Epochs
    calc_topk_for_each = fmax(calc_topk_for_each, net.burn_in);
    calc_topk_for_each = fmax(calc_topk_for_each, 100);
    if (i % 10 == 0) {
      if (calc_topk) {
        fprintf(stderr, "\n (next TOP%d calculation at %d iterations) ",
                topk_data, calc_topk_for_each);
        if (topk > 0)
    ...
    if (calc_topk && (i >= calc_topk_for_each || i == net.max_batches)) {
      ...
      if (net.contrastive && l.type != SOFTMAX && l.type != COST) {
        int k;
        for (k = 0; k < net.n; ++k)
          if (net.layers[k].type == CONTRASTIVE) break;
        topk = *(net.layers[k].loss) / 100;
        sprintf(topk_buff, "Contr");
      } else {
        topk = validate_classifier_single(datacfg, cfgfile, weightfile, &net,
                                          topk_data);  // calc TOP-n
        printf("\n accuracy %s = %f \n", topk_buff, topk);
      }
      draw_precision = 1;
    }

    time_remaining = ((net.max_batches - i) / ngpus) *
                     (what_time_is_it_now() - start) / 60 / 60;
    ...
    start = what_time_is_it_now();
    printf("%d, %.3f: %f, %f avg, %f rate, %lf seconds, %" PRIu64
           " images, %f hours left\n",
           get_current_batch(net), (float)(*net.seen) / train_images_num, loss,
    ...
    if (!dontuse_opencv)
      draw_train_loss(windows_name, img, img_size, avg_loss, max_img_loss, i,
                      net.max_batches, topk, draw_precision, topk_buff,
                      avg_contrastive_acc / 100, dont_show, mjpeg_port,
                      avg_time);
#endif  // OPENCV
...
clear)
   ...
   printf("Learning Rate: %g, Momentum: %g, Decay: %g\n", net.learning_rate,
net.momentum, net.decay); list *options = read_data_cfg(datacfg);
...
get_current_rate(net), sec(clock()-time), *net.seen); if(*net.seen/N > epoch){
    epoch = *net.seen/N;
    char buff[256];
                                 ...
                                 int topk_custom)
{
  ...
  for (i = 0; i < m; ++i) {
    ...
    if (existing_net) printf("\r");
      ...
      printf("\n");
    printf("%d: top 1: %f, top %d: %f", i, avg_acc / (i + 1), topk,
           avg_topk / (i + 1));
  }
  free(indexes);
  if (existing_net) {
                        ...
                        char *filename, int top)
{
  ...
  while (1) {
    ...
    image resized = resize_min(im, net.w);
    image cropped = crop_image(resized, (resized.w - net.w) / 2,
                               (resized.h - net.h) / 2, net.w, net.h);
    printf("%d %d\n", cropped.w, cropped.h);
...
void demo_classifier(char *datacfg, char *cfgfile, char *weightfile,
                     int cam_index, const char *filename, int benchmark,
                     int benchmark_layers)
{
  ...
  while (1) {
    ...
    if (!benchmark) {
      printf(
          "\rFPS: %.2f  (use -benchmark command line flag for correct "
          "measurement)\n",
          fps);
      for (i = 0; i < top; ++i) {
        int index = indexes[i];
...
void run_classifier(int argc, char **argv)
{
  ...
  if (0 == strcmp(argv[2], "predict"))
    ...
    train_classifier(data, cfg, weights, gpus, ngpus, clear, dontuse_opencv,
                     dont_show, mjpeg_port, calc_topk, show_imgs, chart_path);
  else if (0 == strcmp(argv[2], "demo"))
    demo_classifier(data, cfg, weights, cam_index, filename, benchmark,
                    benchmark_layers);
