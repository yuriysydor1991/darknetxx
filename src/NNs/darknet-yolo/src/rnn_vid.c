                            ...
                            int steps)
{
  ...
  float *feats =
      (float *)xcalloc(net.batch * batch * output_size, sizeof(float));
  for (b = 0; b < batch; ++b) {
...
void generate_vid_rnn(char *cfgfile, char *weightfile)
{
  ...
  int i;
  cap_cv *cap = get_capture_video_stream(
      "extra/vid/ILSVRC2015/Data/VID/snippets/val/ILSVRC2015_val_00007030.mp4");
  // CvCapture* cap =
  // cvCaptureFromFile("extra/vid/ILSVRC2015/Data/VID/snippets/val/ILSVRC2015_val_00007030.mp4");
  float *feat;
  float *next;
  next = NULL;
  image last;
