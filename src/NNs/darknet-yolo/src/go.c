...
void propagate_liberty(float *board, int *lib, int *visited, int row, int col,
                       int side)
{
...
int generate_move(network net, int player, float *board, int multi,
                  float thresh, float temp, char *ko, int print)
{
  int i, j;
    ...
    if (!strcmp(buff, "protocol_version")) {
      ...
      int known =
          (!strcmp(comm, "protocol_version") || !strcmp(comm, "name") ||
           ...
           !strcmp(comm, "play") || !strcmp(comm, "genmove"));
