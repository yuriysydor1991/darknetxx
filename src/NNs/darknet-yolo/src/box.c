...
dxrep dx_box_iou(box pred, box truth, IOU_LOSS iou_loss)
{
  ...
  float I = Iw * Ih;
  float U = X + Xhat - I;
  float S = (pred.x - truth.x) * (pred.x - truth.x) +
  ...
  p_dx = p_dl +
         p_dr;  // p_dx, p_dy, p_dw and p_dh are the gradient of IoU or GIoU.
  p_dy = p_dt + p_db;
  ...
  // UNUSED
  //// ground truth
  // float gt_dt = ((U * dI_wrt_xhat_t) - (I * (dXhat_wrt_t - dI_wrt_xhat_t))) /
  // (U * U); float gt_db = ((U * dI_wrt_xhat_b) - (I * (dXhat_wrt_b -
  // dI_wrt_xhat_b))) / (U * U); float gt_dl = ((U * dI_wrt_xhat_l) - (I *
  // (dXhat_wrt_l - dI_wrt_xhat_l))) / (U * U); float gt_dr = ((U *
  // dI_wrt_xhat_r) - (I * (dXhat_wrt_r - dI_wrt_xhat_r))) / (U * U);
...
truth_tblr.top && dx.dt < 0)) { printf("✓"); } else { printf("𝒙");
  ...
  printf(", ");
  if ((pred_tblr.bot < truth_tblr.bot && dx.db > 0) || (pred_tblr.bot >
truth_tblr.bot && dx.db < 0)) { printf("✓"); } else { printf("𝒙");
  }
  printf(", ");
  if ((pred_tblr.left < truth_tblr.left && dx.dl > 0) || (pred_tblr.left >
truth_tblr.left && dx.dl < 0)) { printf("✓"); } else { printf("𝒙");
  }
  printf(", ");
  if ((pred_tblr.right < truth_tblr.right && dx.dr > 0) || (pred_tblr.right >
truth_tblr.right && dx.dr < 0)) { printf("✓"); } else { printf("𝒙");
  }
  printf("\n");
...
void do_nms_sort_v2(box *boxes, float **probs, int total, int classes,
                    float thresh)
{
  int i, j, k;
