...
__global__ void forward_maxpool_layer_kernel(int n, int in_h, int in_w,
                                             int in_c, int stride_x,
                                             int stride_y, int size, int pad,
                                             float *input, float *output,
                                             int *indexes)
{
...
__global__ void backward_zero_nonmax_kernel(int n, int *indexes,
                                            float *prev_delta)
{
  int id = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
  if (id >= n) return;
                                          ...
                                          network_state state)
{
  if (layer.maxpool_depth) {
    ...
    forward_maxpool_depth_layer_kernel<<<cuda_gridsize(n), BLOCK, 0,
                                         get_cuda_stream()>>>(
        n, layer.w, layer.h, layer.c, layer.out_c, layer.batch, state.input,
  ...
  if (!state.train && layer.stride == layer.size) {
    ...
    maxpool_status = cudnnPoolingForward(
        cudnn_handle(), layer.poolingDesc, &alpha, layer.srcTensorDesc,
        state.input, &beta, layer.dstTensorDesc, layer.output_gpu);
...
#endif
  {
    ...
    forward_maxpool_layer_kernel<<<cuda_gridsize(n), BLOCK, 0,
        ...
        layer.size, layer.pad, state.input, layer.output_gpu,
        layer.indexes_gpu);
    CHECK_CUDA(cudaPeekAtLastError());

    if (layer.maxpool_zero_nonmax) {
      forward_zero_nonmax_kernel<<<cuda_gridsize(n), BLOCK, 0,
                                   get_cuda_stream()>>>(n, state.input,
                                                        layer.output_gpu);
      CHECK_CUDA(cudaPeekAtLastError());
    }
  }
  ...
  if (layer.antialiasing) {
    ...
    simple_copy_ongpu(layer.outputs * layer.batch, layer.output_gpu,
                      layer.input_antialiasing_gpu);
    simple_copy_ongpu(layer.input_layer->outputs * layer.input_layer->batch,
                      layer.input_layer->output_gpu, layer.output_gpu);
  }
}
                                           ...
                                           network_state state)
{
  if (layer.antialiasing) {
    ...
    simple_copy_ongpu(layer.input_layer->outputs * layer.input_layer->batch,
                      layer.delta_gpu, layer.input_layer->delta_gpu);
    backward_convolutional_layer_gpu(*(layer.input_layer), s);

    // simple_copy_ongpu(layer.outputs*layer.batch,
    // layer.input_antialiasing_gpu, layer.output_gpu);
  }
  ...
  if (layer.maxpool_depth) {
    ...
    backward_maxpool_depth_layer_kernel<<<cuda_gridsize(n), BLOCK, 0,
                                          get_cuda_stream()>>>(
        n, layer.w, layer.h, layer.c, layer.batch, layer.delta_gpu, state.delta,
        layer.indexes_gpu);
    CHECK_CUDA(cudaPeekAtLastError());
    return;
  }
  ...
  backward_maxpool_layer_kernel<<<cuda_gridsize(n), BLOCK, 0,
                                  get_cuda_stream()>>>(
      n, layer.h, layer.w, layer.c, layer.stride_x, layer.stride_y, layer.size,
      layer.pad, layer.delta_gpu, state.delta, layer.indexes_gpu);
  CHECK_CUDA(cudaPeekAtLastError());

  if (layer.maxpool_zero_nonmax) {
    backward_zero_nonmax_kernel<<<cuda_gridsize(n), BLOCK, 0,
                                  get_cuda_stream()>>>(n, layer.indexes_gpu,
                                                       state.delta);
    CHECK_CUDA(cudaPeekAtLastError());
  }
}

__global__ void forward_local_avgpool_layer_kernel(int n, int in_h, int in_w,
                                                   int in_c, int stride_x,
                                                   int stride_y, int size,
                                                   int pad, float *input,
                                                   float *output)
{
  int h = (in_h + pad - size) / stride_y + 1;
  ...
  output[out_index] =
      avg / counter;  // as CUDNN_POOLING_AVERAGE_COUNT_EXCLUDE_PADDING
}

__global__ void backward_local_avgpool_layer_kernel(int n, int in_h, int in_w,
                                                    int in_c, int stride_x,
                                                    int stride_y, int size,
                                                    int pad, float *delta,
                                                    float *prev_delta)
...
extern "C" void forward_local_avgpool_layer_gpu(maxpool_layer layer,
                                                network_state state)
{
#ifdef CUDNN_DISABLED
  if (!state.train && layer.stride == layer.size) {
...
#endif
  {
    ...
    forward_local_avgpool_layer_kernel<<<cuda_gridsize(n), BLOCK, 0,
                                         get_cuda_stream()>>>(
        n, layer.h, layer.w, layer.c, layer.stride_x, layer.stride_y,
        layer.size, layer.pad, state.input, layer.output_gpu);
    CHECK_CUDA(cudaPeekAtLastError());
...
extern "C" void backward_local_avgpool_layer_gpu(maxpool_layer layer,
                                                 network_state state)
{
  size_t n = layer.h * layer.w * layer.c * layer.batch;

  backward_local_avgpool_layer_kernel<<<cuda_gridsize(n), BLOCK, 0,
                                        get_cuda_stream()>>>(
      n, layer.h, layer.w, layer.c, layer.stride_x, layer.stride_y, layer.size,
      layer.pad, layer.delta_gpu, state.delta);
  CHECK_CUDA(cudaPeekAtLastError());
}
