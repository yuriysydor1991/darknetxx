...
layer make_conv_lstm_layer(int batch, int h, int w, int c, int output_filters,
                           int groups, int steps, int size, int stride,
                           int dilation, int pad, ACTIVATION activation,
                           int batch_normalize, int peephole, int xnor,
                           int bottleneck, int train)
{
  ...
  *(l.uf) = make_convolutional_layer(batch, steps, h, w, c, output_filters,
                                     groups, size, stride, stride, dilation,
                                     pad, activation, batch_normalize, 0, xnor,
                                     0, 0, 0, 0, NULL, 0, 0, train);
  l.uf->batch = batch;
  if (l.workspace_size < l.uf->workspace_size)
    l.workspace_size = l.uf->workspace_size;

  l.ui = (layer *)xcalloc(1, sizeof(layer));
  *(l.ui) = make_convolutional_layer(batch, steps, h, w, c, output_filters,
                                     groups, size, stride, stride, dilation,
                                     pad, activation, batch_normalize, 0, xnor,
                                     0, 0, 0, 0, NULL, 0, 0, train);
  l.ui->batch = batch;
  ...
  *(l.ug) = make_convolutional_layer(batch, steps, h, w, c, output_filters,
                                     groups, size, stride, stride, dilation,
                                     pad, activation, batch_normalize, 0, xnor,
                                     0, 0, 0, 0, NULL, 0, 0, train);
  l.ug->batch = batch;
  ...
  *(l.uo) = make_convolutional_layer(batch, steps, h, w, c, output_filters,
                                     groups, size, stride, stride, dilation,
                                     pad, activation, batch_normalize, 0, xnor,
                                     0, 0, 0, 0, NULL, 0, 0, train);
  l.uo->batch = batch;
  ...
  if (l.bottleneck) {
    ...
    *(l.wf) = make_convolutional_layer(
        batch, steps, h, w, output_filters * 2, output_filters, groups, size,
    ...
    l.wf = (layer *)xcalloc(1, sizeof(layer));
    *(l.wf) = make_convolutional_layer(
        batch, steps, h, w, output_filters, output_filters, groups, size,
        stride, stride, dilation, pad, activation, batch_normalize, 0, xnor, 0,
        0, 0, 0, NULL, 0, 0, train);
    l.wf->batch = batch;
    if (l.workspace_size < l.wf->workspace_size)
...
layer make_history_layer(int batch, int h, int w, int c, int history_size,
                         int steps, int train)
{
  layer l = {(LAYER_TYPE)0};
  l.train = train;
  ...
  fprintf(stderr,
          "HISTORY b = %d, s = %2d, steps = %2d   %4d x%4d x%4d -> %4d x%4d "
          "x%4d \n",
          l.batch / l.steps, l.history_size, l.steps, w, h, c, l.out_w, l.out_h,
          l.out_c);
...
void forward_history_layer(layer l, network_state state)
{
  ...
  for (i = 0; i < l.steps; ++i) {
    ...
    for (b = 0; b < batch; ++b) {
      ...
      copy_cpu(shift_size, prev_output + b * l.outputs, 1, output + output_sift,
               1);
...
void forward_history_layer_gpu(const layer l, network_state state)
{
  ...
  // printf(" copy_size = %d, inputs = %d, batch = %d, steps = %d,
  // l.history_size = %d \n", copy_size, l.inputs, batch, l.steps,
  // l.history_size); simple_copy_ongpu(copy_size, state.input, l.output_gpu);
  // return;
  ...
  for (i = 0; i < l.steps; ++i) {
    ...
    for (b = 0; b < batch; ++b) {
      ...
      // copy_cpu(shift_size, prev_output + b*l.outputs, 1, output +
      // output_sift, 1);
      simple_copy_ongpu(shift_size, prev_output + b * l.outputs,
                        output + output_sift);
      ...
      for (h = 1; h < l.history_size; ++h) {
        // scal_ongpu(l.inputs, (l.history_size - h)/ (float)l.history_size,
        // output + h*l.inputs, 1); scal_ongpu(l.inputs, 0, output + h*l.inputs,
        // 1);
      }
    }
  ...
  int output_start = (l.steps - 1) * l.outputs * batch;
  // copy_cpu(batch*l.outputs, l.output + output_start, 1, l.prev_state_cpu, 1);
  simple_copy_ongpu(batch * l.outputs, l.output_gpu + output_start,
                    l.prev_state_gpu);
}
...
void backward_history_layer_gpu(const layer l, network_state state)
{
  ...
  // printf(" copy_size = %d, inputs = %d, batch = %d, steps = %d,
  // l.history_size = %d \n", copy_size, l.inputs, batch, l.steps,
  // l.history_size); axpy_ongpu(copy_size, 1, l.delta_gpu, 1, state.delta, 1);
  // return;
...
void update_conv_lstm_layer(layer l, int batch, float learning_rate,
...
void resize_conv_lstm_layer(layer *l, int w, int h)
{
  ...
  l->output =
      (float *)xrealloc(l->output, outputs * batch * steps * sizeof(float));
  // l->state = (float*)xrealloc(l->state, outputs * batch * sizeof(float));

  l->prev_state_cpu =
      (float *)xrealloc(l->prev_state_cpu, batch * outputs * sizeof(float));
  l->prev_cell_cpu =
      (float *)xrealloc(l->prev_cell_cpu, batch * outputs * sizeof(float));
  l->cell_cpu =
      (float *)xrealloc(l->cell_cpu, batch * outputs * steps * sizeof(float));
  ...
  l->temp2_cpu =
      (float *)xrealloc(l->temp2_cpu, batch * outputs * sizeof(float));
  l->temp3_cpu =
      (float *)xrealloc(l->temp3_cpu, batch * outputs * sizeof(float));
  l->dc_cpu = (float *)xrealloc(l->dc_cpu, batch * outputs * sizeof(float));
  l->dh_cpu = (float *)xrealloc(l->dh_cpu, batch * outputs * sizeof(float));
  l->stored_c_cpu =
      (float *)xrealloc(l->stored_c_cpu, batch * outputs * sizeof(float));
  l->stored_h_cpu =
      (float *)xrealloc(l->stored_h_cpu, batch * outputs * sizeof(float));
...
void forward_conv_lstm_layer(layer l, network_state state)
{
  ...
  for (i = 0; i < l.steps; ++i) {
    ...
    assert(wf.c == l.out_c && wi.c == l.out_c && wg.c == l.out_c &&
           wo.c == l.out_c);
...
void backward_conv_lstm_layer(layer l, network_state state)
{
  ...
  for (i = l.steps - 1; i >= 0; --i) {
    ...
    // delta for c,f,i,g(w,u,v): temp2 = delta * o * grad_tanh(tanh(c)) +
    // delta_c(???) delta for output:         temp3 = delta
...
void update_conv_lstm_layer_gpu(layer l, int batch, float learning_rate,
                                float momentum, float decay, float loss_scale)
{
  if (l.peephole) {
    update_convolutional_layer_gpu(*(l.vf), batch, learning_rate, momentum,
                                   decay, loss_scale);
    update_convolutional_layer_gpu(*(l.vi), batch, learning_rate, momentum,
                                   decay, loss_scale);
    update_convolutional_layer_gpu(*(l.vo), batch, learning_rate, momentum,
                                   decay, loss_scale);
  }
  update_convolutional_layer_gpu(*(l.wf), batch, learning_rate, momentum, decay,
                                 loss_scale);
  if (!l.bottleneck) {
    update_convolutional_layer_gpu(*(l.wi), batch, learning_rate, momentum,
                                   decay, loss_scale);
    update_convolutional_layer_gpu(*(l.wg), batch, learning_rate, momentum,
                                   decay, loss_scale);
    update_convolutional_layer_gpu(*(l.wo), batch, learning_rate, momentum,
                                   decay, loss_scale);
  }
...
void forward_conv_lstm_layer_gpu(layer l, network_state state)
{
  ...
  for (i = 0; i < l.steps; ++i) {
    ...
    if (l.bottleneck) {
      ...
      simple_copy_ongpu(l.outputs * l.batch, state.input,
                        l.bottelneck_hi_gpu + l.outputs * l.batch);
      s.input = l.bottelneck_hi_gpu;
      forward_convolutional_layer_gpu(wf, s);  // 2x input channels
      activate_array_ongpu(wf.output_gpu, l.outputs * l.batch,
                           l.lstm_activation);
    ...
    // f = wf + uf + vf
    add_3_arrays_activate((l.bottleneck) ? NULL : wf.output_gpu, uf.output_gpu,
                          (l.peephole) ? vf.output_gpu : NULL,
    ...
    // if (l.peephole) axpy_ongpu(l.outputs*l.batch, 1, vf.output_gpu, 1,
    // l.f_gpu, 1); activate_array_ongpu(l.f_gpu, l.outputs*l.batch, LOGISTIC);

    // i = wi + ui + vi
    add_3_arrays_activate((l.bottleneck) ? NULL : wi.output_gpu, ui.output_gpu,
                          (l.peephole) ? vi.output_gpu : NULL,
    ...
    // if (l.peephole) axpy_ongpu(l.outputs*l.batch, 1, vi.output_gpu, 1,
    // l.i_gpu, 1); activate_array_ongpu(l.i_gpu, l.outputs*l.batch, LOGISTIC);

    // g = wg + ug
    add_3_arrays_activate((l.bottleneck) ? NULL : wg.output_gpu, ug.output_gpu,
                          NULL, l.outputs * l.batch, l.lstm_activation,
                          l.g_gpu);
    ...
    // c = f*c + i*g
    sum_of_mults(l.f_gpu, l.c_gpu, l.i_gpu, l.g_gpu, l.outputs * l.batch,
                 l.c_gpu);  // decreases mAP???
    ...
    add_3_arrays_activate((l.bottleneck) ? NULL : wo.output_gpu, uo.output_gpu,
                          (l.peephole) ? vo.output_gpu : NULL,
    ...
    // if (l.peephole) axpy_ongpu(l.outputs*l.batch, 1, vo.output_gpu, 1,
    // l.o_gpu, 1); activate_array_ongpu(l.o_gpu, l.outputs*l.batch, LOGISTIC);

    // h = o * tanh(c)
    activate_and_mult(l.c_gpu, l.o_gpu, l.outputs * l.batch, l.lstm_activation,
                      l.h_gpu);
    ...
    fix_nan_and_inf(l.c_gpu,
                    l.outputs * l.batch);  // should be fix_nan_and_inf()
    fix_nan_and_inf(l.h_gpu,
                    l.outputs * l.batch);  // should be fix_nan_and_inf()
    if (l.state_constrain)
    ...
    simple_copy_ongpu(
        l.outputs * l.batch, l.h_gpu,
        l.output_gpu);  // is required for both Detection and Training

    if (l.shortcut) {
      // partial residual connection
      if (l.bottleneck)
        axpy_ongpu(l.outputs * l.batch / 2, 1, wf.output_gpu, 1, l.output_gpu,
                   1);
...
void backward_conv_lstm_layer_gpu(layer l, network_state state)
{
  ...
  for (i = l.steps - 1; i >= 0; --i) {
    if (i != 0)
      simple_copy_ongpu(l.outputs * l.batch, l.cell_gpu - l.outputs * l.batch,
                        l.prev_cell_gpu);
    // else fill_ongpu(l.outputs * l.batch, 0, l.prev_cell_gpu, 1);   //  dont
    // use
    else if (state.net.current_subdivision % sequence != 0)
      simple_copy_ongpu(l.outputs * l.batch, l.last_prev_cell_gpu,
                        l.prev_cell_gpu);

    simple_copy_ongpu(l.outputs * l.batch, l.cell_gpu, l.c_gpu);

    if (i != 0)
      simple_copy_ongpu(l.outputs * l.batch, l.output_gpu - l.outputs * l.batch,
                        l.prev_state_gpu);
    // else fill_ongpu(l.outputs * l.batch, 0, l.prev_state_gpu, 1);   //  dont
    // use
    else if (state.net.current_subdivision % sequence != 0)
      simple_copy_ongpu(l.outputs * l.batch, l.last_prev_state_gpu,
                        l.prev_state_gpu);
    ...
    simple_copy_ongpu(l.outputs * l.batch, l.delta_gpu,
                      l.temp3_gpu);  // temp3 = delta

    simple_copy_ongpu(l.outputs * l.batch, l.c_gpu, l.temp_gpu);
    activate_array_ongpu(l.temp_gpu, l.outputs * l.batch,
                         l.lstm_activation);  // temp  = tanh(c)

    simple_copy_ongpu(l.outputs * l.batch, l.temp3_gpu, l.temp2_gpu);
    mul_ongpu(l.outputs * l.batch, l.o_gpu, 1, l.temp2_gpu,
              1);  // temp2 = delta * o

    gradient_array_ongpu(
        l.temp_gpu, l.outputs * l.batch, l.lstm_activation,
    ...
    mul_ongpu(l.outputs * l.batch, l.temp3_gpu, 1, l.temp_gpu,
              1);  // temp  = delta * tanh(c)
    gradient_array_ongpu(
        l.o_gpu, l.outputs * l.batch, LOGISTIC,
    ...
    gradient_array_ongpu(l.g_gpu, l.outputs * l.batch, l.lstm_activation,
                         l.temp_gpu);
    // delta for c,f,i,g(w,u,v): temp = (delta * o * grad_tanh(tanh(c)) +
    // delta_c(???)) * i * grad_tanh(g)
    ...
    if (l.bottleneck) {
      // l.bottelneck_hi_gpu size is 2x
      simple_copy_ongpu(l.outputs * l.batch, l.prev_state_gpu,
                        l.bottelneck_hi_gpu);
      simple_copy_ongpu(l.outputs * l.batch, state.input,
                        l.bottelneck_hi_gpu + l.outputs * l.batch);
      ...
      gradient_array_ongpu(wf.output_gpu, l.outputs * l.batch,
                           l.lstm_activation, wf.delta_gpu);
    ...
    if (l.bottleneck) {
      ...
      if (l.dh_gpu)
        axpy_ongpu(l.outputs * l.batch, l.time_normalizer,
                   l.bottelneck_delta_gpu, 1, l.dh_gpu, 1);
      axpy_ongpu(l.outputs * l.batch, 1,
                 l.bottelneck_delta_gpu + l.outputs * l.batch, 1, state.delta,
                 1);  // lead to nan
    } else {
      // if (l.dh_gpu) axpy_ongpu(l.outputs*l.batch, l.time_normalizer,
      // l.temp3_gpu, 1, l.dh_gpu, 1);
    }
    ...
    // delta for c,f,i,g(w,u,v): delta_c = temp = (delta * o *
    // grad_tanh(tanh(c)) + delta_c(???)) * f    // (grad_linear(c)==1)

    state.input -= l.inputs * l.batch;
    if (state.delta)
      state.delta -=
          l.inputs * l.batch;  // new delta: state.delta = prev_layer.delta_gpu;
    l.output_gpu -= l.outputs * l.batch;
    l.cell_gpu -= l.outputs * l.batch;
  ...
  // if (get_current_batch(state.net) % 100) free_state_conv_lstm(l);  // dont
  // use
}
#endif
