...
#include "utils.h"
void reorg_cpu(float *x, int out_w, int out_h, int out_c, int batch, int stride,
    ...
    for (k = 0; k < out_c; ++k) {
      for (j = 0; j < out_h; ++j) {
        for (i = 0; i < out_w; ++i) {
          ...
          int out_index =
              w2 + out_w * stride * (h2 + out_h * stride * (c2 + in_c * b));
          if (forward)
            out[out_index] =
                x[in_index];  // used by default for forward (i.e. forward = 0)
          else
...
void shortcut_multilayer_cpu(int size, int src_outputs, int batch, int n,
                             ...
                             float *out, float *in, float *weights,
                             int nweights,
...
void backward_shortcut_multilayer_cpu(
    int size, int src_outputs, int batch, int n, int *outputs_of_layers,
...
#pragma omp parallel for
  for (id = 0; id < size; ++id) {
    ...
    if (weights && weights_normalization) {
      ...
      grad = 0;
      for (i = 0; i < (n + 1); ++i) {
          const int weights_index = src_i / step + i*layer_step;  // [0 or c or
      (c, h ,w)] const float delta_w = delta_in[id] * in[id]; const float w =
      weights[weights_index]; if (weights_normalization == RELU_NORMALIZATION)
      grad += delta_w * relu(w) / sum; else if (weights_normalization ==
      SOFTMAX_NORMALIZATION) grad += delta_w * expf(w - max_val) / sum;
      }
      */
    }
...
void get_embedding(float *src, int src_w, int src_h, int src_c,
...
int get_sim_P_index(size_t i, size_t j, contrastive_params *contrast_p,
                    int contrast_p_size)
{
  size_t z;
...
int check_sim(size_t i, size_t j, contrastive_params *contrast_p,
              int contrast_p_size)
{
  size_t z;
  for (z = 0; z < contrast_p_size; ++z) {
...
float find_sim(size_t i, size_t j, contrastive_params *contrast_p,
               int contrast_p_size)
{
  size_t z;
  for (z = 0; z < contrast_p_size; ++z) {
...
float find_P_constrastive(size_t i, size_t j, contrastive_params *contrast_p,
                          int contrast_p_size)
{
  size_t z;
  for (z = 0; z < contrast_p_size; ++z) {
  ...
  if (z == contrast_p_size) {
    printf(
        " Error: find_P_constrastive(): P isn't found: i = %zu, j = %zu, z = "
        "%zu \n",
        i, j, z);
    error("Error!", DARKNET_LOC);
  }
...
float P_constrastive_f_det(size_t il, int *labels, float **z,
                           unsigned int feature_size, float temperature,
...
// num_of_samples = 2 * loaded_images = mini_batch_size
float P_constrastive_f(size_t i, size_t l, int *labels, float **z,
                       unsigned int feature_size, float temperature,
                       contrastive_params *contrast_p, int contrast_p_size)
{
  if (i == l) {
    fprintf(
        stderr,
        " Error: in P_constrastive must be i != l, while i = %zu, l = %zu \n",
        i, l);
    error("Error!", DARKNET_LOC);
  }

  const float sim = find_sim(
      i, l, contrast_p,
      contrast_p_size);  // cosine_similarity(z[i], z[l], feature_size);
  const float numerator = expf(sim / temperature);
...
void grad_contrastive_loss_positive_f(size_t i, int *class_ids, int *labels,
                                      size_t num_of_samples, float **z,
                                      unsigned int feature_size,
                                      float temperature, float *delta, int wh,
                                      ...
                                      int contrast_p_size)
{
  ...
  if (N == 0 || temperature == 0 || vec_len == 0) {
    fprintf(stderr,
            " Error: N == 0 || temperature == 0 || vec_len == 0. N=%f, "
            "temperature=%f, vec_len=%f, labels[i] = %d \n",
  ...
  for (j = 0; j < num_of_samples; ++j) {
    ...
    if (i != j && labels[i] == labels[j] && labels[i] >= 0) {
      // printf(" i = %d, j = %d, num_of_samples = %d, labels[i] = %d, labels[j]
      // = %d \n",
      ...
      // const float sim = find_sim(i, j, contrast_p, contrast_p_size);
      // //cos_sim[i*num_of_samples + j];        // cosine_similarity(z[i],
      // z[j], feature_size); const float P = find_P_constrastive(i, j,
      // contrast_p, contrast_p_size); //p_constrastive[i*num_of_samples + j];
      // // P_constrastive(i, j, labels, num_of_samples, z, feature_size,
      // temperature, cos_sim); const float custom_pos_mult = 1 - sim;

      int m;
      // const float d = mult*(sim * z[i][m] - z[j][m]) * (1 - P); // 1
      for (m = 0; m < feature_size; ++m) {
        // const float d = mult*(sim * z[j][m] - z[j][m]) * (1 - P); // my
        // const float d = mult*(sim * z[i][m] + sim * z[j][m] - z[j][m]) *(1 -
        // P); // 1+2
...
void grad_contrastive_loss_negative_f(size_t i, int *class_ids, int *labels,
                                      size_t num_of_samples, float **z,
                                      unsigned int feature_size,
                                      float temperature, float *delta, int wh,
                                      contrastive_params *contrast_p,
                                      int contrast_p_size, int neg_max)
{
  const float vec_len = math_vector_length(z[i], feature_size);
  size_t j;
  float N = 0;
  ...
  for (j = 0; j < num_of_samples; ++j) {
    ...
    if (labels[i] >= 0 && labels[i] == labels[j] && i != j) {
      ...
      for (k = 0; k < num_of_samples; ++k) {
        ...
        if (k != i && k != j && labels[k] != labels[i] &&
            class_ids[j] == class_ids[k]) {
          neg_counter++;
          const int sim_P_i =
              get_sim_P_index(i, k, contrast_p, contrast_p_size);
          if (sim_P_i < 0) continue;
          const float sim = contrast_p[sim_P_i].sim;
          const float P = contrast_p[sim_P_i].P;
          // if (!check_sim(i, k, contrast_p, contrast_p_size)) continue;
          // const float sim = find_sim(i, k, contrast_p, contrast_p_size);
          // //cos_sim[i*num_of_samples + k];        // cosine_similarity(z[i],
          // z[k], feature_size); const float P = find_P_constrastive(i, k,
          // contrast_p, contrast_p_size); //p_constrastive[i*num_of_samples +
          // k];   // P_constrastive(i, k, labels, num_of_samples, z,
          // feature_size, temperature, cos_sim); const float custom_pos_mult =
          // 1 + sim;
          ...
          for (m = 0; m < feature_size; ++m) {
            ...
            // const float d = mult*(z[k][m] - sim * z[k][m] - sim * z[i][m]) *
            // P;   // 1+2
            const float d = mult * (z[k][m] - sim * z[i][m]) * P;  // 1 (70%)
...
// num_of_samples = 2 * loaded_images = mini_batch_size
float P_constrastive(size_t i, size_t l, int *labels, size_t num_of_samples,
                     float **z, unsigned int feature_size, float temperature,
                     float *cos_sim, float *exp_cos_sim)
{
  if (i == l) {
    fprintf(
        stderr,
  ...
  // const float sim = cos_sim[i*num_of_samples + l]; // cosine_similarity(z[i],
  // z[l], feature_size); const float numerator = expf(sim / temperature);
  ...
  for (k = 0; k < num_of_samples; ++k) {
    // if (k != i && labels[k] != labels[i]) {
    if (k != i) {
      // const float sim_den = cos_sim[k*num_of_samples + l]; //
      // cosine_similarity(z[k], z[l], feature_size); denominator +=
      // expf(sim_den / temperature);
...
// labels[num_of_samples] - array with class_id for each sample in the current
// mini_batch z[feature_size][num_of_samples] - array of arrays with contrastive
// features (output of conv-layer, f.e. 128 floats for each sample)
// delta[feature_size] - array with deltas for backpropagation
// temperature - scalar temperature param (temperature > 0), f.e. temperature =
// 0.07: Supervised Contrastive Learning
void grad_contrastive_loss_positive(size_t i, int *labels,
                                    size_t num_of_samples, float **z,
                                    unsigned int feature_size,
                                    float temperature, float *cos_sim,
                                    float *p_constrastive, float *delta, int wh)
{
  const float vec_len = math_vector_length(z[i], feature_size);
  size_t j;
  float N = 0;
  ...
  for (j = 0; j < num_of_samples; ++j) {
    ...
    if (i != j && labels[i] == labels[j]) {
      ...
      //     i, j, num_of_samples, labels[i], labels[j]);
      const float sim =
          cos_sim[i * num_of_samples +
                  j];  // cosine_similarity(z[i], z[j], feature_size);
      const float P =
          p_constrastive[i * num_of_samples +
                         j];  // P_constrastive(i, j, labels, num_of_samples, z,
...
// temperature - scalar temperature param (temperature > 0), f.e. temperature =
// 0.07: Supervised Contrastive Learning
void grad_contrastive_loss_negative(size_t i, int *labels,
                                    size_t num_of_samples, float **z,
                                    unsigned int feature_size,
                                    float temperature, float *cos_sim,
                                    float *p_constrastive, float *delta, int wh)
    ...
    if (i != j && labels[i] == labels[j]) {
      ...
      for (k = 0; k < num_of_samples; ++k) {
        ...
        if (k != i && k != j && labels[k] >= 0) {
          ...
          const float P =
              p_constrastive[i * num_of_samples +
                             k];  // P_constrastive(i, k, labels,
                                  // num_of_samples, z, feature_size,
                                  // temperature, cos_sim);
          // const float custom_pos_mult = 1 + sim;
