...
void *fetch_in_thread(void *ptr)
{
  while (!custom_atomic_load_int(&flag_exit)) {
    while (!custom_atomic_load_int(&run_fetch_in_thread)) {
      ...
      if (demo_skip_frame) consume_frame(cap);
      this_thread_yield();
    }
    int dont_close_stream = 0;  // set 1 if your IP-camera periodically turns
                                // off and turns on video-stream
    if (letter_box)
      in_s = get_image_from_stream_letterbox(cap, net.w, net.h, net.c, &in_img,
                                             dont_close_stream);
    else
  ...
  while (!custom_atomic_load_int(&flag_exit)) {
    ...
    if (letter_box)
      dets = get_network_boxes(&net, get_width_mat(in_img),
                               get_height_mat(in_img), demo_thresh, demo_thresh,
                               0, 1, &nboxes, 1);  // letter box
    else
      dets = get_network_boxes(&net, net.w, net.h, demo_thresh, demo_thresh, 0,
                               1, &nboxes, 0);  // resized

    // const float nms = .45;
    // if (nms) {
    //     if (l.nms_kind == DEFAULT_NMS) do_nms_sort(dets, nboxes, l.classes,
    //     nms); else diounms_sort(dets, nboxes, l.classes, nms, l.nms_kind,
    //     l.beta_nms);
    // }
...
void demo(char *cfgfile, char *weightfile, float thresh, float hier_thresh,
          ...
          int avgframes, int frame_skip, char *prefix, char *out_filename,
          int mjpeg_port, int dontdraw_bbox, int json_port, int dont_show,
          int ext_output, int letter_box_in, int time_limit_sec,
          ...
          char *json_file_output)
{
...
#ifdef WIN32
    printf(
        "Check that you have copied file opencv_ffmpeg340_64.dll to the same "
        "directory where is darknet.exe \n");
#endif
  ...
  if (l.classes != demo_classes) {
    printf(
        "\n Parameters don't match: in cfg-file classes=%d, in data-file "
        "classes=%d \n",
    ...
    ++count;
    {
      ...
      if (!benchmark)
        custom_atomic_store_int(
            &run_fetch_in_thread,
            1);  // if (custom_create_thread(&fetch_thread, 0, fetch_in_thread,
                 // 0)) error("Thread creation failed", DARKNET_LOC);
      custom_atomic_store_int(
          &run_detect_in_thread,
          1);  // if (custom_create_thread(&detect_thread, 0, detect_in_thread,
               // 0)) error("Thread creation failed", DARKNET_LOC);

      // if (nms) do_nms_obj(local_dets, local_nboxes, l.classes, nms);    //
      // bad results
      if (nms) {
        if (l.nms_kind == DEFAULT_NMS)
          do_nms_sort(local_dets, local_nboxes, l.classes, nms);
        else
          diounms_sort(local_dets, local_nboxes, l.classes, nms, l.nms_kind,
                       l.beta_nms);
      }

      if (l.embedding_size)
        set_track_id(local_dets, local_nboxes, demo_thresh, l.sim_thresh,
      ...
      if (json_file_output) {
        ...
        json_buf = detection_to_json(local_dets, local_nboxes, l.classes,
                                     demo_names, frame_id, NULL);
        fwrite(json_buf, sizeof(char), strlen(json_buf), json_file);
        free(json_buf);
      }

      // char *http_post_server =
      // "webhook.site/898bbd9b-0ddd-49cf-b81d-1f56be98d870";
      if (http_post_host && !send_http_post_once) {
        ...
        if (send_http_post_request(http_post_host, http_post_port, filename,
                                   local_dets, nboxes, classes, names, frame_id,
                                   ext_output, timeout)) {
      ...
      // if you run it with param -mjpeg_port 8090  then open URL in your
      ...
      while (custom_atomic_load_int(&run_detect_in_thread)) {
        if (avg_fps > 50) this_thread_yield();
          ...
          this_thread_sleep_for(
              thread_wait_ms);  // custom_join(detect_thread, 0);
      }
      if (!benchmark) {
        while (custom_atomic_load_int(&run_fetch_in_thread)) {
          if (avg_fps > 50) this_thread_yield();
            ...
            this_thread_sleep_for(
                thread_wait_ms);  // custom_join(fetch_thread, 0);
        }
        free_image(det_s);
      }

      if (time_limit_sec > 0 &&
          (get_time_point() - start_time_lim) / 1000000 > time_limit_sec) {
        printf(
            " start_time_lim = %f, get_time_point() = %f, time spent = %f \n",
