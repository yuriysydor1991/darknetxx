...
void check_error(cudaError_t status, const char *const filename,
...
void check_error_extended(cudaError_t status, const char *const filename,
                          const char *const funcname, const int line)
{
  if (status != cudaSuccess) {
  ...
  if (cuda_debug_sync) {
    ...
    if (status != cudaSuccess)
      printf(
          "CUDA status = cudaDeviceSynchronize() Error: file: %s: func: %s() "
          "line: %d\n",
  ...
  if (!streamInit[i]) {
    ...
    if (status != cudaSuccess) {
      ...
      status = cudaStreamCreateWithFlags(
          &streamsArray[i], cudaStreamNonBlocking);  // cudaStreamDefault
      CHECK_CUDA(status);
    }
    streamInit[i] = 1;
  }
...
/*
static cudaStream_t streamsArray2[16];    // cudaStreamSynchronize(
get_cuda_memcpy_stream() ); static int streamInit2[16] = { 0 };
...
cudaStreamNonBlocking); cudaError_t status =
cudaStreamCreateWithFlags(&streamsArray2[i], cudaStreamNonBlocking); if (status
!= cudaSuccess) { printf(" cudaStreamCreate-Memcpy error: %d \n", status); const
char *s = cudaGetErrorString(status); printf("CUDA Error: %s\n", s); status =
cudaStreamCreateWithFlags(&streamsArray2[i], cudaStreamNonBlocking);
            CHECK_CUDA(status);
        }
        streamInit2[i] = 1;
    }
...
void cudnn_check_error(cudnnStatus_t status, const char *const filename,
                                ...
                                const char *const function, const int line)
{
  ...
  if (cuda_debug_sync) {
    ...
    if (status != CUDNN_STATUS_SUCCESS)
      printf(
          "\n cudaError_t status = cudaDeviceSynchronize() Error in: file: %s "
          "function: %s() line: %d\n",
          filename, function, line);
  }
  cudnn_check_error(status, filename, function, line);
...
cudaStream_t switch_stream(int i)
{
  ...
  if (!switchStreamInit[i]) {
    CHECK_CUDA(cudaStreamCreateWithFlags(&switchStreamsArray[i],
...
void wait_stream(int i)
{
  ...
  CHECK_CUDA(cudaEventCreateWithFlags(&switchEventsArray[event_counter],
                                      cudaEventDisableTiming));
  // printf(" create event = %d (wait for stream = %d) \n", event_counter, i);

  // CHECK_CUDA(cudaEventRecordWithFlags(switchEventsArray[i],
  // switchStreamsArray[i], cudaEventRecordExternal) );
  CHECK_CUDA(
      cudaEventRecord(switchEventsArray[event_counter], switchStreamsArray[i]));
  CHECK_CUDA(cudaStreamWaitEvent(streamsArray[dev_id],
                                 switchEventsArray[event_counter],
                                 cudaEventWaitDefault));
  // cudaStreamWaitEvent(streamsArray[dev_id], switchEventsArray[i],
...
static const size_t pinned_block_size =
...
void pre_allocate_pinned_memory(const size_t size)
{
  ...
  if (!pinned_ptr) {
    ...
    printf(
        "pre_allocate: size = %zu MB, num_of_blocks = %zu, block_size = %zu MB "
        "\n",
...
float *cuda_make_array_pinned_preallocated(float *x, size_t n)
{
  ...
  if (!x_cpu) {
    if (allocation_size > pinned_block_size / 2) {
      ...
      cudaError_t status =
          cudaHostAlloc((void **)&pinned_ptr[pinned_block_id],
...
float *cuda_make_array_pinned(float *x, size_t n)
{
  ...
  cudaError_t status =
      cudaHostAlloc((void **)&x_gpu, size, cudaHostRegisterMapped);
  if (status != cudaSuccess)
...
int *cuda_make_int_array_new_api(int *x, size_t n)
{
  ...
  if (x) {
    // status = cudaMemcpy(x_gpu, x, size, cudaMemcpyHostToDevice);
    cudaError_t status = cudaMemcpyAsync(x_gpu, x, size, cudaMemcpyHostToDevice,
                                         get_cuda_stream());
    CHECK_CUDA(status);
  }
  if (!x_gpu) error("Cuda malloc failed", DARKNET_LOC);
...
#ifdef CUDNN
  fprintf(stderr, ", cuDNN: %d.%d.%d", CUDNN_MAJOR, CUDNN_MINOR,
          CUDNN_PATCHLEVEL);
#endif  // CUDNN
#ifdef CUDNN_HALF
