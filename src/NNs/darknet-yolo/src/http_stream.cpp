...
//
// a single-threaded, multi client(using select), debug webserver - streaming
// out mjpg.
//  on win, _WIN32 has to be defined, must link against ws2_32.lib (socks on
//  linux are for free)
//
...
// Graceful closes will first close their output channels and then wait for the
// peer on the other side of the connection to close its output channels. When
// both sides are done telling each other they won,t be sending any more data
// (i.e., closing output channels), the connection can be closed fully, with no
// risk of reset.
static int close_socket(SOCKET s)
{
  int close_output =
      ::shutdown(s, 1);  // 0 close input, 1 close output, 2 close both
...
struct _IGNORE_PIPE_SIGNAL
{
  ...
  _IGNORE_PIPE_SIGNAL()
  {
    ...
    // sigaction (SIGPIPE, &old_actn, NULL); // - to restore the previous signal
    // handling
  }
} _init_once;
...
static int close_socket(SOCKET s)
{
  ...
  int close_input = ::shutdown(s, 0);
  int result = close(s);
  std::cerr << "Close socket: out = " << close_output
            << ", in = " << close_input << " \n";
  return result;
}
#endif  // _WIN32
...
class JSON_sender
{
 ...
 public:
  ...
  bool open(int port)
  {
    ...
    if (result != NO_ERROR) {
      std::cerr << "ioctlsocket(FIONBIO) failed with error: " << result
                << std::endl;
    }
#else   // WIN32
    ...
    if (::bind(sock, (SOCKADDR *)&address, sizeof(SOCKADDR_IN)) ==
        SOCKET_ERROR) {
      cerr << "error JSON_sender: couldn't bind sock " << sock << " to port "
           << port << "!" << endl;
      return release();
    }
    if (::listen(sock, 10) == SOCKET_ERROR) {
      cerr << "error JSON_sender: couldn't listen on sock " << sock
           << " on port " << port << " !" << endl;
      return release();
    }
    FD_ZERO(&master);
    ...
    for (int s = 0; s <= maxfd; s++) {
      ...
      if (s == sock)  // request on master socket, accept and send main header.
      {
        ...
        if (client == SOCKET_ERROR) {
          cerr << "error JSON_sender: couldn't accept connection on sock "
               << sock << " !" << endl;
          return false;
        }
        if (setsockopt(client, SOL_SOCKET, SO_RCVTIMEO, (char *)&socket_timeout,
                       sizeof(socket_timeout)) < 0) {
          cerr << "error JSON_sender: SO_RCVTIMEO setsockopt failed\n";
        }
        if (setsockopt(client, SOL_SOCKET, SO_SNDTIMEO, (char *)&socket_timeout,
                       sizeof(socket_timeout)) < 0) {
      ...
      {
        // char head[400];
        //  application/x-resource+json or application/x-collection+json -  when
        //  you are representing REST resources and collections application/json
        //  or text/json or text/javascript or text/plain.
               ...
               long long int frame_id, int port, int timeout)
{
  try {
    char *send_buf =
        detection_to_json(dets, nboxes, classes, names, frame_id, NULL);
...
class MJPG_sender
{
 ...
 public:
  ...
  bool open(int port)
  {
    ...
    if (::bind(sock, (SOCKADDR *)&address, sizeof(SOCKADDR_IN)) ==
        SOCKET_ERROR) {
      cerr << "error MJPG_sender: couldn't bind sock " << sock << " to port "
           << port << "!" << endl;
      return release();
    }
    if (::listen(sock, 10) == SOCKET_ERROR) {
      cerr << "error MJPG_sender: couldn't listen on sock " << sock
           << " on port " << port << " !" << endl;
      return release();
    }
    FD_ZERO(&master);
  ...
  bool write(const Mat &frame)
  {
    ...
    for (int s = 0; s <= maxfd; s++) {
      ...
      if (s == sock)  // request on master socket, accept and send main header.
      {
        ...
        SOCKET client = ::accept(sock, (SOCKADDR *)&address, &addrlen);
        if (client == SOCKET_ERROR) {
          cerr << "error MJPG_sender: couldn't accept connection on sock "
               << sock << " !" << endl;
          return false;
        }
        if (setsockopt(client, SOL_SOCKET, SO_RCVTIMEO, (char *)&socket_timeout,
      ...
      } else  // existing client, just stream pix
      {
        ...
        cerr << "known client: " << s << ", sent = " << n
             << ", must be sent outlen = " << outlen << endl;
        if (n < (int)outlen) {
...
#ifdef __CYGWIN__
int send_http_post_request(char *http_post_host, int server_port,
                           const char *videosource, detection *dets, int nboxes,
                           int classes, char **names, long long int frame_id,
                           ...
                           int ext_output, int timeout)
{
  ...
  for (int i = 0; i < nboxes; ++i) {
    ...
    if (class_id >= 0) {
      message += std::string(names[class_id]) + std::string(", id: ") +
  ...
  if (!message.empty()) {
    ...
    std::string http_path = http_post_host_str.substr(
        slash_index, http_post_host_str.length() - slash_index);
...
int get_num_threads() { return std::thread::hardware_concurrency(); }
...
int fill_remaining_id(detection *new_dets, int new_dets_num, int new_track_id,
                      float thresh, int detection_count)
{
...
void set_track_id(detection *new_dets, int new_dets_num, float thresh,
                  float sim_thresh, float track_ciou_norm, int deque_size,
                  int dets_for_track, int dets_for_show)
{
  ...
  for (int old_id = 0; old_id < old_dets.size(); ++old_id) {
    for (int new_id = 0; new_id < new_dets_num; ++new_id) {
      ...
      const float sim = cosine_similarity(new_dets[new_id].embeddings,
                                          old_dets[old_id].embeddings,
  ...
  std::sort(sim_det.begin(), sim_det.end(),
            [](similarity_detections_t v1, similarity_detections_t v2) {
              return v1.sim > v2.sim;
            });
  ...
  for (int index = 0; index < new_dets_num * old_dets.size(); ++index) {
    ...
    // printf(" ciou = %f \n", box_ciou(new_dets[new_id].bbox,
    // old_dets[old_id].bbox));
