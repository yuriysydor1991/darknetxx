...
__global__ void dropblock_fast_kernel(float *rand, float prob, int w, int h,
                                      int spatial, int filters, int batch,
                                      int block_size, float *drop_blocks_scale,
                                      float *output)
{
...
__global__ void set_scales_dropblock_kernel(float *drop_blocks_scale,
                                            int block_size_w, int block_size_h,
                                            int outputs, int batch)
{
...
__global__ void scale_dropblock_kernel(float *output, int size, int outputs,
  ...
  if (l.dropblock) {
    ...
    /*
    {
        ...
        for (int b = 0; b < l.batch; ++b) {
            ...
            avg_scale += scale;
            printf(" %d x %d - block_size = %d, block_size*block_size = %d , ",
    l.w, l.h, block_size, block_size*block_size); printf(" ,
    l.drop_blocks_scale[b] = %f, scale = %f \t cur_prob = %f, cur_scale = %f
