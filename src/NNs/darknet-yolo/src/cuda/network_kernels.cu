...
void forward_network_gpu(network net, network_state state)
{
    ...
    /*
            ...
            if (l.out_w >= 0 && l.out_h >= 1 && l.c >= 3) {
                ...
                for (j = 0; j < l.out_c; ++j) {
                    ...
                    memcpy(img.data, l.output + l.out_w*l.out_h*j,
       l.out_w*l.out_h * 1 * sizeof(float)); memcpy(img.data + l.out_w*l.out_h *
       1, l.output + l.out_w*l.out_h*j, l.out_w*l.out_h * 1 * sizeof(float));
                    memcpy(img.data + l.out_w*l.out_h * 2, l.output +
       l.out_w*l.out_h*j, l.out_w*l.out_h * 1 * sizeof(float)); char buff[256];
  ...
  if (net.benchmark_layers) {
    ...
    for (i = 0; i < net.n; ++i) {
      ...
      // avg_time_per_layer[i].layer_id, avg_time_per_layer[i].layer_type,
      // avg_time_per_layer[i].time);
...
void backward_network_gpu(network net, network_state state)
{
  ...
  for (i = net.n - 1; i >= 0; --i) {
    ...
    if (i != 0) {
      ...
      if (net.optimized_memory && state.delta && !prev.keep_delta_gpu) {
        if (prev.delta_gpu != state.delta)
          simple_copy_ongpu(prev.outputs * prev.batch, state.delta,
                            prev.delta_gpu);
    ...
    if(i != 0)
    {
        ...
        int state_delta_nan_inf = is_nan_or_inf(state.delta, l.outputs *
    l.batch); int state_input_nan_inf = is_nan_or_inf(state.input, l.outputs *
    l.batch); printf("\n i - %d  is_nan_or_inf(s.delta) = %d \n", i,
    state_delta_nan_inf); printf(" i - %d  is_nan_or_inf(s.input) = %d \n", i,
    state_input_nan_inf); if (state_delta_nan_inf || state_input_nan_inf) {
    printf(" found "); }
    }
    */
  }
  ...
  if (net.adversarial && net.attention) {
    ...
    image attention_img =
        make_attention_image(img_size, original_delta_cpu, original_input_cpu,
                             net.w, net.h, net.c, 0.7);
    ...
    image attention_mask_img =
        make_attention_image(img_size, original_delta_cpu, original_delta_cpu,
                             net.w, net.h, net.c, 1.0);
    ...
    if (net.cudnn_half) {
      if (l.type == CONVOLUTIONAL && l.weights_gpu && l.weights_gpu16) {
        ...
        cuda_convert_f32_to_f16(l.input_layer->weights_gpu,
                                l.input_layer->nweights,
                                l.input_layer->weights_gpu16);
        cuda_convert_f32_to_f16(l.self_layer->weights_gpu,
                                l.self_layer->nweights,
                                l.self_layer->weights_gpu16);
        cuda_convert_f32_to_f16(l.output_layer->weights_gpu,
                                l.output_layer->nweights,
                                l.output_layer->weights_gpu16);
      } else if (l.type == CONV_LSTM && l.wf->weights_gpu &&
                 l.wf->weights_gpu16) {
        assert((l.wf->c * l.wf->n * l.wf->size * l.wf->size) > 0);
        if (l.peephole) {
          cuda_convert_f32_to_f16(l.vf->weights_gpu, l.vf->nweights,
                                  l.vf->weights_gpu16);
          cuda_convert_f32_to_f16(l.vi->weights_gpu, l.vi->nweights,
                                  l.vi->weights_gpu16);
          cuda_convert_f32_to_f16(l.vo->weights_gpu, l.vo->nweights,
                                  l.vo->weights_gpu16);
        }
        cuda_convert_f32_to_f16(l.wf->weights_gpu, l.wf->nweights,
                                l.wf->weights_gpu16);
        if (!l.bottleneck) {
          cuda_convert_f32_to_f16(l.wi->weights_gpu, l.wi->nweights,
                                  l.wi->weights_gpu16);
          cuda_convert_f32_to_f16(l.wg->weights_gpu, l.wg->nweights,
                                  l.wg->weights_gpu16);
          cuda_convert_f32_to_f16(l.wo->weights_gpu, l.wo->nweights,
                                  l.wo->weights_gpu16);
        }
        cuda_convert_f32_to_f16(l.uf->weights_gpu, l.uf->nweights,
                                l.uf->weights_gpu16);
        cuda_convert_f32_to_f16(l.ui->weights_gpu, l.ui->nweights,
                                l.ui->weights_gpu16);
        cuda_convert_f32_to_f16(l.ug->weights_gpu, l.ug->nweights,
                                l.ug->weights_gpu16);
        cuda_convert_f32_to_f16(l.uo->weights_gpu, l.uo->nweights,
                                l.uo->weights_gpu16);
      }
    }
  }
...
float train_network_datum_gpu(network net, float *x, float *y)
{
  *net.seen += net.batch;
  if (net.adversarial_lr && rand_int(0, 1) == 1 &&
      get_current_iteration(net) > net.burn_in) {
    net.adversarial = 1;
    ...
    for (b = 0; b < net.batch; ++b) {
      if (b % 2 == 1 && net.contrastive) {
        ...
        memcpy(x + img_size * b, old_input + img_size * b,
  ...
  // if (((*net.seen) / net.batch) % (net.subdivisions*sequence) == 0)
  // update_network_gpu(net);
...
void merge_updates(layer l, layer base)
{
  if (l.type == CONVOLUTIONAL) {
    ...
    axpy_cpu(l.outputs * l.inputs, 1, l.weight_updates, 1, base.weight_updates,
             1);
  }
}
...
float train_networks(network *nets, int n, data d, int interval)
{
  ...
  *nets[0].seen = nets[0].batch * nets[0].subdivisions *
                  get_current_iteration(
                      nets[0]);  // remove this line, when you will save to
...
float *network_predict_gpu(network net, float *input)
{
  ...
  state.index = 0;
  state.net = net;
  // state.input = cuda_make_array(input, size);   // memory will be allocated
  // in the parse_network_cfg_custom()
...
float *network_predict_gpu_gl_texture(network net, uint32_t texture_id)
{
  ...
  CHECK_CUDA(cudaGraphicsGLRegisterImage(&graphics_resource, texture_id,
                                         GL_TEXTURE_2D, flags));
  CHECK_CUDA(cudaGraphicsMapResources(1, &graphics_resource, 0));

  // void* dev_ptr = NULL;
  cudaArray_t dev_array = NULL;
  CHECK_CUDA(cudaGraphicsSubResourceGetMappedArray(&dev_array,
                                                   graphics_resource, 0, 0));

  size_t width = net.w;
  size_t height = net.h;
  size_t pitch = width * sizeof(float);

  CHECK_CUDA(cudaMemcpy2DFromArray(net.input_state_gpu,      // dst
                                   pitch,                    // dst_pitch
                                   dev_array,                // src
                                   0,                        // width offset
                                   0,                        // height offset
