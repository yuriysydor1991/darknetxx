...
int init(const char *configurationFilename, const char *weightsFilename,
         int gpu, int batch_size)
{
  detector.reset(
      new Detector(configurationFilename, weightsFilename, gpu, batch_size));
  return 1;
}
...
int detect_mat(const uint8_t *data, const size_t data_length,
...
LIB_API Detector::Detector(std::string cfg_filename,
                           std::string weight_filename, int gpu_id,
                           int batch_size)
    : cur_gpu_id(gpu_id)
{
  wait_stream = 0;
  ...
  detector_gpu_t &detector_gpu =
      *static_cast<detector_gpu_t *>(detector_gpu_ptr.get());
  ...
  detector_gpu.track_id =
      (unsigned int *)calloc(l.classes, sizeof(unsigned int));
...
LIB_API Detector::~Detector()
{
  ...
  for (int j = 0; j < NFRAMES; ++j)
    if (detector_gpu.images[j].data) free(detector_gpu.images[j].data);
...
LIB_API std::vector<bbox_t> Detector::detect(std::string image_filename,
                                             float thresh, bool use_mean)
{
...
static image load_image_stb(char *filename, int channels)
{
  ...
  if (!data) throw std::runtime_error("file not found");
...
LIB_API std::vector<bbox_t> Detector::detect(image_t img, float thresh,
                                             bool use_mean)
{
  detector_gpu_t &detector_gpu =
      *static_cast<detector_gpu_t *>(detector_gpu_ptr.get());
  ...
  int old_gpu_index;
  cudaGetDevice(&old_gpu_index);
  if (cur_gpu_id != old_gpu_index) cudaSetDevice(net.gpu_index);
  ...
  if (use_mean) {
    memcpy(detector_gpu.predictions[detector_gpu.demo_index], prediction,
  ...
  // get_region_boxes(l, 1, 1, thresh, detector_gpu.probs, detector_gpu.boxes,
  // 0, 0); if (nms) do_nms_sort(detector_gpu.boxes, detector_gpu.probs,
  // l.w*l.h*l.n, l.classes, nms);
...
#ifdef GPU
  if (cur_gpu_id != old_gpu_index) cudaSetDevice(old_gpu_index);
#endif

  return bbox_vec;
}

LIB_API std::vector<std::vector<bbox_t>> Detector::detectBatch(
    image_t img, int batch_size, int width, int height, float thresh,
    bool make_nms)
{
  detector_gpu_t &detector_gpu =
      *static_cast<detector_gpu_t *>(detector_gpu_ptr.get());
  ...
  det_num_pair *prediction = network_predict_batch(
      &net, in_img, batch_size, width, height, thresh, hier_thresh, 0, 0, 0);
  ...
  for (int bi = 0; bi < batch_size; ++bi) {
    ...
    if (make_nms && nms) do_nms_sort(dets, prediction[bi].num, l.classes, nms);
...
LIB_API std::vector<bbox_t> Detector::tracking_id(
    std::vector<bbox_t> cur_bbox_vec, bool const change_history,
    int const frames_story, int const max_dist)
{
  detector_gpu_t &det_gpu =
      *static_cast<detector_gpu_t *>(detector_gpu_ptr.get());
  ...
  std::vector<unsigned int> dist_vec(cur_bbox_vec.size(),
    ...
    for (auto &i : prev_bbox_vec) {
      ...
      for (size_t m = 0; m < cur_bbox_vec.size(); ++m) {
        ...
        if (i.obj_id == k.obj_id) {
          ...
          unsigned int cur_dist = sqrt(center_x_diff * center_x_diff +
                                       center_y_diff * center_y_diff);
          if (cur_dist < max_dist &&
              (k.track_id == 0 || dist_vec[m] > cur_dist)) {
            dist_vec[m] = cur_dist;
            cur_index = m;
          }
        }
      ...
      bool track_id_absent = !std::any_of(
          cur_bbox_vec.begin(), cur_bbox_vec.end(), [&i](bbox_t const &b) {
...
void *Detector::get_cuda_context()
{
  ...
  int old_gpu_index;
  cudaGetDevice(&old_gpu_index);
  if (cur_gpu_id != old_gpu_index) cudaSetDevice(cur_gpu_id);
