...
static constexpr const float TWO_TWO_FIVE = 255.f;
...
extern "C" mat_cv *load_image_mat_cv(const char *filename, int flag)
{
  ...
  try {
    ...
    if (mat.empty()) {
      ...
      LOGE("Cannot load image " << shrinked_filename);
    ...
    LOGE("OpenCV exception: load_image_mat_cv");
  }
...
cv::Mat load_image_mat(char *filename, int channels)
{
  ...
  if (channels == 0) flag = cv::IMREAD_COLOR;
    ...
    LOGE("OpenCV can't force load with " << channels << " channels");
  }
...
extern "C" image load_image_resize(char *filename, int w, int h, int c,
                                   image *im)
{
  image out;
  try {
    ...
    *im = mat_to_image(loaded_image);
    ...
    LOGE(" OpenCV exception: load_image_resize() can't load image "
         << filename);
...
{
  if (mat == NULL) {
    LOGE(" Pointer is NULL in get_height_mat()");
    return 0;
  }
...
extern "C" void release_mat(mat_cv **mat)
{
  try {
    ...
    *mat_ptr = NULL;
    ...
    LOGE("OpenCV exception: release_mat");
  }
}
    ...
    for (y = 0; y < im.h; ++y) {
        for (x = 0; x < im.w; ++x) {
            for (c = 0; c < im.c; ++c) {
                ...
                disp->imageData[y*step + x*im.c + c] = (unsigned char)(val *
255);
            }
        }
    }
...
extern "C" cv::Mat image_to_mat(image img)
{
  ...
  const size_t channels = static_cast<size_t>(img.c);
  const size_t width = static_cast<size_t>(img.w);
  const size_t height = static_cast<size_t>(img.h);

  cv::Mat mat = cv::Mat(static_cast<int>(height), static_cast<int>(width),
  ...
  for (size_t y = 0; y < height; ++y) {
    for (size_t x = 0; x < width; ++x) {
      for (size_t c = 0; c < channels; ++c) {
        const size_t index = c * height * width + y * width + x;
        const float val = img.data[index];
        const unsigned char castedVal =
            static_cast<unsigned char>(val * TWO_TWO_FIVE);
        const size_t index2 = y * step + x * channels + c;
        mat.data[index2] = castedVal;
      }
    }
  }
...
extern "C" image mat_to_image(cv::Mat mat)
{
  assert(mat.cols > 0);
  assert(mat.rows > 0);
  assert(mat.channels() > 0);

  const size_t w = static_cast<size_t>(mat.cols);
  const size_t h = static_cast<size_t>(mat.rows);
  const size_t c = static_cast<size_t>(mat.channels());

  image im = make_image(mat.cols, mat.rows, mat.channels());

  assert(mat.data != nullptr);

  const unsigned char *const idata = mat.data;
  ...
  for (size_t y = 0; y < h; ++y) {
    for (size_t k = 0; k < c; ++k) {
      for (size_t x = 0; x < w; ++x) {
        ...
        const size_t index = k * w * h + y * w + x;
        const size_t idataIndex = y * step + x * c + k;

        im.data[index] = idata[idataIndex] / TWO_TWO_FIVE;
      }
    }
  }
...
// Window
// ====================================================================
extern "C" void create_window_cv(char const *window_name, int full_screen,
                                 int width, int height)
{
  try {
    ...
    LOGE("OpenCV exception: create_window_cv");
  }
}
// ----------------------------------------
...
extern "C" void destroy_all_windows_cv()
{
  try {
    ...
    LOGE("OpenCV exception: destroy_all_windows_cv");
  }
}
// ----------------------------------------
...
extern "C" int wait_key_cv(int delay)
{
  try {
    ...
    LOGE("OpenCV exception: wait_key_cv");
  }
  return -1;
}
// ----------------------------------------
...
extern "C" void make_window(char *name, int w, int h, int fullscreen)
{
  try {
    ...
    LOGE("OpenCV exception: make_window");
  }
}
// ----------------------------------------
...
extern "C" void show_image_cv(image p, const char *name)
{
  try {
    ...
    LOGE("OpenCV exception: show_image_cv");
  }
}
// ----------------------------------------
...
extern "C" void show_image_mat(mat_cv *mat_ptr, const char *name)
{
  try {
    ...
    LOGE("OpenCV exception: show_image_mat");
  }
}
...
extern "C" write_cv *create_video_writer(char *out_filename, char c1, char c2,
                                         char c3, char c4, int fps, int width,
                                         int height, int is_color)
{
  try {
    ...
    LOGE("OpenCV exception: create_video_writer");
  }
  return NULL;
}
...
extern "C" void write_frame_cv(write_cv *output_video_writer, mat_cv *mat)
{
  try {
    ...
    LOGE("OpenCV exception: write_frame_cv");
  }
}
...
extern "C" void release_video_writer(write_cv **output_video_writer)
{
  try {
    if (output_video_writer) {
      ...
      LOGE("OpenCV exception: output_video_writer isn't created");
    }
  }
  catch (...) {
...
extern "C" cap_cv *get_capture_video_stream(const char *path)
{
  ...
  try {
    ...
    LOGI(" OpenCV exception: video-stream " << path << " can't be opened!");
  }
  return (cap_cv *)cap;
}
// ----------------------------------------
...
extern "C" cap_cv *get_capture_webcam(int index)
{
  ...
  try {
    ...
    LOGI(" OpenCV exception: Web-cam " << index << " can't be opened!");
  }
  return (cap_cv *)cap;
}
// ----------------------------------------
...
extern "C" void release_capture(cap_cv *cap)
{
  try {
    ...
    LOGI(" OpenCV exception: cv::VideoCapture " << cap
                                                << " can't be released!");
  }
}
// ----------------------------------------
  ...
  try {
    ...
    if (cap) {
      ...
      if (cpp_cap.isOpened()) {
        ...
        LOGI(" Video-stream stopped!");
    } else
      LOGE(" cv::VideoCapture isn't created");
  }
  catch (...) {
    LOGE(" OpenCV exception: Video-stream stoped!");
  }
  return (mat_cv *)mat;
}
// ----------------------------------------
...
extern "C" int get_stream_fps_cpp_cv(cap_cv *cap)
{
  ...
  try {
    ...
    LOGE(
        " Can't get FPS of source videofile. For output video FPS = 25 by "
        "default.");
  }
  return fps;
}
...
extern "C" double get_capture_property_cv(cap_cv *cap, int property_id)
{
  try {
    ...
    LOGE(" OpenCV exception: Can't get property of source video-stream.");
  }
  return 0;
}
// ----------------------------------------
...
extern "C" double get_capture_frame_count_cv(cap_cv *cap)
{
  try {
    ...
    LOGE(
        " OpenCV exception: Can't get CAP_PROP_FRAME_COUNT of source "
        "videofile.");
  }
  return 0;
}
...
extern "C" int set_capture_property_cv(cap_cv *cap, int property_id,
                                       double value)
{
  try {
    cv::VideoCapture &cpp_cap = *(cv::VideoCapture *)cap;
    ...
    LOGE(" Can't set property of source video-stream.");
  }
  return false;
}
// ----------------------------------------
...
extern "C" int set_capture_position_frame_cv(cap_cv *cap, int index)
{
  try {
    ...
    LOGE(" Can't set CAP_PROP_POS_FRAMES of source videofile.");
  }
  return false;
}
// ----------------------------------------
...
extern "C" image get_image_from_stream_cpp(cap_cv *cap)
{
  ...
  if (once) {
    ...
    LOGI("Video stream: " << src->cols << " x " << src->rows);
  } else
    src = (cv::Mat *)get_capture_frame_cv(cap);
...
extern "C" image get_image_from_stream_resize(cap_cv *cap, int w, int h, int c,
...
extern "C" image get_image_from_stream_letterbox(cap_cv *cap, int w, int h,
                                                 int c, mat_cv **in_img,
                                                 int dont_close)
{
  c = c ? c : 3;
...
extern "C" void draw_detections_cv_v3(mat_cv *mat, detection *dets, int num,
                                      ...
                                      int ext_output)
{
  static constexpr const double ONEHUNDRED = 100;
  ...
  try {
    ...
    for (i = 0; i < num; ++i) {
      char labelstr[PATH_MAX] = {0};
      int class_id = -1;
      for (j = 0; j < classes; ++j) {
        const int show = strncmp(names[j], "dont_show", 9);
        const double cdets = dets[i].prob[j];
        ...
        if (class_id < 0) {
          ...
          sprintf(buff, " (%2.0f%%)", cdets * ONEHUNDRED);
          strcat(labelstr, buff);
          LOGI(names[j] << ":" << (cdets * ONEHUNDRED));
          if (dets[i].track_id)
            LOGI(" (track = " << dets[i].track_id << ", sim = " << dets[i].sim
                              << ") ");
        } else {
          strcat(labelstr, ", ");
          strcat(labelstr, names[j]);
      ...
      if (class_id >= 0) {
        ...
        // sprintf(labelstr, "%d x %d - w: %d, h: %d", b_x_center, b_y_center,
        // b_width, b_height);

        float const font_size = show_img->rows / 1000.F;
        cv::Size const text_size = cv::getTextSize(
            labelstr, cv::FONT_HERSHEY_COMPLEX_SMALL, font_size, 1, 0);
        ...
        //    if(copy_img == NULL) copy_img =
        //    cvCreateImage(cvSize(show_img->width, show_img->height),
        //    show_img->depth, show_img->nChannels); cvCopy(show_img, copy_img,
        //    0);
        //}
        // static int img_id = 0;
        // img_id++;
        // char image_name[1024];
        // sprintf(image_name, "result_img/img_%d_%d_%d_%s.jpg", frame_id,
        // img_id, class_id, names[class_id]); CvRect rect = cvRect(pt1.x,
        // pt1.y, pt2.x - pt1.x, pt2.y - pt1.y); cvSetImageROI(copy_img, rect);
        ...
        if (ext_output)
          FMTLOGI(
              "\t(left_x: %4.0f   top_y: %4.0f   width: %4.0f   height: "
              "%4.0f)",
              (float)left, (float)top, b.w * show_img->cols,
              b.h * show_img->rows);

        cv::rectangle(*show_img, pt_text_bg1, pt_text_bg2, color, width, 8, 0);
        cv::rectangle(*show_img, pt_text_bg1, pt_text_bg2, color, CV_FILLED, 8,
                      0);  // filled
        cv::Scalar black_color = CV_RGB(0, 0, 0);
        cv::putText(*show_img, labelstr, pt_text,
                    cv::FONT_HERSHEY_COMPLEX_SMALL, font_size, black_color,
                    2 * font_size, CV_AA);
    ...
    LOGE("OpenCV exception: draw_detections_cv_v3()");
  }
}
// ----------------------------------------
...
extern "C" mat_cv *draw_train_chart(char *windows_name, float max_img_loss,
                                    int max_batches, int number_of_lines,
                                    int img_size, int dont_show,
                                    char *chart_path)
{
  int img_offset = 60;
  ...
  try {
    ...
    if (chart_path != NULL && chart_path[0] != '\0') {
      *img_ptr = cv::imread(chart_path);
      ...
      char max_batches_buff[PATH_MAX];
    ...
    if (!dont_show) {
      LOGI(" If error occurs - run training with flag: -dont_show");
...
extern "C" void draw_train_loss(char *windows_name, mat_cv *img_src,
                                int img_size, float avg_loss,
                                float max_img_loss, int current_batch,
                                int max_batches, float precision,
                                int draw_precision, char *accuracy_name,
                                float contr_acc, int dont_show, int mjpeg_port,
                                double time_remaining)
{
  draw_train_loss_ex(windows_name, img_src, img_size, avg_loss, max_img_loss,
                     current_batch, max_batches, precision, draw_precision,
                     accuracy_name, contr_acc, dont_show, mjpeg_port,
                     time_remaining, 0);
}

extern "C" void draw_train_loss_ex(
    char *windows_name, mat_cv *img_src, int img_size, float avg_loss,
    float max_img_loss, int current_batch, int max_batches, float precision,
    int draw_precision, char *accuracy_name, float contr_acc, int dont_show,
    int mjpeg_port, double time_remaining, int save_every_chart)
{
  try {
    cv::Mat &img = *(cv::Mat *)img_src;
    int img_offset = 60;
    ...
    if (contr_acc >= 0) {
      ...
      if (current_batch > 0) {
        cv::line(img,
                 cv::Point(img_offset + draw_size * (float)(current_batch - 1) /
                                            max_batches,
                           draw_size * (1 - old_contr_acc)),
                 cv::Point(img_offset +
                               draw_size * (float)current_batch / max_batches,
      ...
      cv::putText(img, char_buff, cv::Point(1, 45),
                  cv::FONT_HERSHEY_COMPLEX_SMALL, 0.7, CV_RGB(255, 255, 255), 5,
                  CV_AA);
      cv::putText(img, char_buff, cv::Point(1, 45),
                  cv::FONT_HERSHEY_COMPLEX_SMALL, 0.7, CV_RGB(0, 150, 70), 1,
                  CV_AA);
    }
    ...
    if (draw_precision) {
      ...
      if (iteration_old == 0)
        cv::putText(img, accuracy_name, cv::Point(10, 12),
                    cv::FONT_HERSHEY_COMPLEX_SMALL, 0.7, CV_RGB(255, 0, 0), 1,
                    CV_AA);

      if (iteration_old != 0) {
        cv::line(img,
                 cv::Point(img_offset +
                               draw_size * (float)iteration_old / max_batches,
      ...
      cv::putText(img, char_buff, cv::Point(10, 28),
                  ...
                  CV_AA);
      cv::putText(img, char_buff, cv::Point(10, 28),
                  cv::FONT_HERSHEY_COMPLEX_SMALL, 0.7, CV_RGB(200, 0, 0), 1,
                  CV_AA);
      ...
      if ((std::fabs(old_precision - precision) > 0.1) ||
        ...
        cv::putText(img, char_buff,
                    ...
                    cv::FONT_HERSHEY_COMPLEX_SMALL, 0.7, CV_RGB(255, 255, 255),
                    5, CV_AA);
        cv::putText(img, char_buff,
                    cv::Point(pt1.x - 30, draw_size * (1 - precision) + 15),
                    cv::FONT_HERSHEY_COMPLEX_SMALL, 0.7, CV_RGB(200, 0, 0), 1,
    ...
    if (k == 's' || current_batch == (max_batches - 1) ||
        (current_batch / 100 > old_batch / 100) || save_every_chart) {
      ...
      cv::putText(img, "- Saved", cv::Point(260, img_size - 10),
                  cv::FONT_HERSHEY_COMPLEX_SMALL, 0.7, CV_RGB(255, 255, 255), 1,
                  CV_AA);
    ...
    LOGE("OpenCV exception: draw_train_loss()");
  }
}
// ----------------------------------------
...
extern "C" image image_data_augmentation(mat_cv *mat, int w, int h, int pleft,
                                         int ptop, int swidth, int sheight,
                                         int flip, float dhue, float dsat,
                                         float dexp, int gaussian_noise,
                                         int blur, int num_boxes,
                                         int truth_size, float *truth)
{
  ...
  try {
    ...
    LOGE("OpenCV can't augment image: " << w << " x " << h << " : "
                                        << e.what());
...
extern "C" void blend_images_cv(image new_img, float alpha, image old_img,
                                float beta)
{
  cv::Mat new_mat(cv::Size(new_img.w, new_img.h), CV_32FC(new_img.c),
                  new_img.data);  // , size_t step = AUTO_STEP)
  cv::Mat old_mat(cv::Size(old_img.w, old_img.h), CV_32FC(old_img.c),
                  old_img.data);
...
extern "C" void cv_draw_object(image sized, float *truth_cpu, int max_boxes,
                               int num_truth, int *it_num_set, float *lr_set,
                               int *boxonly, int classes, char **names)
{
  ...
  cv::createTrackbar(cl_trackbar_name, window_name, &cl_trackbar_value,
                     classes - 1);
  ...
  while (!selected) {
    ...
    cv::putText(frame_clone, lr_value, cv::Point2i(10, 20),
                cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(10, 50, 10), 3);
    cv::putText(frame_clone, lr_value, cv::Point2i(10, 20),
                cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(20, 120, 60), 2);
    cv::putText(frame_clone, lr_value, cv::Point2i(10, 20),
                cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(50, 200, 100), 1);

    if (names) {
      std::string obj_name = names[cl_trackbar_value];
      cv::putText(frame_clone, obj_name, cv::Point2i(10, 40),
                  cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(10, 50, 10), 3);
      cv::putText(frame_clone, obj_name, cv::Point2i(10, 40),
                  cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(20, 120, 60), 2);
      cv::putText(frame_clone, obj_name, cv::Point2i(10, 40),
  ...
  if (selected) {
    ...
    FMTLOGI(" x_start = %d, y_start = %d, x_size = %d, y_size = %d",
...
extern "C" void show_acnhors(int number_of_boxes, int num_of_clusters,
                             float *rel_width_height_array, model anchors_data,
                             int width, int height)
{
...
{
  LOGI(" OpenCV version: " << CV_VERSION_MAJOR << "." << CV_VERSION_MINOR << "."
...
{
  LOGW(" OpenCV isn't used - data augmentation will be slow");
}
