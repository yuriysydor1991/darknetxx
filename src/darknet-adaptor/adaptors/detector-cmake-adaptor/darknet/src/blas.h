...
void grad_contrastive_loss_positive(size_t i, int *labels,
                                    ...
                                    float *p_constrastive, float *delta,
                                    int wh);
...
void constrain_weight_updates_ongpu(int N, float coef, float *weights_gpu,
...
void fast_v_cbn_gpu(const float *x, float *mean, int batch, int filters,
                    int spatial, int minibatch_index, int max_minibatch_index,
                    float *m_avg, float *v_avg, float *variance,
                    const float alpha, float *rolling_mean_gpu,
                    float *rolling_variance_gpu, int inverse_variance,
                    float epsilon);
void inverse_variance_ongpu(int size, float *src, float *dst, float epsilon);
void normalize_scale_bias_gpu(float *x, float *mean, float *variance,
                              float *scales, float *biases, int batch,
                              int filters, int spatial, int inverse_variance,
                              float epsilon);
...
void shortcut_multilayer_gpu(int src_outputs, int batch, int n,
                             ...
                             float *weights_gpu, int nweights,
                             WEIGHTS_NORMALIZATION_T weights_normalization);
void backward_shortcut_multilayer_gpu(
...
void softmax_gpu_new_api(float *input, int n, int batch, int batch_offset,
...
void softmax_tree_gpu(float *input, int spatial, int batch, int stride,
...
void add_3_arrays_activate(float *a1, float *a2, float *a3, size_t size,
                           ACTIVATION a, float *dst);
void sum_of_mults(float *a1, float *a2, float *b1, float *b2, size_t size,
                  float *dst);
void activate_and_mult(float *a1, float *a2, size_t size, ACTIVATION a,
                       float *dst);

void scale_channels_gpu(float *in_w_h_c, int size, int channel_size,
                        int batch_size, int scale_wh, float *scales_c,
                        float *out);
void backward_scale_channels_gpu(float *in_w_h_c_delta, int size,
                                 int channel_size, int batch_size, int scale_wh,
...
void reduce_and_expand_array_gpu(const float *src_gpu, float *dst_gpu, int size,
                                 int groups);
void expand_array_gpu(const float *src_gpu, float *dst_gpu, int size,
                      int groups);
void mult_inverse_array_gpu(const float *src_gpu, float *dst_gpu, int size,
                            float eps, float divider, float clip,
                            float abs_add);
void P_constrastive_f_det_gpu(int *labels, unsigned int feature_size,
                              float temperature, contrastive_params *contrast_p,
                              const int contrast_p_size);
void coord_conv_gpu(float *dst, int size, int w, int h, int chan, int b,
                    int type);

void forward_implicit_gpu(int batch, int nweights, float *weight_gpu,
                          float *output_gpu);
void backward_implicit_gpu(int batch, int nweights, float *weight_updates_gpu,
                           float *delta_gpu);
