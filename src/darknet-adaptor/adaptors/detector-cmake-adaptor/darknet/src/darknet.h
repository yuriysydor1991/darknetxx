...
#include "src/structs/contrastive_params.h"
...
LIB_API network *load_network_custom(char *cfg, char *weights, int clear,
                                     int batch);
...
LIB_API det_num_pair *network_predict_batch(network *net, image im,
                                            int batch_size, int w, int h,
                                            float thresh, float hier, int *map,
...
LIB_API float validate_detector_map(char *datacfg, char *cfgfile,
                                    char *weightfile, float thresh_calc_avg_iou,
...
LIB_API void test_detector(int argc, char **argv);
LIB_API int network_width(network *net);
LIB_API int network_height(network *net);
LIB_API void optimize_picture(network *net, image orig, int max_layer,
                              float scale, float rate, float thresh, int norm);

// image.h
LIB_API void make_image_red(image im);
LIB_API image make_attention_image(int img_size, float *original_delta_cpu,
                                   float *original_input_cpu, int w, int h,
                                   int c, float alpha);
...
LIB_API void set_track_id(detection *new_dets, int new_dets_num, float thresh,
                          float sim_thresh, float track_ciou_norm,
                          int deque_size, int dets_for_track,
                          int dets_for_show);
LIB_API int fill_remaining_id(detection *new_dets, int new_dets_num,
                              int new_track_id, float thresh);
