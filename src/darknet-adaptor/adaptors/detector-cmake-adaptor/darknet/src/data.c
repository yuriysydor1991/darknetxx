...
list *get_paths(char *filename)
{
  ...
  if (!file) file_error(__FILE__, __LINE__, filename);
...
char **get_sequential_paths(char **paths, int n, int m, int mini_batch,
                            int augment_speed, int contrastive)
{
  ...
  for (i = 0; i < n; ++i) {
    do {
      ...
      if (strlen(sequentia_paths[i]) <= 4)
        CLOGI("Very small path to the image: %s", sequentia_paths[i]);
...
char **get_random_paths_custom(char **paths, int n, int m, int contrastive)
{
  ...
  CLOGT("N for rand gen: %d", n);

  static const size_t bsize = PATH_MAX * 10;
  ...
  for (i = 0; i < n; ++i) {
    do {
      ...
      snprintf(buff + strlen(buff), bsize, "%d ", index);

      random_paths[i] = paths[index];

      CLOGT("Randomly selected image: %s", random_paths[i]);

      // if(i == 0) printf("%s\n", paths[index]);
      // printf("grp: %s\n", paths[index]);
      if (strlen(random_paths[i]) <= 4)
        CLOGI(" Very small path to the image: %s \n", random_paths[i]);
    } while (strlen(random_paths[i]) == 0);
  }

  pthread_mutex_unlock(&mutex);

  CLOGT("Random images indexes from train.txt: %s", buff);
...
int fill_truth_detection(const char *path, int num_boxes, int truth_size,
                         float *truth, int classes, int flip, float dx,
                         float dy, float sx, float sy, int net_w, int net_h)
{
  ...
  for (i = 0; i < count; ++i) {
    ...
    if (x == 999999 || y == 999999) {
      CLOGW("Wrong annotation: x = 0, y = 0, < 0 or > 1, file: %s", labelpath);
      sprintf(buff,
    ...
    if (x <= 0 || x > 1 || y <= 0 || y > 1) {
      CLOGW("Wrong annotation: x = %f, y = %f, file: %s", x, y, labelpath);
      sprintf(buff,
    ...
    if (w > 1) {
      CLOGW("Wrong annotation: w = %f, file: %s", w, labelpath);
    ...
    if (h > 1) {
      CLOGW("Wrong annotation: h = %f, file: %s", h, labelpath);
    ...
    // sub, truth_size, track_id, truth[(i - sub)*truth_size + 5], val);
...
void fill_truth_captcha(char *path, int n, float *truth)
{
  ...
  for (i = 0; i < strlen(begin) && i < n && begin[i] != '.'; ++i) {
    ...
    if (index > 35) CLOGE("Bad %c", begin[i]);
...
void fill_truth(char *path, char **labels, int k, float *truth)
{
  ...
  if (count != 1) {
    CLOGE("Too many or too few labels: %d, %s", count, path);
    count = 0;
    for (i = 0; i < k; ++i) {
      if (strstr(path, labels[i])) {
        CLOGE("\tlabel %d: %s", count, labels[i]);
        count++;
      }
    }
  }
...
void fill_truth_smooth(char *path, char **labels, int k, float *truth,
                         ...
                         int contrastive)
{
  ...
  if (labels) {
    ...
    for (i = 0; i < n; ++i) {
      const int img_index = (contrastive) ? (i / 2) : i;
      fill_truth_smooth(paths[img_index], labels, k, y.vals[i],
                        label_smooth_eps);
      // printf(" n = %d, i = %d, img_index = %d, class_id = %d \n", n, i,
      // img_index, find_max(y.vals[i], k));
      if (hierarchy) {
    ...
    for (i = 0; i < n; ++i) {
      const int img_index = (contrastive) ? (i / 2) : i;
      const uintptr_t path_p =
          (uintptr_t)paths[img_index];  // abs(random_gen());
...
void blend_truth_mosaic(float *new_truth, int boxes, int truth_size,
                        float *old_truth, int w, int h, float cut_x,
                        float cut_y, int i_mixup, int left_shift,
...
data load_data_detection(int n, char **paths, int m, int w, int h, int c,
                         ...
                         int use_gaussian_noise, int use_blur, int use_mixup,
                         float jitter, float resize, float hue,
                         float saturation, float exposure, int mini_batch,
                         int track, int augment_speed, int letter_box,
                         int mosaic_bound, int contrastive,
                         int contrastive_jit_flip, int contrastive_color,
                         int show_imgs)
{
  ...
  if (use_mixup == 2 || use_mixup == 4) {
    CLOGWM(
        "cutmix=1 - isn't supported for Detector (use cutmix=1 only for "
        "Classifier)");
  ...
  if (use_mixup == 3 && letter_box) {
    error(
        "Combination: letter_box=1 & mosaic=1 - isn't supported, use only 1 of "
        "these parameters",
        DARKNET_LOC);
  }
  ...
  for (i_mixup = 0; i_mixup <= use_mixup; i_mixup++) {
    ...
    char **random_paths;
    if (track)
      random_paths = get_sequential_paths(paths, n, m, mini_batch,
                                          augment_speed, contrastive);
    else
    ...
    for (i = 0; i < n; ++i) {
      ...
      if (src == NULL) {
        CLOGEM("Error in load_data_detection() - OpenCV");
        continue;
      }
      ...
      if (swidth <= 0 || sheight <= 0 || (ow - pleft) <= 0 ||
          (oh - ptop) <= 0) {
        CLOGW(
            "WARNING: invalid resize. Resetting swidth: %d , sheight:  %d, "
            "pleft: %d, ptop: %d",
            dw, dh, 0, 0);
        CLOGW(
            "Original values: \n swidth = %d, sheight = %d, pleft = %d, "
            "pright = %d, ptop = %d, pbot = %d, ow = %d, oh = %d",
      ...
      int min_w_h =
          fill_truth_detection(filename, boxes, truth_size, truth, classes,
                               flip, dx, dy, 1. / sx, 1. / sy, w, h);
      // for (int z = 0; z < boxes; ++z) if(truth[z*truth_size] > 0) printf("
      ...
      image ai = image_data_augmentation(
          src, w, h, pleft, ptop, swidth, sheight, flip, dhue, dsat, dexp,
      ...
      if (use_mixup == 0) {
        ...
        blend_truth_mosaic(d.y.vals[i], boxes, truth_size, truth, w, h,
                           cut_x[i], cut_y[i], i_mixup, left_shift, right_shift,
                           top_shift, bot_shift, w, h, mosaic_bound);
...
data load_data_detection(int n, char **paths, int m, int w, int h, int c,
                         int boxes, int truth_size, int classes, int use_flip,
                         int gaussian_noise, int use_blur, int use_mixup,
                         float jitter, float resize, float hue,
                         float saturation, float exposure, int mini_batch,
                         int track, int augment_speed, int letter_box,
                         int mosaic_bound, int contrastive,
                         ...
                         int show_imgs)
{
  ...
  if (use_mixup == 3 || use_mixup == 4) {
    error("mosaic=1 - compile Darknet with OpenCV for using mosaic=1",
          DARKNET_LOC);
  }
  ...
  if (mixup) {
    if (track)
      mixup_random_paths = get_sequential_paths(paths, n, m, mini_batch,
                                                augment_speed, contrastive);
    else
  ...
  for (i_mixup = 0; i_mixup <= mixup; i_mixup++) {
    ...
    for (i = 0; i < n; ++i) {
      ...
      fill_truth_detection(filename, boxes, truth_size, truth, classes, flip,
                           dx, dy, 1. / sx, 1. / sy, w, h);
      ...
      if (show_imgs)  // && i_mixup)
      {
        ...
        CLOGI("You use flag -show_imgs, so will be saved aug_...jpg images");
      }
...
void *load_thread(void *ptr)
{
  ...
  if (a.type == OLD_CLASSIFICATION_DATA) {
    ...
    *a.d = load_data_augment(a.paths, a.n, a.m, a.labels, a.classes,
                             a.hierarchy, a.flip, a.min, a.max, a.w, a.h,
                             a.angle, a.aspect, a.hue, a.saturation, a.exposure,
                             a.mixup, a.blur, a.show_imgs, a.label_smooth_eps,
                             a.dontuse_opencv, a.contrastive);
    ...
    *a.d = load_data_region(a.n, a.paths, a.m, a.w, a.h, a.num_boxes, a.classes,
    ...
    free_data(*a.d);
    *a.d = load_data_detection(
        a.n, a.paths, a.m, a.w, a.h, a.c, a.num_boxes, a.truth_size, a.classes,
        a.flip, a.gaussian_noise, a.blur, a.mixup, a.jitter, a.resize, a.hue,
    ...
    *(a.resized) = letterbox_image(*(a.im), a.w, a.h);
    ...
    free_data(*a.d);
    *a.d =
        load_data_tag(a.paths, a.n, a.m, a.classes, a.flip, a.min, a.max, a.w,
                      a.h, a.angle, a.aspect, a.hue, a.saturation, a.exposure);
  }
  free(ptr);
  return 0;
}
...
static volatile atomic_int flag_exit;
static volatile atomic_int *run_load_data = NULL;
static load_args *args_swap = NULL;
static pthread_t *threads = NULL;

pthread_mutex_t mtx_load_data = PTHREAD_MUTEX_INITIALIZER;
pthread_cond_t mtx_load_data_cond = PTHREAD_COND_INITIALIZER;
...
void *run_thread_loop(void *ptr)
{
  ...
  while (!atomic_load(&flag_exit)) {
    pthread_mutex_lock(&mtx_load_data);

    while (!atomic_load(&run_load_data[i])) {
      if (atomic_load(&flag_exit)) {
        pthread_mutex_unlock(&mtx_load_data);
        return 0;
      }
      pthread_cond_wait(&mtx_load_data_cond, &mtx_load_data);
    }
    ...
    *args_local = args_swap[i];
    ...
    pthread_mutex_lock(&mtx_load_data);
    atomic_store(&run_load_data[i], 0);
    pthread_cond_broadcast(&mtx_load_data_cond);
    pthread_mutex_unlock(&mtx_load_data);
  }
...
void *load_threads(void *ptr)
{
  ...
  if (!threads) {
    ...
    run_load_data =
        (volatile atomic_int *)xcalloc(args.threads, sizeof(atomic_int));
    args_swap = (load_args *)xcalloc(args.threads, sizeof(load_args));
    CLOGI("Create %d permanent data load cpu-threads", args.threads);
  ...
  for (i = 0; i < args.threads; ++i) {
    ...
    args_swap[i] = args;
    atomic_store(&run_load_data[i], 1);  // run thread
  }
  pthread_cond_broadcast(&mtx_load_data_cond);
  pthread_mutex_unlock(&mtx_load_data);
  ...
  while (cc) {
    for (i = 0; i < args.threads; ++i) {
      if (!atomic_load(&run_load_data[i])) {
        cc++;
      }
    }
...
void free_load_threads(void *ptr)
{
  ...
  atomic_store(&flag_exit, 0);
}

pthread_t load_data(load_args args)
{
  pthread_t thread;
  struct load_args *ptr = (load_args *)xcalloc(1, sizeof(struct load_args) + 1);
  memset(ptr, 0, sizeof(struct load_args) + 1);
  *ptr = args;
...
/*
   ...
   d.shallow = 0;
   d.X = load_image_augment_paths(paths, n, flip, min, max, size, angle, aspect,
   hue, saturation, exposure); d.y = load_labels_paths(paths, n, labels, k);
   if(m) free(paths);
   return d;
   }
 */
...
data load_data_augment(char **paths, int n, int m, char **labels, int k,
                       tree *hierarchy, int use_flip, int min, int max, int w,
                       int h, float angle, float aspect, float hue,
                       float saturation, float exposure, int use_mixup,
                       int use_blur, int show_imgs, float label_smooth_eps,
                       int dontuse_opencv, int contrastive)
{
  ...
  d.X = load_image_augment_paths(paths, n, use_flip, min, max, w, h, angle,
                                 aspect, hue, saturation, exposure,
  ...
  if (use_mixup && rand_int(0, 1)) {
    ...
    d2.X = load_image_augment_paths(paths_mix, n, use_flip, min, max, w, h,
                                    angle, aspect, hue, saturation, exposure,
                                    dontuse_opencv, contrastive);
    d2.y = load_labels_paths(paths_mix, n, labels, k, hierarchy,
                             label_smooth_eps, contrastive);
    free(paths_mix);
  ...
  if (show_imgs) {
    ...
    for (i = 0; i < d.X.rows; ++i) {
      ...
      char buff_string[PATH_MAX];
      sprintf(buff_string, "\n Classes: ");
      for (j = 0; j < d.y.cols; ++j) {
        if (d.y.vals[i][j] > 0) {
          char buff_tmp[PATH_MAX];
...
data load_data_tag(char **paths, int n, int m, int k, int use_flip, int min,
                   ...
                   float saturation, float exposure)
{
  ...
  d.shallow = 0;
  d.X = load_image_augment_paths(paths, n, use_flip, min, max, w, h, angle,
                                 aspect, hue, saturation, exposure, 0, 0);
...
data load_all_cifar10()
{
  ...
  for (b = 0; b < 5; ++b) {
    ...
    if (!fp) file_error(__FILE__, __LINE__, buff);
    for (i = 0; i < 10000; ++i) {
      unsigned char bytes[3073];
      fread(bytes, 1, 3073, fp);
      int class_id = bytes[0];
