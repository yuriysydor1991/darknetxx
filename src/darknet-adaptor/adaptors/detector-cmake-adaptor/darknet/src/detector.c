                                   ...
                                   char *outfile, char *prefix)
{
  ...
  FILE *fpd = 0;
  char buffd[BUFF_SIZE];
  snprintf(buffd, BUFF_SIZE, "%s/%s/data/%s.txt", prefix, outfile, id);
                       ...
                       char *outfile)
{
  ...
  CLOGI("Learning Rate: %g, Momentum: %g, Decay: %g", net.learning_rate,
  ...
  int k;
  // replace with traverse_detection_layers_ctx"
  for (k = 0; k < net.n; ++k) {
    layer lk = net.layers[k];
    if (lk.type == YOLO || lk.type == GAUSSIAN_YOLO || lk.type == REGION) {
      l = lk;
      CLOGI(" Detection layer: %d - type = %d", k, l.type);
    }
  }
  ...
  if (0 == strcmp(type, "coco")) {
    ...
    snprintf(buff, BUFF_SIZE, "%s/%s.json", prefix, outfile);
    fp = fopen(buff, "w");
    fprintf(fp, "[\n");
    coco = 1;
    ...
    snprintf(buff, BUFF_SIZE, "%s/%s", prefix, outfile);
    // int mkd = make_directory(buff, 0777);
    if (make_directory(buff, 0777) != 0) {
      CLOGE("Failure while creating directory: %s", buff);
    }

    snprintf(buff2, BUFF_SIZE, "%s/%s/data", prefix, outfile);
    // int mkd2 = make_directory(buff2, 0777);
    if (make_directory(buff2, 0777) != 0) {
      CLOGE("Failure while creating directory: %s", buff);
    }
    ...
    snprintf(buff, BUFF_SIZE, "%s/%s.txt", prefix, outfile);
    fp = fopen(buff, "w");
    imagenet = 1;
    classes = 200;
  } else {
    ...
    for (j = 0; j < classes; ++j) {
      snprintf(buff, BUFF_SIZE, "%s/%s%s.txt", prefix, outfile, names[j]);
      fps[j] = fopen(buff, "w");
    }
  }
  ...
  CLOGI("Total Detection Time: %f Seconds", (double)time(0) - start);
}
...
void validate_detector_recall(char *datacfg, char *cfgfile, char *weightfile)
{
  ...
  for (i = 0; i < m; ++i) {
    ...
    CLOGI("%5d %5d %5d\tRPs/Img: %.2f\tIOU: %.2f%%\tRecall:%.2f%%", i, correct,
          total, (float)proposals / (i + 1), avg_iou * 100 / total,
          100. * correct / total);
    free(truth);
    free(id);
    free_image(orig);
                            ...
                            network *existing_net)
{
  ...
  srand(time(0));
  CLOGIM("calculation mAP (mean average precision)...");
  ...
  layer l = net.layers[net.n - 1];
  int k;
  /// @todo: replace with traverse_detection_layers
  for (k = 0; k < net.n; ++k) {
    layer lk = net.layers[k];
    if (lk.type == YOLO || lk.type == GAUSSIAN_YOLO || lk.type == REGION) {
      l = lk;
      CLOGI(" Detection layer: %d - type = %d \n", k, l.type);
    }
  }
  int classes = l.classes;
  ...
  CLOGI(" detections_count = %d, unique_truth_count = %d", detections_count,
        unique_truth_count);
  ...
  for (rank = 0; rank < detections_count; ++rank) {
    if (rank % 100 == 0)
      CLOGI(" rank = %d of ranks = %d", rank, detections_count);
    ...
    for (i = 0; i < classes; ++i) {
      ...
      if (rank == (detections_count - 1) &&
        ...
        CLOGI(" class_id: %d - detections = %d, tp+fp = %d, tp = %d, fp = %d",
  ...
  for (i = 0; i < classes; ++i) {
    ...
    CLOGI("class_id = %d, name = %s, ap = %2.2f%%   \t (TP = %d, FP = %d)", i,
          names[i], avg_precision * 100, tp_for_thresh_per_class[i],
  ...
  if (map_points) printf("used %d Recall-points \n", map_points);
    ...
    CLOGIM("used Area-Under-Curve for each unique Recall");

  CLOGI(" mean average precision (mAP@%0.2f) = %f, or %2.2f %%", iou_thresh,
  ...
  CLOGI("Total Detection Time: %d Seconds", (int)(time(0) - start));
  CLOGIM("Set -points flag:");
  CLOGIM(" `-points 101` for MS COCO");
  CLOGIM(
                  ...
                  int show)
{
  ...
  CLOGI("num_of_clusters = %d, width = %d, height = %d", num_of_clusters, width,
        height);
  if (width < 0 || height < 0) {
    CLOGEM(
        "Usage: darknet detector calc_anchors data/voc.data -num_of_clusters 9 "
        "-width 416 -height 416");
    CLOGEM("Error: set width and height");
    return;
  }
  ...
  CLOGI(" read labels from %d images", number_of_images);
  ...
  if (fwc) {
    ...
    CLOGEM("Error: file counters_per_class.txt can't be open");
  }
  ...
  if (fw) {
    CLOGIM("Saving anchors to the file: anchors.txt");
    CLOGIM("anchors = ");
    ...
    CLOGEM(" Error: file anchors.txt can't be open");
  }
