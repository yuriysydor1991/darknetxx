                  ...
                  int stride, int pad, float *data_col)
{
  ...
  im2col_gpu_kernel<<<(num_kernels + BLOCK - 1) / BLOCK, BLOCK, 0,
                      get_cuda_stream()>>>(num_kernels, im, height, width,
    ...
    float *data_col, const int bit_align)
{
    ...
    for (; index < n; index += blockDim.x*gridDim.x) {
        ...
        int w_in = w_out * stride - pad;
        float* data_col_ptr = data_col;
        //data_col_ptr += (channel_out * height_col + h_out) * width_col +
w_out; data_col_ptr += channel_out * bit_align + h_out * width_col + w_out;
        float* data_col_ptr_32 = data_col + (channel_out * bit_align + h_out *
width_col + w_out)/32; const float* data_im_ptr = data_im; data_im_ptr +=
(channel_in * height + h_in) * width + w_in; for (int i = 0; i < ksize; ++i) {
            for (int j = 0; j < ksize; ++j) {
                int h = h_in + i;
                int w = w_in + j;
                ...
                *data_col_ptr = val;
                ...
                //(*data_col_ptr) = (h >= 0 && w >= 0 && h < height && w <
width) ?
                //    data_im_ptr[i * width + j] : 0;

                //float src_val = (h >= 0 && w >= 0 && h < height && w < width)
? data_im_ptr[i * width + j] : 0;
                //unsigned int bit_mask = __ballot_sync(0xffffffff, src_val >
0);
                //if (threadIdx.x % WARP_SIZE == 0) *((unsigned
int*)data_col_ptr_32) = bit_mask;
                                        ...
                                        const int bit_align)
{
  ...
  for (; index < n; index += blockDim.x * gridDim.x) {
    ...
    // float* data_col_ptr = data_col;
    // float* data_col_ptr_32 = data_col + (channel_out * bit_align + h_out *
    // width_col + w_out) / 32; data_col_ptr += (channel_out * height_col +
    // h_out) * width_col + w_out; data_col_ptr += channel_out * bit_align +
    // h_out * width_col + w_out;
    float *data_col_ptr =
        &data_col[channel_out * bit_align + h_out * width_col + w_out];
    const float *data_im_ptr = data_im;
    data_im_ptr += (channel_in * height + h_in) * width + w_in;
    for (int i = 0; i < ksize; ++i) {
      for (int j = 0; j < ksize; ++j) {
        ...
        int out_index = (channel_out + i * ksize + j) * bit_align +
                        pre_out_index;  // h_out * width_col + w_out;
        ...
        // float src_val = (h >= 0 && w >= 0 && h < height && w < width) ?
        // data_im_ptr[i * width + j] : 0; unsigned int bit_mask =
        // __ballot_sync(0xffffffff, src_val > 0); if (threadIdx.x % WARP_SIZE
        // == 0) *((unsigned int*)data_col_ptr_32) = bit_mask;
        //  use atomicOr() // *dst_ptr |= (mask << (col_index % 8));
        // data_col_ptr_32 += bit_align / 32;
...
void im2col_align_ongpu(float *im, int channels, int height, int width,
                        ...
                        int bit_align)
{
  ...
  int width_col = (width + 2 * pad - ksize) / stride + 1;
  int num_kernels = channels * height_col * width_col;
  im2col_align_gpu_kernel<<<(num_kernels + BLOCK - 1) / BLOCK, BLOCK, 0,
                            get_cuda_stream()>>>(
...
__global__ void im2col_align_bin_gpu_kernel(
    const int n, const float *data_im, const int height, const int width,
    const int ksize, const int channels, const int pad, const int stride,
...
void im2col_align_bin_ongpu(float *im, int channels, int height, int width,
                            int ksize, int stride, int pad, float *data_col,
                            int bit_align)
{
  // We are going to launch channels * height_col * width_col kernels, each
  ...
  im2col_align_bin_gpu_kernel<<<num_blocks, BLOCK, 0, get_cuda_stream()>>>(
      num_kernels, im, height, width, ksize, channels, pad, stride, height_col,
...
/*
__global__ void float_to_bit_gpu_kernel(float *src, unsigned char *dst, size_t
size)
{
    ...
    //for (; index < size_aligned; index += blockDim.x*gridDim.x)
    {
        ...
        if (threadIdx.x % WARP_SIZE == 0) ((unsigned int*)dst)[index / 32] =
bit_mask;
    }
}
*/
...
__global__ void float_to_bit_gpu_kernel(float *src, unsigned char *dst,
                                        size_t size)
{
...
void float_to_bit_gpu(float *src, unsigned char *dst, size_t size)
{
  ...
  float_to_bit_gpu_kernel<<<num_blocks, 1024, 0, get_cuda_stream()>>>(src, dst,
                                                                      size);
  CHECK_CUDA(cudaPeekAtLastError());
}
// --------------------------------
...
/*
__device__ __host__ static inline void remove_bit(unsigned char *const dst,
size_t index) { size_t dst_i = index / 8; int dst_shift = index % 8; dst[dst_i]
&= ~(1 << dst_shift);
}

__device__ __host__ static inline void set_bit(unsigned char *const dst, size_t
index) { size_t dst_i = index / 8; int dst_shift = index % 8; dst[dst_i] |= 1 <<
dst_shift;
    //dst[dst_i] |= 1 << (8 - dst_shift);
}
*/

__device__ __host__ static inline unsigned char get_bit(
    unsigned char const *const src, size_t index)
{
...
{
  return ((a * 0x0802LU & 0x22110LU) | (a * 0x8020LU & 0x88440LU)) *
             0x10101LU >>
         16;
}
...
// transpose 8x8 bit
__global__ void transpose_bin_gpu_kernel(unsigned char *A, unsigned char *B,
  ...
  // for (i = 0; i < n; i += 8)
  {
    ...
    // for (j = 0; j < m - 8; j += 8)
    {
      ...
      if (j < m) {
        int a_index = i * lda + j;
        int b_index = j * ldb + i;
        transpose8rS32_reversed_diagonale(&A[a_index / 8], &B[b_index / 8],
                                          lda / 8, ldb / 8);
      }
      // else if (j < m) {
      //     for (; j < m; ++j) {
...
__device__ uint32_t reverse_32_bit(uint32_t a)
{
  ...
  // unsigned int __rbit(unsigned int val) // for ARM    //__asm__("rbit %0,
  // %1\n" : "=r"(output) : "r"(input));
  return __brev(a);
...
// transpose 32x32 bit
__global__ void transpose_bin_gpu_kernel_32(uint32_t *A, uint32_t *B,
                                            const int n, const int m,
                                            const int lda, const int ldb,
                                            const int block_size)
{
    ...
    // for (j = 0; j < m - 8; j += 8)
    {
      ...
      if (j < m) {
        ...
        transpose_32x32_bits_reversed_diagonale(
            &A[a_index / 32], &B[b_index / 32], lda / 32, ldb / 32);
      }
    }
  }
}

void transpose_bin_gpu(unsigned char *A, unsigned char *B, const int n,
                       ...
                       const int block_size)
{
  ...
  transpose_bin_gpu_kernel_32<<<num_blocks32, BLOCK_TRANSPOSE32, 0,
                                get_cuda_stream()>>>(
      (uint32_t *)A, (uint32_t *)B, n, m, lda, ldb, block_size);
  // transpose_bin_gpu_kernel <<<num_blocks, BLOCK, 0, get_cuda_stream() >>>(A,
  // B, n, m, lda, ldb, block_size);
  CHECK_CUDA(cudaPeekAtLastError());
}
// --------------------------------

__global__ void transpose_uint32_kernel(uint32_t *src, uint32_t *dst, int src_h,
                                        int src_w, int src_align, int dst_align)
{
  ...
  int i = index % src_h;  // l.size*l.size*l.c;
  {
    ...
    if (j < src_w) {
      ((uint32_t *)dst)[j * dst_align / 32 + i] =
          ((uint32_t *)src)[i * src_align + j];
    }
  }
}

void transpose_uint32_gpu(uint32_t *src, uint32_t *dst, int src_h, int src_w,
                          int src_align, int dst_align)
{
  int size = src_w * src_h;
  const int num_blocks = size / BLOCK + 1;
  transpose_uint32_kernel<<<num_blocks, BLOCK, 0, get_cuda_stream()>>>(
...
void transpose_uint32_gpu_2(uint32_t *src, uint32_t *dst, int src_h, int src_w,
                            int src_align, int dst_align)
{
  ...
  transpose_uint32_kernel_2<<<num_blocks, TRANS_BLOCK, 0, get_cuda_stream()>>>(
      src, dst, src_h, src_w, src_align, dst_align);
  CHECK_CUDA(cudaPeekAtLastError());
}
// --------------------------------
...
__global__ void repack_input_kernel(float *input, float *re_packed_input, int w,
                                    int h, int c)
{
  int index = blockIdx.x * blockDim.x + threadIdx.x;
...
void repack_input_gpu(float *input, float *re_packed_input, int w, int h, int c)
{
  int size = w * h * c;
  const int num_blocks = size / BLOCK + 1;
  repack_input_kernel<<<num_blocks, BLOCK, 0, get_cuda_stream()>>>(
      input, re_packed_input, w, h, c);
  CHECK_CUDA(cudaPeekAtLastError());
}
// --------------------------------
...
// 32 channels -> 1 channel (with 32 floats)
// 256 channels -> 8 channels (with 32 floats)
__global__ void repack_input_kernel_2(float *input, float *re_packed_input,
                                      int w, int h, int c)
{
  //__shared__ uint32_t tmp[33 * 32];  // 33x32 is misaligned 32 x 32 to avoid
  // bank conflicts
...
void repack_input_gpu_2(float *input, float *re_packed_input, int w, int h,
                        int c)
{
  int size = w * h * c;
  const int num_blocks = size / BLOCK + 1;
  repack_input_kernel_2<<<num_blocks, BLOCK, 0, get_cuda_stream()>>>(
      input, re_packed_input, w, h, c);
  CHECK_CUDA(cudaPeekAtLastError());
}
// --------------------------------
...
__global__ void repack_input_kernel_bin(float *input,
                                        uint32_t *re_packed_input_bin, int w,
                                        int h, int c)
{
  ...
  const int items_per_channel = w * h;
  const int items_per_channel_aligned =
      items_per_channel + WARP_SIZE - (items_per_channel % WARP_SIZE);
  ...
  if (chan < c) {
    ...
    if ((i + lane_id) < items_per_channel) {
      re_packed_input_bin[chan * items_per_channel / 32 + (i + lane_id)] =
          result_bits;
    }
  }
}

void repack_input_gpu_bin(float *input, uint32_t *re_packed_input_bin, int w,
                          int h, int c)
{
  int size = (w * h * c) / 32 + 1;
  ...
  repack_input_kernel_bin<<<num_blocks, block_size, 0, get_cuda_stream()>>>(
      input, re_packed_input_bin, w, h, c);
  CHECK_CUDA(cudaPeekAtLastError());
}
...
// 32 channels -> 1 channel (with 32 floats)
// 256 channels -> 8 channels (with 32 floats)
__global__ void repack_input_kernel_bin(float *input, uint32_t
*re_packed_input_bin, int w, int h, int c)
{
    //__shared__ uint32_t tmp[32];
    int index = blockIdx.x*blockDim.x + threadIdx.x;
    ...
    if(chan < c)
    {
        ...
        //if (i < items_per_channel)
        {
            //for (c_pack = 0; c_pack < 32; ++c_pack)
            {
                ...
                uint32_t bit_mask = __ballot_custom(src > 0);
                if (threadIdx.x % 32 == 0)
                    re_packed_input_bin[chan*items_per_channel / 32 + i] =
bit_mask;
            }
        }
    }
...
int h, int c)
{
    ...
    const int num_blocks = get_number_of_blocks(size, block_size);
    printf("\n num_blocks = %d, num_blocks/32 = %d,  block_size = %d \n",
num_blocks, num_blocks/32, block_size); repack_input_kernel_bin <<<num_blocks,
block_size, 0, get_cuda_stream() >>>(input, re_packed_input_bin, w, h, c);
    CHECK_CUDA(cudaPeekAtLastError());
}
*/

__global__ void fill_int8_gpu_kernel(unsigned char *src, unsigned char val,
                                     size_t size)
{
  int index = blockIdx.x * blockDim.x + threadIdx.x;
...
{
  const int num_blocks = size / BLOCK + 1;
  fill_int8_gpu_kernel<<<num_blocks, BLOCK, 0, get_cuda_stream()>>>(src, val,
                                                                    size);
  CHECK_CUDA(cudaPeekAtLastError());
}
// --------------------------------
...
__device__ __host__ static inline ulonglong4 xnor_int256(ulonglong4 a,
ulonglong4 b) { ulonglong4 res; res.w = ~(a.w^b.w); res.x = ~(a.x^b.x); res.y =
~(a.y^b.y); res.z = ~(a.z^b.z); return res;
}
*/
//-------
/*
...
*/
__device__ __host__ static inline ulonglong4 xor_int256(ulonglong4 a,
                                                        ulonglong4 b)
{
  ulonglong4 res;
  res.w = (a.w ^ b.w);
...
__global__ void gemm_nn_custom_bin_mean_transposed_gpu_kernel(int M, int N, int
K, unsigned char *A, int lda, unsigned char *B, int ldb, float *C, int ldc,
float *mean_arr)
{
    int index = blockIdx.x*blockDim.x + threadIdx.x;
    ...
    //if (index == 0)
    {
        ...
        {   // l.n - filters [16 - 55 - 1024]
            ...
            { // out_h*out_w - one channel output size [169 - 173056]
                ...
                for (k = 0; k < K; k += 64) {   // l.size*l.size*l.c - one
filter size [27 - 9216] uint64_t a_bit64 = *((uint64_t *)(A + (i*lda + k) / 8));
                    uint64_t b_bit64 = *((uint64_t *)(B + (j*ldb + k) / 8));
                    uint64_t c_bit64 = xnor_int64(a_bit64, b_bit64);

                    int tmp_count = __popcll(c_bit64);

                    if (K - k < 64)  tmp_count = tmp_count - (64 - (K - k)); //
remove extra bits count += tmp_count;
...
{ int x = start_j*ldb + k; if (x < (N*ldb)) *((ulonglong4 *)(B_s + k / 8)) =
*((ulonglong4 *)(B + x / 8));
        }

        ////if (j_cur < N && (index % M == 0 || threadIdx.x == 0)) {
          ////  for (int k = 0; k < K; k += 64) {   // l.size*l.size*l.c - one
filter size [27 - 9216]
            ////    *((uint64_t *)(B_s + (local_j*ldb + k) / 8)) = *((uint64_t
*)(B + (j_cur*ldb + k) / 8));    // input
            ////}
        ////}
    }
    __syncthreads();
    ...
    //for(int in_tmp = threadIdx.x; in_tmp < 1*blockDim.x; in_tmp += blockDim.x)
    {
        ...
        {   // l.n - filters [16 - 55 - 1024]
            ...
            { // out_h*out_w - one channel output size [169 - 173056]
                ...
                int count = 0;
                int k = 0;
                for (k = 0; k < K; k += bit_step) {   // l.size*l.size*l.c - one
filter size [27 - 144 - 9216] ulonglong4 a_bit256 = *((ulonglong4 *)(A + (i*lda
+ k) / 8));    // weights
                    //ulonglong4 b_bit256 = *((ulonglong4 *)(B + (j*ldb + k) /
8)); ulonglong4 b_bit256 = *((ulonglong4 *)(B_s + (local_j*ldb + k) / 8));    //
input ulonglong4 c_bit256 = xnor_int256(a_bit256, b_bit256);
                ...
                count = count - f1;    // remove extra bits (from empty space
for align only) C[i*ldc + j] = (2 * count - K) * mean_val;
...
float *mean_arr)
{
    ...
    __shared__ uint64_t A_s[6144];  // 48 KB // [lda x M`]
                                    //__shared__ uint8_t A_s[6144*8];  // 48 KB
// [lda x M`]
    ...
    for (int k = threadIdx.x * 64; k < shared_size; k += blockDim.x * 64) {
        int x = start_i*lda + k;
        if (x < (M*lda)) *((uint64_t *)(A_s + k / 8)) = *((uint64_t *)(A + x /
8));
    }

    //if (i_cur < M && (index % N == 0 || threadIdx.x == 0)) {
    //for (int k = 0; k < K; k += 64) {   // l.size*l.size*l.c - one filter size
[27 - 9216]
    //(*(uint64_t *)(A_s + (local_i*lda + k) / 8)) = *((uint64_t *)(A +
(i_cur*lda + k) / 8));    // weights
    //  }
    //}
    ...
    {    // out_h*out_w - one channel output size [169 - 173056]
        ...
        if (i < M)  // l.n - filters [16 - 55 - 1024]
        {
            ...
            for (k = 0; k < K; k += 64) {   // l.size*l.size*l.c - one filter
size [27 - 9216]
                //uint64_t a_bit64 = *((uint64_t *)(A + (i*lda + k) / 8));    //
weights uint64_t a_bit64 = *((uint64_t *)(A_s + (local_i*lda + k) / 8));    //
weights uint64_t b_bit64 = *((uint64_t *)(B + (j*ldb + k) / 8));            //
input uint64_t c_bit64 = xnor_int64(a_bit64, b_bit64);
...
__global__ void gemm_nn_custom_bin_mean_transposed_tensor_kernel(
    ...
    float *shortcut_in_gpu, float *shortcut_out_gpu)
{
  ...
  __shared__ int C_s[WMMA_N * WMMA_M * 32 * 2];  // 2 * 8 KB - Temprorary result
                                                 // of GEMM WMMA for 32 warps
  ...
  for (int tmp_index = threadIdx.x; tmp_index < (8 * 512); tmp_index +=
  blockDim.x)
  {
  ...
  */
  ...
  {  // out_h*out_w - one channel output size [169 - 173056]
    ...
    if (i < M)  // if (i < M)  // l.n - filters [16 - 55 - 1024]
    {
      ...
      for (; k < K;
           k += 128)  // l.size*l.size*l.c - one filter size [27 - 144 - 9216]
      {
        ...
        // if (i == start_i) wmma::load_matrix_sync(a_frag, &A_s[k / 32], (512 *
        // 32));   // lda = (128*32) bits else wmma::load_matrix_sync(a_frag,
        // (uint32_t *)(A + A_cur_index), lda);   // lda = M

        // lda, ldb - are in bits
        wmma::load_matrix_sync(a_frag, (uint32_t *)(A + A_cur_index),
                               lda);  // lda = M

        wmma::load_matrix_sync(b_frag, (uint32_t *)(B + B1_cur_index),
                               ldb);                        // ldb = K
        wmma::bmma_sync(c1_frag, a_frag, b_frag, c1_frag);  // XOR-GEMM

        wmma::load_matrix_sync(b_frag, (uint32_t *)(B + B2_cur_index),
                               ldb);                        // ldb = K
        wmma::bmma_sync(c2_frag, a_frag, b_frag, c2_frag);  // XOR-GEMM
      }
      // C[i*ldc + j]
      wmma::store_matrix_sync(&C_s[warp_id * WMMA_M * WMMA_N], c1_frag, WMMA_N,
                              wmma::mem_row_major);
      wmma::store_matrix_sync(
          &C_s[warp_id * WMMA_M * WMMA_N + WMMA_M * WMMA_N * 32], c2_frag,
           ...
           k += 128)  // l.size*l.size*l.c - one filter size [27 - 144 - 9216]
      {
        ...
        int j_d = lane_id / 4;
        uint32_t a_val =
            *(uint32_t *)(A + ((i + i_d) * lda + (k + k_d * 32)) /
                                  8);  // wmma::load_matrix_sync(a_frag,
                                       // (uint32_t *)(A + A_cur_index), lda);

        for (int c_x = 0; c_x < 2; c_x++) {
          uint32_t b_val =
              *(uint32_t *)(B + ((c_x * 8 + j + j_d) * ldb + (k + k_d * 32)) /
                                    8);  // wmma::load_matrix_sync(b_frag,
                                         // (uint32_t *)(B + B_cur_index), ldb);
...
#pragma unroll
            for (int local_k = 0; local_k < 4; ++local_k) {
              accum_c_val[local_j + c_x * 8] +=
                  __shfl_custom(c_val[local_j], i_d * 4 + local_k);
            }
          }
        }
      }

      // only the first 8 threads (i) contain 8 good values each, in c_val[8]
      // (j) = 8 x 8 =64 wmma::store_matrix_sync(&C_s[warp_id*WMMA_M*WMMA_N],
      // c_frag, WMMA_N, wmma::mem_row_major);
      if (k_d == 0) {
        for (int c_x = 0; c_x < 2; c_x++) {
          for (int local_j = 0; local_j < 8; ++local_j) {
...
#pragma unroll
          for (int i_d = lane_id / WMMA_N; i_d < WMMA_M; i_d += WMMA_M / 2) {
            int count = C_s[warp_id * WMMA_M * WMMA_N + i_d * WMMA_N + j_d +
                            WMMA_M * WMMA_N * 32 * c_x];

            const int bit_step = 128;
            int f1 = (K % bit_step == 0) ? 0 : (bit_step - (K % bit_step));
            count = count -
                    f1;  // remove extra bits (from empty space for align only)
            ...
            if (leaky_activation)
              dst_val = (dst_val >= 0) ? (dst_val)
                                       : (0.1f * dst_val);  // Leaky activation

            size_t out_index = (i + i_d) * ldc + (c_x * 8 + j + j_d);
            C[out_index] = dst_val;

            if (shortcut_out_gpu) {
              shortcut_out_gpu[out_index] =
                  shortcut_in_gpu[out_index] + dst_val;
            }
          }
        }
      }
...
/*
// Tensor Cores are used for XOR-GEMM
__global__ void gemm_nn_custom_bin_mean_transposed_tensor_kernel(int M, int N,
int K, unsigned char *A, int lda, unsigned char *B, int ldb, float *C, int ldc,
float *mean_arr, float *bias_arr, int leaky_activation)
{
    // total 57%
    int index = blockIdx.x*blockDim.x + threadIdx.x;
    ...
    {    // out_h*out_w - one channel output size [169 - 173056]
        ...
        if (i < M)  //if (i < M)  // l.n - filters [16 - 55 - 1024]
        {
            ...
            wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K,
wmma::experimental::precision::b1, wmma::row_major> a_frag;
            wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_N, WMMA_K,
wmma::experimental::precision::b1, wmma::col_major> b_frag;
            wmma::fragment<wmma::accumulator, WMMA_M, WMMA_N, WMMA_K, int>
c_frag; wmma::fill_fragment(c_frag, 0); // !!!! XOR isn't XNOR !!!!!!!!!!

            // 8 x 8 x 4 (uint32_t, 4 * 32 = 128 bit)
            for (; k < K; k += 128)  // l.size*l.size*l.c - one filter size [27
- 144 - 9216]
            {
                int64_t A_cur_index = (i*lda + k) / 8;
                //int64_t A_cur_index = (local_i*lda + k) / 8;
                ...
                // lda, ldb - are in bits
                wmma::load_matrix_sync(a_frag, (uint32_t *)(A + A_cur_index),
lda);   // lda = M wmma::load_matrix_sync(b_frag, (uint32_t *)(B + B_cur_index),
ldb);   // ldb = K

                wmma::bmma_sync(c_frag, a_frag, b_frag, c_frag);    // XOR-GEMM
            }
            // C[i*ldc + j]
            wmma::store_matrix_sync(&C_s[warp_id*WMMA_M*WMMA_N], c_frag, WMMA_N,
wmma::mem_row_major); #else // __CUDA_ARCH__ >= 730
...
- 144 - 9216]
            {
                ...
                int i_d = lane_id / 4;
                int j_d = lane_id / 4;
                uint32_t a_val = *(uint32_t *)(A + ((i + i_d)*lda + (k +
k_d*32)) / 8); // wmma::load_matrix_sync(a_frag, (uint32_t *)(A + A_cur_index),
lda); uint32_t b_val = *(uint32_t *)(B + ((j + j_d)*ldb + (k + k_d*32)) / 8); //
wmma::load_matrix_sync(b_frag, (uint32_t *)(B + B_cur_index), ldb);
                ...
                for (int local_j = 0; local_j < 8; ++local_j)
                {
                    #pragma unroll
                    for (int local_k = 0; local_k < 4; ++local_k) {
                        accum_c_val[local_j] += __shfl_custom(c_val[local_j],
i_d * 4 + local_k);
                    }
                }
            }

            // only the first 8 threads (i) contain 8 good values each, in
c_val[8] (j) = 8 x 8 =64
            // wmma::store_matrix_sync(&C_s[warp_id*WMMA_M*WMMA_N], c_frag,
WMMA_N, wmma::mem_row_major); if (k_d == 0) { for (int local_j = 0; local_j < 8;
++local_j)
                {
                    C_s[warp_id*WMMA_M*WMMA_N + i_d*WMMA_N + local_j] =
accum_c_val[local_j];
...

            {
                int i_d = lane_id % WMMA_M;
                {
                    ...
                    for (int j_d = lane_id / WMMA_M; j_d < WMMA_N; j_d += WMMA_N
/ 2)
                    {
                        int count = C_s[warp_id*WMMA_M*WMMA_N + i_d*WMMA_N +
j_d];

                        const int bit_step = 128;
                        int f1 = (K % bit_step == 0) ? 0 : (bit_step - (K %
bit_step)); count = count - f1;    // remove extra bits (from empty space for
align only)
...
__global__ void gemm_nn_custom_bin_mean_transposed_gpu_kernel(
    int M, int N, int K, unsigned char *A, int lda, unsigned char *B, int ldb,
    float *C, int ldc, float *mean_arr, float *bias_arr, int leaky_activation,
    float *shortcut_in_gpu, float *shortcut_out_gpu)
{
  ...
  for (int k = threadIdx.x * 64; k < shared_size; k += blockDim.x * 64) {
    int x = start_i * lda + k;
    if (x < (M * lda))
      *((uint64_t *)(A_s + k / 8)) = *((uint64_t *)(A + x / 8));
  }
  __syncthreads();
  ...
  {  // out_h*out_w - one channel output size [169 - 173056]
    ...
    // if (i < M)  // l.n - filters [16 - 55 - 1024]
    {
      ...
      for (; k < (K - 8192);
           k += 8192) {  // l.size*l.size*l.c - one filter size [27 - 9216]
        ulonglong4 c_bit256;
...
#pragma unroll
      ...
      if (i < M) {
        ...
        for (; k < K;
          ...
          // ulonglong4 a_bit256 = *((ulonglong4 *)(A + (i*lda + k) / 8));    //
          // weights
          ulonglong4 a_bit256 =
              *((ulonglong4 *)(A_s + (local_i * lda + k) / 8));  // weights
          ulonglong4 b_bit256 =
              *((ulonglong4 *)(B + (j * ldb + k) / 8));  // input
...
#ifdef NOT_USED
        for (; k < K;
             k += 64) {  // l.size*l.size*l.c - one filter size [27 - 9216]
          // uint64_t a_bit64 = *((uint64_t *)(A + (i*lda + k) / 8));    //
          // weights
          uint64_t a_bit64 =
              *((uint64_t *)(A_s + (local_i * lda + k) / 8));         // weights
        ...
        int f1 = (K % bit_step == 0) ? 0 : (bit_step - (K % bit_step));
        count =
            count - f1;  // remove extra bits (from empty space for align only)
...
// nvcuda::wmma::col_major ->  cutlass::MatrixLayout::kColumnMajor (matrix is
// not transposed)
    ...
    float *shortcut_in_gpu, float *shortcut_out_gpu)
{
  ...
  M*K / 1024, N*K / 1024, M*lda / num_blocks / 1024, N*ldb / num_blocks / 1024);
  printf(" M / 512 = %d, N / 512 = %d, M*lda / 512 = %d, N*ldb / 512 = %d \n", M
  / 512, N / 512, M*lda/512, N*ldb/512);
  */
  // printf(" shared_memory: (w) lda*BLOCK/N = %d, (i) ldb*BLOCK/M = %d, \t lda
  ...
  if (1) {
    ...
    // printf(" lda = %d, ldb = %d, ldc = %d, lda/32 = %d, ldb/32 = %d, ldc/32 =
    // %d \n", lda, ldb, ldc, lda / 32, ldb / 32, ldc / 32); printf("  l.c (K/9)
    // = %d, M (l.n) = %d \n", (K%9 == 0)? K / 9: K, M);
    gemm_nn_custom_bin_mean_transposed_tensor_kernel<<<num_blocks, BLOCK, 0,
                                                       get_cuda_stream()>>>(
        M, N, K, A, lda, B, ldb, C, ldc, mean_arr, bias, leaky_activation,
...
/*
void convolve_cpu(float *input, float *weights, float *output, int in_w, int
in_h, int in_c, int n, int size, int pad)
{
    int fil;
    // filter index
//#pragma omp parallel for      // "omp parallel for" - automatic
parallelization of loop by using OpenMP for (fil = 0; fil < n; ++fil) { int
chan, y, x, f_y, f_x;
        // channel index
        for (chan = 0; chan < in_c; ++chan)
            ...
            for (y = 0; y < in_h; ++y)
                ...
                for (x = 0; x < in_w; ++x)
                {
                    ...
                    int const weights_pre_index = fil*in_c*size*size +
chan*size*size; int const input_pre_index = chan*in_w*in_h; float sum = 0;
                    ...
                    for (f_y = 0; f_y < size; ++f_y)
                    {
                        ...
                        for (f_x = 0; f_x < size; ++f_x)
                        {
                            ...
                            if (input_y < 0 || input_x < 0 || input_y >= in_h ||
input_x >= in_w) continue;

                            int input_index = input_pre_index + input_y*in_w +
input_x; int weights_index = weights_pre_index + f_y*size + f_x;
                    ...
                    //
l.weights[filters][channels][filter_width][filter_height]; output[output_index]
+= sum;
                }
    }
...
void convolve_bin_cpu(float *input, float *weights, float *output, int in_w, int
in_h, int in_c, int n, int size, int pad, int new_lda, float *mean_arr_gpu)
{
    int fil;
    // filter index
//#pragma omp parallel for      // "omp parallel for" - automatic
parallelization of loop by using OpenMP for (fil = 0; fil < n; ++fil) { float
mean_val = mean_arr_gpu[fil]; int chan, y, x, f_y, f_x;
        // channel index
        for (chan = 0; chan < in_c; ++chan)
            // input - y
            for (y = 0; y < in_h; ++y)
                ...
                for (x = 0; x < in_w; ++x)
                {
                    int const output_index = fil*in_w*in_h + y*in_w + x;
                    int const weights_pre_index = fil*in_c*size*size +
chan*size*size; int const input_pre_index = chan*in_w*in_h; int sum = 0; int
good_val = 0;
...
input_x >= in_w) continue;
                            ...
                            int input_index = input_pre_index + input_y*in_w +
input_x;
                            //int weights_index = weights_pre_index + f_y*size +
f_x;
                            //int weights_index = fil*in_c*size*size +
chan*size*size + f_y*size + f_x; int weights_index = fil*new_lda +
chan*size*size + f_y*size + f_x;

                            //sum += input[input_index] *
weights[weights_index];

                            int8_t in_bit = get_bit((uint8_t *)input,
input_index); int8_t w_bit = get_bit((uint8_t *)weights, weights_index); int res
= xnor_bit1(in_bit, w_bit); sum += res; good_val++;
                            //sum += (res > 0) ? 1 : -1;
                            //in_bit = (in_bit > 0) ? 1 : -1;
                            //w_bit = (w_bit > 0) ? 1 : -1;
                            //int8_t res = in_bit*w_bit;
                            //sum += res;
                            //printf("\n i: %d x w: %d = res: %d \t sum: %d \t
...
__global__ void convolve_gpu_kernel(float *input, float *weights, float *output,
                                    int in_w, int in_h, int in_c, int n,
                                    int size, int pad)
{
  int index = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  if (fil < n) {
    ...
    for (chan = 0; chan < in_c; ++chan) {
      int const weights_pre_index =
          fil * in_c * size * size + chan * size * size;
      ...
      for (f_y = 0; f_y < size; ++f_y) {
        ...
        for (f_x = 0; f_x < size; ++f_x) {
          int input_x = x + f_x - pad;
          if (input_y < 0 || input_x < 0 || input_y >= in_h || input_x >= in_w)
            continue;
...
void convolve_gpu(float *input, float *weights, float *output, int in_w,
                  int in_h, int in_c, int n, int size, int pad)
{
  int array_size = in_w * in_h * n;  // width X height X filters
  const int num_blocks = array_size / BLOCK + 1;
  // printf("\n array_size = %d, num_blocks = %d, w = %d, h = %d, n = %d, c =
  // %d, pad = %d \n", array_size, num_blocks, in_w, in_h, n, in_c, pad);

  convolve_gpu_kernel<<<num_blocks, BLOCK, 0, get_cuda_stream()>>>(
      input, weights, output, in_w, in_h, in_c, n, size, pad);
  CHECK_CUDA(cudaPeekAtLastError());
}
...
/*
__global__ void convolve_bin_gpu_kernel(float *input, float *weights, float
*output, int in_w, int in_h, int in_c, int n, int size, int pad, int new_lda,
float *mean_arr_gpu)
{
    int index = blockIdx.x*blockDim.x + threadIdx.x;
    ...
    if (fil < n)    // (1-6 for one BLOCK)
    {
                ...
                {
                    //int const weights_pre_index = fil*in_c*size*size +
chan*size*size; int const weights_pre_index = fil*new_lda + chan*size*size; int
const input_pre_index = chan*in_w*in_h;
...
input_x; int weights_index = weights_pre_index + f_y*size + f_x;
                            //int weights_index = fil*in_c*size*size +
chan*size*size + f_y*size + f_x;
                            //int weights_index = fil*new_lda + chan*size*size +
f_y*size + f_x;

                            uint8_t in_bit = get_bit((uint8_t *)input,
input_index); uint8_t w_bit = get_bit((uint8_t *)weights, weights_index); int
res = xnor_bit1(in_bit, w_bit); sum += res; good_val++;
...
l.weights[filters][channels][filter_width][filter_height];
                ...
                sum = sum - (good_val - sum);
                output[output_index] = sum * mean_arr_gpu[fil]; // atoimcAdd for
inter-BLOCK sum
    }
...
__global__ void convolve_bin_gpu_kernel(float *input, float *weights,
                                        float *output, int in_w, int in_h,
                                        int in_c, int n, int size, int pad,
                                        int new_lda, float *mean_arr_gpu)
{
  ...
  // if (fil < n)    // (1-6 for one BLOCK)
  {
    ...
    __shared__ uint32_t weights_shared[3 * 3 * 1024 * 6 / 32 +
                                       1];  // 7 KB (6 filters) - use (new_lda)
    ...
    for (int tmp_fil = min_fil; tmp_fil <= max_fil; tmp_fil++) {
      for (int s = threadIdx.x; s < weights_size; s += blockDim.x) {
        // weights_shared[s + (tmp_fil - min_fil)*new_lda / 8] = ((uint8_t
        // *)weights)[tmp_fil*new_lda / 8 + s];
        weights_shared[s + (tmp_fil - min_fil) * new_lda / 32] =
            ((uint32_t *)weights)[tmp_fil * new_lda / 32 + s];
      }
    }
    __syncthreads();
    ...
    for (chan = 0; chan < in_c; ++chan) {
      ...
      __shared__ uint32_t
          input_shared[416 * 416 / 32 +
                       1];  // 21.2 KB bytes (for input size 832x832)
      ...
      /*
      __shared__ uint8_t input_shared[208 * 208 / 8 + 1];   // 5.4 KB bytes (for
      input size 416x416) const int input_shared_size = in_w*in_h / 8 + 1; const
      int add_input_index = input_pre_index % 8;
      __syncthreads();

      for (int s = threadIdx.x; s < input_shared_size; s += blockDim.x) {
          ((uint8_t *)input_shared)[s] = ((uint8_t *)input)[input_pre_index / 8
      + s];
      }
      __syncthreads();
      */
      ...
      if (fil < n)  // (1-6 for one BLOCK)
      {
        ...
        for (f_y = 0; f_y < size; ++f_y) {
          ...
          for (f_x = 0; f_x < size; ++f_x) {
            ...
            // int input_index = input_pre_index + input_y*in_w + input_x;
            // int weights_index = weights_pre_index + f_y*size + f_x;
            // int weights_index = fil*in_c*size*size + chan*size*size +
            // f_y*size + f_x; int weights_index = fil*new_lda + chan*size*size
            // + f_y*size + f_x;
            ...
            // int weights_index = fil*in_c*size*size + chan*size*size +
            // f_y*size + f_x;
            int weights_shared_index = (fil - min_fil) * new_lda +
                                       chan * size * size + f_y * size + f_x;
            // uint8_t in_bit = get_bit((uint8_t *)weights_shared,
            // weights_shared_index);
            uint8_t w_bit =
                get_bit((uint8_t *)weights_shared, weights_shared_index);

            // int input_index = input_pre_index + input_y*in_w + input_x;
            int input_shared_index = /*input_pre_index +*/ input_y * in_w +
                                     input_x + add_input_index;
            uint8_t in_bit =
                get_bit((uint8_t *)input_shared, input_shared_index);
            /*
...
void convolve_bin_gpu(float *input, float *weights, float *output, int in_w,
                      int in_h, int in_c, int n, int size, int pad, int new_lda,
                      float *mean_arr_gpu)
{
  int array_size = in_w * in_h * n;  // width X height X filters
  const int num_blocks = array_size / BLOCK + 1;
  ...
  convolve_bin_gpu_kernel<<<num_blocks, BLOCK, 0, get_cuda_stream()>>>(
      input, weights, output, in_w, in_h, in_c, n, size, pad, new_lda,
      mean_arr_gpu);
  CHECK_CUDA(cudaPeekAtLastError());
}
...
__global__ void im2col_gpu_kernel_ext(
    const int n, const float *data_im, const int height, const int width,
...
void im2col_gpu_ext(const float *data_im, const int channels, const int height,
