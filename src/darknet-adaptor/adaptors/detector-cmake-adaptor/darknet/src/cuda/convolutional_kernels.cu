...
{
  binarize_kernel<<<cuda_gridsize(n), BLOCK, 0, get_cuda_stream()>>>(x, n,
                                                                     binary);
...
// only if (size % 32 == 0)
__global__ void reduce_kernel(float *weights, int n, int size,
                              float *mean_arr_gpu)
{
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  int f = i / size;
  ...
  if (i % 32 == 0) atomicAdd(&mean_arr_gpu[f], warp_mean / size);
}

__global__ void binarize_weights_mean_kernel(float *weights, int n, int size,
                                             float *binary, float *mean_arr_gpu)
{
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  int f = i / size;
  if (f >= n) return;
...
void fast_binarize_weights_gpu(float *weights, int n, int size, float *binary,
                               float *mean_arr_gpu)
{
  if (size % 32 == 0) {
    ...
    set_zero_kernel<<<(n / BLOCK + 1), BLOCK, 0, get_cuda_stream()>>>(
        mean_arr_gpu, n);
    reduce_kernel<<<num_blocks, BLOCK, 0, get_cuda_stream()>>>(weights, n, size,
                                                               mean_arr_gpu);
...
void forward_convolutional_layer_gpu(convolutional_layer l, network_state state)
{
  ...
  if (l.xnor) {
    if (!l.align_bit_weights_gpu || state.train) {
      // binarize_weights_gpu(l.weights_gpu, l.n, (l.c /
      // l.groups)*l.size*l.size, l.binary_weights_gpu);

      fast_binarize_weights_gpu(l.weights_gpu, l.n,
                                (l.c / l.groups) * l.size * l.size,
                                l.binary_weights_gpu, l.mean_arr_gpu);
    }
    ...
    if (l.align_bit_weights_gpu && !state.train && l.c >= 32 &&
      ...
      if (l.c % 32 == 0) {
        // printf("\n\n l.index = %d, l.w = %d, l.c = %d, l.n = %d, l.stride =
        // %d, l.pad = %d - new XNOR \n", l.index, l.w, l.c, l.n, l.stride,
        // l.pad); printf("l.align_workspace_size = %d, (l.c * l.w * l.h)  = %d
        // \n", l.align_workspace_size, (l.c * l.w * l.h));

        // float *intput_cpu = (float *)calloc(l.inputs, sizeof(float));
        //  state.input
        // cudaMemcpy(intput_cpu, state.input, l.inputs * sizeof(float),
        // cudaMemcpyDefault);
        ...
        // float *re_packed_input = (float *)calloc(l.c * l.w * l.h,
        // sizeof(float)); uint32_t *bin_re_packed_input = (uint32_t
        // *)calloc(new_c * l.w * l.h + 1, sizeof(uint32_t));

        // float32x4 by channel (as in cuDNN)
        // repack_input(intput_cpu, re_packed_input, l.w, l.h, l.c);

        // 32 x floats -> 1 x uint32_t
        // float_to_bit(re_packed_input, (uint8_t *)bin_re_packed_input, l.c *
        // l.w * l.h);

        // cudaDeviceSynchronize();
        // start_timer();

        repack_input_gpu_bin(state.input, (uint32_t *)l.align_workspace_gpu,
                             l.w, l.h, l.c);

        // repack_input_gpu(state.input, state.workspace, l.w, l.h, l.c);

        // 32 x floats -> 1 x uint32_t
        // float_to_bit_gpu(state.workspace, (unsigned char
        // *)l.align_workspace_gpu, l.c * l.w * l.h);// l.align_workspace_size);
        ...
        // slow - convolution the packed inputs and weights: float x 32 by
        // channel (as in cuDNN)
        // convolution_repacked((uint32_t *)bin_re_packed_input, (uint32_t
        // *)l.align_bit_weights, l.output,
        ...
        // im2col_cpu_custom((float *)bin_re_packed_input, new_c, l.h, l.w,
        // l.size, l.stride, l.pad, b);

        // cudaMemcpy(l.align_workspace_gpu, bin_re_packed_input, (new_c * l.w *
        // l.h + 1) * sizeof(uint32_t), cudaMemcpyDefault);

        // start_timer();
        im2col_ongpu(l.align_workspace_gpu, new_c, l.h, l.w, l.size, l.stride,
                     l.pad, state.workspace);
        // cudaDeviceSynchronize();
        ...
        // transpose_uint32((uint32_t *)b, (uint32_t *)t_bit_input, new_k, n, n,
        // new_ldb); cudaMemcpy(l.transposed_align_workspace_gpu, t_bit_input,
        // t_bit_input_size * sizeof(char), cudaMemcpyDefault);

        // cudaMemcpy(state.workspace, b, t_bit_input_size * sizeof(char),
        // cudaMemcpyDefault); printf("\n n = %d, n % 32 = %d, new_ldb = %d,
        // new_ldb % 32 = %d \n", n, n % 32, new_ldb, new_ldb % 32);

        // start_timer();
        transpose_uint32_gpu((uint32_t *)state.workspace,
                             (uint32_t *)l.transposed_align_workspace_gpu,
                             new_k, n, n, new_ldb);
        // cudaDeviceSynchronize();
        // stop_timer_and_show_name("transpose_uint32_gpu");

        // cudaDeviceSynchronize();
        // stop_timer_and_show_name("repack_input_gpu_bin + im2col_ongpu +
        // transpose_uint32_gpu_2");

        // start_timer();
        gemm_nn_custom_bin_mean_transposed_gpu(
            m, n, k, (unsigned char *)l.align_bit_weights_gpu, new_ldb,
        ...
        // gemm_nn_custom_bin_mean_transposed(m, n, k, 1, (uint8_t
        // *)l.align_bit_weights, new_ldb, (uint8_t *)t_bit_input, new_ldb, c,
        // n, l.mean_arr);

        // add_bias(l.output, l.biases, l.batch, l.n, l.out_h*l.out_w);

        // cudaMemcpy(l.output_gpu, l.output, l.outputs * sizeof(float),
        // cudaMemcpyDefault);
        ...
        // printf("\n\n l.index = %d, l.w = %d, l.c = %d, l.n = %d, l.stride =
        // %d, l.pad = %d - old XNOR \n", l.index, l.w, l.c, l.n, l.stride,
        // l.pad); cudaDeviceSynchronize();

        int i = 0;
        /*
        // if (l.stride == 1 && l.c >= 256 && l.size > 1)
        if (l.stride == 1 && l.c >= 1024 && l.size > 1 && 0)// && l.w >= 13) //
        disabled
        {
            // stride=1 only
            //start_timer();
            im2col_align_bin_ongpu(state.input + i*l.c*l.h*l.w, l.c, l.h, l.w,
        l.size, l.stride, l.pad, state.workspace, l.bit_align);
            //cudaDeviceSynchronize();
        ...
        {
          // start_timer();
          im2col_align_ongpu(state.input + i * l.c * l.h * l.w, l.c, l.h, l.w,
                             l.size, l.stride, l.pad, l.align_workspace_gpu,
                             l.bit_align);
          // cudaDeviceSynchronize();
          ...
          float_to_bit_gpu(l.align_workspace_gpu,
                           (unsigned char *)state.workspace,
                           l.align_workspace_size);
          // cudaDeviceSynchronize();
          // stop_timer_and_show_name("float_to_bit_gpu");
        }
        // start_timer();
        transpose_bin_gpu((unsigned char *)state.workspace,
                          (unsigned char *)l.transposed_align_workspace_gpu, k,
                          n, l.bit_align, new_ldb, 8);
        // cudaDeviceSynchronize();
        // stop_timer_and_show_name("transpose_bin_gpu");

        // cudaDeviceSynchronize();
        // stop_timer_and_show_name("im2col_align_ongpu + float_to_bit_gpu +
        // transpose_bin_gpu");

        // should be optimized
        // if(0) {//if (k > 1000) {    // sequentially input-shared - BAD
        //    gemm_nn_custom_bin_mean_transposed_sequentially_gpu(m, n, k,
        //        (unsigned char *)l.align_bit_weights_gpu, new_ldb, (unsigned
        //        char *)l.transposed_align_workspace_gpu, new_ldb,
        //        l.output_gpu, n, l.mean_arr_gpu);
        //}
      ...
      /*
      {
          float_to_bit_gpu(state.input, (unsigned char *)l.align_workspace_gpu,
      input_size); convolve_bin_gpu(l.align_workspace_gpu, (float
      *)l.align_bit_weights_gpu, l.output_gpu, l.w, l.h, l.c, l.n, l.size,
      l.pad, l.new_lda, l.mean_arr_gpu);

          //convolve_gpu(state.input, l.weights_gpu, l.output_gpu, l.w, l.h,
      l.c, l.n, l.size, l.pad);
      ...
      */
      ...
      if (l.activation == SWISH)
        ...
        activate_array_hard_mish_ongpu(l.output_gpu, l.outputs * l.batch,
                                       l.activation_input_gpu, l.output_gpu);
      else if (l.activation == NORM_CHAN)
        activate_array_normalize_channels_ongpu(
            l.output_gpu, l.outputs * l.batch, l.batch, l.out_c,
      ...
      // if(l.activation != LINEAR && l.activation != LEAKY)
      // activate_array_ongpu(l.output_gpu, l.outputs*l.batch, l.activation); if
      // (l.binary || l.xnor) swap_binary(&l); cudaDeviceSynchronize();
      return;
    }
  }
  ...
  if (state.index != 0 && state.net.cudnn_half && !l.xnor &&
    ...
    CHECK_CUDNN(cudnnConvolutionForward(
        cudnn_handle(), &alpha, l.srcTensorDesc16, input16, l.weightDesc16,
        l.weights_gpu16, l.convDesc, l.fw_algo16, state.workspace,
        l.workspace_size, &beta, l.dstTensorDesc16, output16));
    ...
    if (l.batch_normalize) {
      if (state.train && !state.net.adversarial)  // Training
      {
        ...
        // cudaMemcpyAsync(l.x_gpu, output16, l.outputs*l.batch*sizeof(half),
        // cudaMemcpyDefault, get_cuda_stream());
        float one = 1.0f;
        float zero = 0.0f;
  ...
  for (i = 0; i < l.batch; ++i) {
    for (j = 0; j < l.groups; ++j) {
      ...
      if (l.size == 1 && l.stride == 1 && l.dilation == 1) {
        ...
        // im2col_ongpu(im, l.c / l.groups, l.h, l.w, l.size, l.stride, l.pad,
        // state.workspace);
  ...
  if (l.activation == SWISH)
    ...
    activate_array_normalize_channels_ongpu(l.output_gpu, l.outputs * l.batch,
                                            l.batch, l.out_c, l.out_w * l.out_h,
                                            l.output_gpu);
  else if (l.activation == NORM_CHAN_SOFTMAX)
    activate_array_normalize_channels_softmax_ongpu(
        l.output_gpu, l.outputs * l.batch, l.batch, l.out_c, l.out_w * l.out_h,
        l.output_gpu, 0);
  else if (l.activation == NORM_CHAN_SOFTMAX_MAXVAL)
    activate_array_normalize_channels_softmax_ongpu(
        l.output_gpu, l.outputs * l.batch, l.batch, l.out_c, l.out_w * l.out_h,
  ...
  if (l.antialiasing) {
    ...
    simple_copy_ongpu(l.outputs * l.batch, l.output_gpu,
  ...
  if (l.coordconv) {
    coord_conv_gpu(l.output_gpu, l.outputs * l.batch, l.out_w, l.out_h, l.out_c,
                   l.batch, 0);
  }
}
...
{
  if (l.coordconv) {
    coord_conv_gpu(l.delta_gpu, l.outputs * l.batch, l.out_w, l.out_h, l.out_c,
                   l.batch, 1);
  }
  ...
  if (l.antialiasing) {
    ...
    // if (!state.train) s.index = state.index;  // don't use TC for training
    // (especially without cuda_convert_f32_to_f16() )
    simple_copy_ongpu(l.input_layer->outputs * l.input_layer->batch,
                      l.delta_gpu, l.input_layer->delta_gpu);
    backward_convolutional_layer_gpu(*(l.input_layer), s);

    simple_copy_ongpu(l.outputs * l.batch, l.input_antialiasing_gpu,
                      l.output_gpu);
  }
  ...
  if (l.activation == SWISH)
    ...
    gradient_array_mish_ongpu(l.outputs * l.batch, l.activation_input_gpu,
                              l.delta_gpu);
  else if (l.activation == HARD_MISH)
    gradient_array_hard_mish_ongpu(l.outputs * l.batch, l.activation_input_gpu,
                                   l.delta_gpu);
  else if (l.activation == NORM_CHAN_SOFTMAX ||
    ...
    gradient_array_normalize_channels_ongpu(l.output_gpu, l.outputs * l.batch,
                                            l.batch, l.out_c, l.out_w * l.out_h,
                                            l.delta_gpu);
  else
  ...
  if (state.index != 0 && state.net.cudnn_half && !l.xnor &&
    ...
    if (l.batch_normalize) {
      ...
      // cudaMemcpyAsync(delta16, l.x_norm_gpu, l.outputs*l.batch *
      // sizeof(half), cudaMemcpyDefault, get_cuda_stream());
    } else {
      // backward_bias_gpu(l.bias_updates_gpu, l.delta_gpu, l.batch, l.n,
      // l.out_w*l.out_h);
    }
    ...
    if (!state.net.adversarial && !l.train_only_bn) {
      ...
      cuda_convert_f16_to_f32(l.weight_updates_gpu16, l.nweights,
    ...
    if (state.delta) {
      ...
      CHECK_CUDNN(cudnnConvolutionBackwardData(
          cudnn_handle(), &alpha, l.weightDesc16,
      ...
      if (l.binary || l.xnor) swap_binary(&l);
      if (l.xnor)
        gradient_array_ongpu(original_input, l.batch * l.c * l.h * l.w, HARDTAN,
                             state.delta);
    }
  } else {
    // #else    // CUDNN_HALF
      ...
      if (l.reverse) {
          if (*state.net.max_output16_size < l.inputs*l.batch) {
              *state.net.max_output16_size = l.inputs*l.batch;
              if (*state.net.output16_gpu) cuda_free(*state.net.output16_gpu);
              assert(*state.net.max_output16_size > 0);
              *state.net.output16_gpu = cuda_make_array(NULL,
      *state.net.max_output16_size);
          }
          float clip = 0.0;
          float divider = 1.0;
          float abs_add = 1.0;
          mult_inverse_array_gpu(state.input, *state.net.output16_gpu,
      l.inputs*l.batch, l.reverse, divider, clip, abs_add); state.input =
      *state.net.output16_gpu;
      }
      */
      ...
      CHECK_CUDNN(cudnnConvolutionBackwardFilter(
          cudnn_handle(), &one, l.srcTensorDesc, state.input, l.ddstTensorDesc,
      ...
      if (l.reverse) {
          if (*state.net.max_output16_size < l.nweights) {
              *state.net.max_output16_size = l.nweights;
              if (*state.net.output16_gpu && *state.net.max_output16_size > 0)
      cuda_free(*state.net.output16_gpu); assert(*state.net.max_output16_size >
      0); *state.net.output16_gpu = cuda_make_array(NULL, l.nweights);
          }
          float clip = 0.0;
          float divider = 1.0;
          float abs_add = 1.0;
          mult_inverse_array_gpu(l.weights_gpu, *state.net.output16_gpu,
      l.nweights, l.reverse, divider, clip, abs_add); l.weights_gpu =
      *state.net.output16_gpu;
      }
      */
  ...
  for (i = 0; i < l.batch; ++i) {
    for (j = 0; j < l.groups; ++j) {
      ...
      if (state.delta) {
        ...
        // col2im_ongpu(state.workspace, l.c / l.groups, l.h, l.w, l.size,
        ...
        col2im_gpu_ext(state.workspace,  // input
                       ...
                       l.size, l.size,   // kernel size (h, w)
                       l.pad * l.dilation,
                       l.pad * l.dilation,      // padding size (h, w)
...
__global__ void calc_avg_activation_kernel(float *src, float *dst, int size,
...
void calc_avg_activation_gpu(float *src, float *dst, int size, int channels,
                             int batches)
{
...
__global__ void assisted_activation_kernel(float alpha, float *output,
                                           float *gt_gpu, float *a_avg_gpu,
                                           int size, int channels, int batches)
{
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  int xy = i % size;
  int b = i / size;
...
void assisted_activation_gpu(float alpha, float *output, float *gt_gpu,
                             float *a_avg_gpu, int size, int channels,
                             int batches)
{
  const int num_blocks = get_number_of_blocks(size * batches, BLOCK);
                                            ...
                                            int size, int channels, int batches)
{
  ...
  if (b < batches) {
    for (int c = 0; c < channels; ++c) {
      if (gt_gpu[i] == 0) output[xy + size * (c + channels * b)] *= beta;
    }
  }
}
...
void assisted_excitation_forward_gpu(convolutional_layer l, network_state state)
{
  ...
  for (b = 0; b < l.batch; ++b) {
    ...
    for (t = 0; t < state.net.num_boxes; ++t) {
      ...
      // printf(" alpha = %f, beta = %f, truth.w = %f, dw = %f, tw+dw = %f,
  ...
  calc_avg_activation_gpu(l.output_gpu, l.a_avg_gpu, l.out_w * l.out_h, l.out_c,
                          l.batch);
  // cudaStreamSynchronize(get_cuda_stream());
  // CHECK_CUDA(cudaPeekAtLastError());

  // calc new output
  // assisted_activation2_gpu(1, l.output_gpu, l.gt_gpu, l.a_avg_gpu, l.out_w *
  // l.out_h, l.out_c, l.batch);  // AE3: gt increases (beta = 1 - alpha = 0)
  // assisted_activation2_gpu(alpha, l.output_gpu, l.gt_gpu, l.a_avg_gpu,
  ...
  for (b = 0; b < l.batch; ++b)
  {
      ...
      for (w = 0; w < l.out_w; w++) {
          for (h = 0; h < l.out_h; h++) {
              for (c = 0; c < l.out_c; c++) {
                  a_avg[w + l.out_w*(h + l.out_h*b)] += l.output[w + l.out_w*(h
  + l.out_h*(c + l.out_c*b))];
              }
  ...
  for (b = 0; b < l.batch; ++b)
  {
      for (w = 0; w < l.out_w; w++) {
          for (h = 0; h < l.out_h; h++) {
              for (c = 0; c < l.out_c; c++)
              {
                  // a = a + alpha(t) + e(c,i,j) = a + alpha(t) + g(i,j) *
  avg_a(i,j) / channels l.output[w + l.out_w*(h + l.out_h*(c + l.out_c*b))] +=
                      alpha *
                      g[w + l.out_w*(h + l.out_h*b)] *
                      a_avg[w + l.out_w*(h + l.out_h*b)];

                  //l.output[w + l.out_w*(h + l.out_h*(c + l.out_c*b))] =
                  //    alpha * g[w + l.out_w*(h + l.out_h*b)] * a_avg[w +
  l.out_w*(h + l.out_h*b)];
              }
          }
      }
  ...
  */
  ...
  if (0)  // visualize ground truth
  {
    ...
    for (b = 0; b < l.batch; ++b) {
      ...
      image img2 = float_to_image_scaled(
          l.out_w, l.out_h, 1, &l.output[l.out_w * l.out_h * l.out_c * b]);
      ...
      int c = l.out_c;
      if (c > 4) c = 4;
      image img3 = float_to_image(l.out_w, l.out_h, c,
      &l.output[l.out_w*l.out_h*l.out_c*b]); image dc =
      collapse_image_layers(img3, 1); char buff3[BUFF_SIZE]; sprintf(buff3,
      "a_excitation_act_collapsed_%d", b); show_image_cv(dc, buff3);
      */
...
void update_convolutional_layer_gpu(layer l, int batch,
                                    float learning_rate_init, float momentum,
  ...
  */
  ...
  if (l.deform) {
    ...
    if (l.rotate)
      ...
      stretch_weights_gpu(l.weight_updates_gpu, l.weight_deform_gpu, l.nweights,
                          l.n, l.size, 0, 1);
    ...
    reduce_and_expand_array_gpu(l.weight_deform_gpu, l.weight_updates_gpu,
                                l.nweights, 4);
  ...
  if (l.grad_centr && l.batch_normalize) {
    ...
    gradient_centralization_gpu(l.size, l.size, l.c / l.groups, l.n,
                                l.weight_updates_gpu);
  }

  if (l.adam) {
    // adam_update_gpu(l.weights_gpu, l.weight_updates_gpu, l.m_gpu, l.v_gpu,
    // a.B1, a.B2, a.eps, decay, learning_rate, l.nweights, batch, a.t);
    adam_update_gpu(l.weights_gpu, l.weight_updates_gpu, l.m_gpu, l.v_gpu, l.B1,
                    l.B2, l.eps, decay, learning_rate, l.nweights, batch, l.t);

    adam_update_gpu(l.biases_gpu, l.bias_updates_gpu, l.bias_m_gpu,
                    l.bias_v_gpu, l.B1, l.B2, l.eps, decay, learning_rate, l.n,
                    batch, l.t);
    if (l.scales_gpu) {
      adam_update_gpu(l.scales_gpu, l.scale_updates_gpu, l.scale_m_gpu,
                      l.scale_v_gpu, l.B1, l.B2, l.eps, decay, learning_rate,
                      l.n, batch, l.t);
    }
  } else {
    // axpy_ongpu(l.nweights, -decay*batch, l.weights_gpu, 1,
    // l.weight_updates_gpu, 1); axpy_ongpu(l.nweights, learning_rate / batch,
    // l.weight_updates_gpu, 1, l.weights_gpu, 1); scal_ongpu(l.nweights,
    ...
    if (l.reverse) {
      ...
      float divider = 1.0;
      float abs_add = 1.0;
      mult_inverse_array_gpu(l.weight_updates_gpu, l.output_gpu,
                             l.inputs * l.batch, l.reverse, divider, clip,
                             abs_add);
      l.weight_updates_gpu = l.output_gpu;
    }

    axpy_ongpu(l.nweights, -decay * batch * loss_scale, l.weights_gpu, 1,
    ...
    axpy_ongpu(l.n, learning_rate / batch, l.bias_updates_gpu, 1, l.biases_gpu,
               1);
  ...
  if (l.deform) {
    ...
    if (l.rotate)
      rotate_weights_gpu(l.weight_deform_gpu, l.weights_gpu, l.nweights, l.n,
                         l.size, 0);
    else if (l.sway)
      sway_and_flip_weights_gpu(l.weight_deform_gpu, l.weights_gpu, l.nweights,
                                l.n, l.size, l.angle, 0);
    else if (l.stretch)
      stretch_weights_gpu(l.weight_deform_gpu, l.weights_gpu, l.nweights, l.n,
                          l.size, 0, 0);
    else if (l.stretch_sway)
