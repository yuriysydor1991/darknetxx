...
#include "src/structs/network_state.h"
#include "utils.h"
...
int64_t get_current_iteration(network net) { return *net.cur_iteration; }
...
float get_current_rate(network net)
{
  ...
  switch (net.policy) {
    ...
    case POLY:
      ...
      // if (batch_num < net.burn_in) return net.learning_rate *
      // pow((float)batch_num / net.burn_in, net.power); return
      // net.learning_rate * pow(1 - (float)batch_num / net.max_batches,
      // net.power);
    case RANDOM:
...
void forward_network(network net, network_state state)
{
  ...
  for (i = 0; i < net.n; ++i) {
    ...
    l.forward(l, state);
    // CLOGT("%d - Predicted in %lf milli-seconds.", i,
...
float train_network_datum(network net, float *x, float *y)
{
  ...
  *net.seen += net.batch;
  ...
  if (*(state.net.total_bbox) > 0)
    CLOGI("total_bbox = %d, rewritten_bbox = %f %%", *(state.net.total_bbox),
...
float train_network_waitkey(network net, data d, int wait_key)
{
  ...
  if (net.ema_alpha && (*net.cur_iteration) >= ema_start_point) {
    // int ema_period =
    //     (net.max_batches - ema_start_point - 1000) * (1.0 - net.ema_alpha);
    int ema_apply_point = net.max_batches - 1000;

    if (!is_ema_initialized(net)) {
      ema_update(net, 0);  // init EMA
      CLOGIM(" EMA initialization");
    }

    if ((*net.cur_iteration) == ema_apply_point) {
      ema_apply(
          net);  // apply EMA (BN rolling mean/var recalculation is required)
      CLOGIM(" ema_apply()");
    } else if ((*net.cur_iteration) <
               ema_apply_point)  // && (*net.cur_iteration) % ema_period == 0)
    {
      ema_update(net, net.ema_alpha);  // update EMA
      CLOGI(" ema_update(), ema_alpha = %f", net.ema_alpha);
    }
  }

  int reject_stop_point = net.max_batches * 3 / 4;

  if ((*net.cur_iteration) < reject_stop_point && net.weights_reject_freq &&
...
#ifdef GPU
  if (gpu_index >= 0) {
    CLOGI("try to allocate additional workspace_size = %1.2f MB",
          (float)workspace_size / 1000000);
    net->workspace = cuda_make_array(0, workspace_size / sizeof(float) + 1);
    CLOGIM(" CUDA allocate done!");
  } else {
    free(net->workspace);
...
int resize_network(network *net, int w, int h)
{
  ...
  int inputs = 0;
  size_t workspace_size = 0;
  CLOGI("Resizing to %d x %d...", w, h);
  // fflush(stderr);
  for (i = 0; i < net->n; ++i) {
    layer l = net->layers[i];
    CLOGT(" (resize %d: layer = %s)", i, layer_type_2_string(l.type));
    if (l.type == CONVOLUTIONAL) {
      ...
      CLOGE("Cannot resize this type of layer %d", layer_type_2_string(l.type));
...
int get_network_input_size(network net) { return net.layers[0].inputs; }
...
detection_layer get_network_detection_layer(network net)
{
  ...
  CLOGEM("Detection layer not found!!!");
...
layer *get_network_layer(network *net, int i) { return net->layers + i; }
...
void visualize_network(network net)
{
  ...
  for (int i = 0; i < net.n; ++i) {
    ...
    snprintf(&buff, PATH_MAX, "Layer %d", i);
    layer l = net.layers[i];
    if (l.type == CONVOLUTIONAL) {
...
detection *make_network_boxes(network *net, float thresh, int *num)
{
  ...
  for (i = 0; i < net->n; ++i) {
    ...
    if (l_tmp.type == YOLO || l_tmp.type == GAUSSIAN_YOLO ||
        l_tmp.type == DETECTION || l_tmp.type == REGION) {
      l = l_tmp;
      break;
    }
  }
...
void custom_get_region_detections(layer l, int w, int h, int net_w, int net_h,
                                  float thresh, int *map, float hier,
                                  int relative, detection *dets, int letter)
{
  ...
  correct_yolo_boxes(dets, l.w * l.h * l.n, w, h, net_w, net_h, relative,
                     letter);
}
                        ...
                        int *map, int relative, detection *dets, int letter)
{
  ...
  for (j = 0; j < net->n; ++j) {
    ...
    if (l.type == GAUSSIAN_YOLO) {
      int count = get_gaussian_yolo_detections(l, w, h, net->w, net->h, thresh,
                                               map, relative, dets, letter);
      dets += count;
    }
    if (l.type == REGION) {
      custom_get_region_detections(l, w, h, net->w, net->h, thresh, map, hier,
                                   relative, dets, letter);
...
void fill_network_boxes_batch(network *net, int w, int h, float thresh,
                              ...
                              detection *dets, int letter, int batch)
{
  ...
  for (j = 0; j < net->n; ++j) {
    layer l = net->layers[j];
    if (l.type == YOLO) {
      int count = get_yolo_detections_batch(l, w, h, net->w, net->h, thresh,
                                            map, relative, dets, letter, batch);
      dets += count;
      if (prev_classes < 0) prev_classes = l.classes;
      else if (prev_classes != l.classes) {
        CLOGE(
  ...
  for (i = 0; i < nboxes; ++i) {
    for (j = 0; j < classes; ++j) {
      ...
      if (dets[i].prob[j] > thresh && show) {
        ...
        if (!buf) return 0;
        // sprintf(buf, "{\"image_id\":%d, \"category_id\":%d, \"bbox\":[%f, %f,
        // %f, %f], \"score\":%f}",
        //     image_id, j, dets[i].bbox.x, dets[i].bbox.y, dets[i].bbox.w,
...
det_num_pair *network_predict_batch(network *net, image im, int batch_size,
                                    int w, int h, float thresh, float hier,
                                    int *map, int relative, int letter)
{
  ...
  for (batch = 0; batch < batch_size; batch++) {
    ...
    fill_network_boxes_batch(net, w, h, thresh, hier, map, relative, dets,
                             letter, batch);
    pdets[batch].num = num;
...
void print_network(network net)
{
  ...
  for (i = 0; i < net.n; ++i) {
    ...
    CLOGI("Layer %d - Mean: %f, Variance: %f", i, mean, vari);
    if (n > 100) n = 100;
    for (j = 0; j < n; ++j) CLOGI("%f, ", output[j]);
    if (n == 100) CLOGIM(".....");
  }
}
...
void free_network_ptr(network *net) { free_network(*net); }
...
#endif
  free(net.names);
  free(net.backup_directory);
  free(net.train_images);
  free(net.valid_images);
  free(net.lastweights);
}
...
void fuse_conv_batchnorm(network net)
{
  ...
  for (j = 0; j < net.n; ++j) {
    ...
    if (l->type == CONVOLUTIONAL) {
      CLOGD(" Merges Convolutional-%d and batch_norm", j);
      ...
      if (l->batch_normalize) {
        ...
        for (f = 0; f < l->n; ++f) {
          ...
          double precomputed =
              l->scales[f] / (sqrt((double)l->rolling_variance[f] + .00001));
      ...
      if (l->nweights > 0) {
        ...
        int i;
        for (i = 0; i < l->nweights; ++i) CLOGI(" w = %f,", l->weights[i]);
        CLOGI(" l->nweights = %d, j = %d", l->nweights, j);
      }
      ...
      CLOGT(" Fusion skip layer type: %d", l->type);
    }
  }
}
...
void calculate_binary_weights(network net)
{
  ...
  for (j = 0; j < net.n; ++j) {
    ...
    if (l->type == CONVOLUTIONAL) {
      CLOGT(" Merges Convolutional-%d and batch_norm", j);
  ...
  CLOGTM(" calculate_binary_weights Done!");
}
...
void copy_weights_net(network net_train, network *net_map)
{
  ...
  for (k = 0; k < net_train.n; ++k) {
    ...
    if (l->type == CRNN) {
      ...
      copy_cudnn_descriptors(*net_map->layers[k].output_layer,
                             &tmp_output_layer);
...
network combine_train_valid_networks(network net_train, network net_map)
{
  ...
  for (k = 0; k < net_train.n; ++k) {
    ...
    if (l->type == CONVOLUTIONAL) {
      ...
      net_combined.layers[k].normDstTensorDesc =
          net_map.layers[k].normDstTensorDesc;
      net_combined.layers[k].normDstTensorDescF16 =
          net_map.layers[k].normDstTensorDescF16;
      ...
      net_combined.layers[k].srcTensorDesc16 =
          net_map.layers[k].srcTensorDesc16;
      net_combined.layers[k].dstTensorDesc16 =
          net_map.layers[k].dstTensorDesc16;
#endif  // CUDNN
    }
  }
  return net_combined;
  ...
  for (i = 0; i < net.n; ++i) {
    ...
    if (l.type == CONVOLUTIONAL) {
      ...
      if (l.weights_ema) {
        for (k = 0; k < l.nweights; ++k) {
          l.weights_ema[k] =
              ema_alpha * l.weights_ema[k] + (1 - ema_alpha) * l.weights[k];
        }
      }
      ...
      for (k = 0; k < l.n; ++k) {
        if (l.biases_ema)
          l.biases_ema[k] =
              ema_alpha * l.biases_ema[k] + (1 - ema_alpha) * l.biases[k];
        if (l.scales_ema)
          l.scales_ema[k] =
              ema_alpha * l.scales_ema[k] + (1 - ema_alpha) * l.scales[k];
      }
    }
  }
}
...
void reject_similar_weights(network net, float sim_threshold)
{
  ...
  for (i = 0; i < net.n; ++i) {
    ...
    if (i == 0) continue;
    if (net.n > i + 1)
      if (net.layers[i + 1].type == YOLO) continue;
    if (net.n > i + 2)
      if (net.layers[i + 2].type == YOLO) continue;
    if (net.n > i + 3)
      if (net.layers[i + 3].type == YOLO) continue;
    ...
    if (l.type == CONVOLUTIONAL && l.activation != LINEAR) {
      ...
      for (k = 0; k < l.n; ++k) {
        for (j = k + 1; j < l.n; ++j) {
          ...
          float sim =
              cosine_similarity(&l.weights[filter_size * w1],
                                &l.weights[filter_size * w2], filter_size);
          if (sim > max_sim) {
            max_sim = sim;
            max_sim_index = w1;
            max_sim_index2 = w2;
      ...
      CLOGE(
          "reject_similar_weights: i = %d, l.n = %d, w1 = %d, w2 = %d, sim = "
          "%f, thresh = %f \n",
      ...
      if (max_sim > sim_threshold) {
        ...
        for (k = 0; k < filter_size; ++k) {
          l.weights[max_sim_index * filter_size + k] =
