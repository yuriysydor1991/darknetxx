...
void draw_train_loss(char *windows_name, mat_cv *img_src, int img_size,
                     ...
                     char *accuracy_name, float contr_acc, int dont_show,
                     int mjpeg_port, double time_remaining);
void draw_train_loss_ex(char *windows_name, mat_cv *img_src, int img_size,
                        float avg_loss, float max_img_loss, int current_batch,
                        int max_batches, float precision, int draw_precision,
                        char *accuracy_name, float contr_acc, int dont_show,
                        int mjpeg_port, double time_remaining,
                        int save_every_chart);

// Data augmentation
image image_data_augmentation(mat_cv *mat, int w, int h, int pleft, int ptop,
                              int swidth, int sheight, int flip, float dhue,
                              float dsat, float dexp, int gaussian_noise,
...
void cv_draw_object(image sized, float *truth_cpu, int max_boxes, int num_truth,
                    int *it_num_set, float *lr_set, int *boxonly, int classes,
                    char **names);
