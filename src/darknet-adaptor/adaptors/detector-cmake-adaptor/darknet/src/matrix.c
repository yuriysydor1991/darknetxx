...
void matrix_to_csv(matrix m)
{
  ...
  for (i = 0; i < m.rows; ++i) {
    for (j = 0; j < m.cols; ++j) {
      CLOGI(" m.vals[%d][%d] = %.17g", i, j, m.vals[i][j]);
    }
  }
}
...
{
  int i, j;
  CLOGI("%d X %d Matrix:", m.rows, m.cols);

  for (i = 0; i < m.rows; ++i) {
    for (j = 0; j < m.cols; ++j) {
      CLOGI(" m.vals[%d][%d] = %15.7f ", i, j, m.vals[i][j]);
    }
  }
}
...
float dist(float *x, float *y, int n)
{
  CLOGD(" x0 = %f, x1 = %f, y0 = %f, y1 = %f", x[0], x[1], y[0], y[1]);
...
model do_kmeans(matrix data, int k)
{
  ...
  /*
  // IoU = 63.29%, anchors = 10,13,  16,30,  33,23,  30,61,  62,45,  59,119,
  116,90,  156,198,  373,326 centers.vals[0][0] = 10; centers.vals[0][1] = 13;
  ...
  */
  ...
  int i;
  for (i = 0; i < 1000 && !kmeans_expectation(data, assignments, centers);
       ++i) {
    kmeans_maximization(data, assignments, centers);
  }

  CLOGI(" iterations = %d", i);
