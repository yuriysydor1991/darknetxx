                               ...
                               const float max_delta)
{
  ...
  if (cpc) {
    ...
    CLOGIM(" classes_multipliers: ");
    for (i = 0; i < classes_counters; ++i)
      CLOGI("classes_multipliers[%d] = %.1f", i, classes_multipliers[i]);
  }
...
layer parse_yolo(list *options, size_params params)
{
  ...
  CLOGD("Making yolor layer: %d %d %d %d %d %d %d %d", params.batch, params.w,
        params.h, num, total, mask, classes, max_boxes);
  ...
  if (strcmp(iou_loss, "mse") == 0) l.iou_loss = MSE;
    ...
    l.iou_loss = IOU;
  CLOGI(
      "[yolo] params: iou loss: %s (%d), iou_norm: %2.2f, obj_norm: %2.2f, "
      "cls_norm: %2.2f, delta_norm: %2.2f, scale_x_y: %2.2f",
      iou_loss, l.iou_loss, l.iou_normalizer, l.obj_normalizer,
      l.cls_normalizer, l.delta_normalizer, l.scale_x_y);
  ...
  if (strcmp(iou_thresh_kind_str, "iou") == 0) l.iou_thresh_kind = IOU;
    ...
    CLOGW(" Wrong iou_thresh_kind = %s", iou_thresh_kind_str);
  ...
  if (strcmp(nms_kind, "default") == 0) l.nms_kind = DEFAULT_NMS;
    ...
    CLOGI("nms_kind: %s (%d), beta = %f", nms_kind, l.nms_kind, l.beta_nms);
  }
  ...
  if (embedding_layer_id != 999999) {
    CLOGI(" embedding_layer_id = %d", embedding_layer_id);
    ...
    CLOGI(" embedding_size = %d", l.embedding_size);
    if (le.n % l.n != 0) {
      CLOGW(
          "Warning: filters=%d number in embedding_layer=%d isn't divisable "
          "by number of anchors %d",
...
layer parse_gaussian_yolo(list *options, size_params params)  // Gaussian_YOLOv3
{
  ...
  if (strcmp(iou_thresh_kind_str, "iou") == 0) l.iou_thresh_kind = IOU;
    ...
    CLOGE("Wrong iou_thresh_kind = %s", iou_thresh_kind_str);
    l.iou_thresh_kind = IOU;
  }
  ...
  if (strcmp(nms_kind, "default") == 0) l.nms_kind = DEFAULT_NMS;
    ...
    if (strcmp(nms_kind, "greedynms") == 0) l.nms_kind = GREEDY_NMS;
      ...
      l.nms_kind = DEFAULT_NMS;
    CLOGE("nms_kind: %s (%d), beta = %f", nms_kind, l.nms_kind, l.beta_nms);
  }
  ...
  CLOGI(
      "[Gaussian_yolo] iou loss: %s (%d), iou_norm: %2.2f, obj_norm: %2.2f, "
      "cls_norm: %2.2f, delta_norm: %2.2f, scale: %2.2f, point: %d",
      iou_loss, l.iou_loss, l.iou_normalizer, l.obj_normalizer,
      l.cls_normalizer, l.delta_normalizer, l.scale_x_y, l.yolo_point);
...
layer parse_shortcut(list *options, size_params params, network net)
{
  ...
  if (strcmp(weights_type_str, "per_feature") == 0 ||
    ...
    CLOGE(
        "Error: Incorrect weights_type = %s Use one of: none, per_feature, "
        "per_channel",
        weights_type_str);
    error("Error!", DARKNET_LOC);
  }
  ...
  if (strcmp(weights_normalization_str, "relu") == 0 ||
    ...
    CLOGE(
        "Error: Incorrect weights_normalization = %s Use one of: none, "
        "relu, softmax",
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (params.w != net.layers[index].out_w ||
      ...
      CLOGI(" (%4d x%4d x%4d) + (%4d x%4d x%4d)", params.w, params.h, params.c,
            net.layers[index].out_w, net.layers[index].out_h,
...
layer parse_scale_channels(list *options, size_params params, network net)
{
  ...
  if (activation == SWISH || activation == MISH) {
    CLOGWM(" [scale_channels] layer doesn't support SWISH or MISH activations");
  }
  return s;
}
...
layer parse_sam(list *options, size_params params, network net)
{
  ...
  s.activation = activation;
  if (activation == SWISH || activation == MISH) {
    CLOGWM(" [sam] layer doesn't support SWISH or MISH activations");
  }
  return s;
}
...
route_layer parse_route(list *options, size_params params)
{
  ...
  for (i = 1; i < n; ++i) {
    ...
    if (next.out_w == first.out_w && next.out_h == first.out_h) {
      ...
      CLOGWM(" The width and height of the input layers are different.");
  ...
  if (layer.groups > 1) CLOGI("%d/%d", layer.group_id, layer.groups);

  CLOGI(" -> %4d x%4d x%4d", layer.out_w, layer.out_h, layer.out_c);
...
learning_rate_policy get_policy(char *s)
{
  ...
  CLOGW("Couldn't find policy %s, going with constant", s);
  return CONSTANT;
}
  ...
  *net->cuda_graph_ready = 0;
  ...
  if (net->gpu_index >= 0) {
    ...
    if (compute_capability >= 700) net->cudnn_half = 1;
      ...
      net->cudnn_half = 0;
#endif  // CUDNN_HALF
    CLOGI(" %d : compute_capability = %d, cudnn_half = %d, GPU: %s",
          net->gpu_index, compute_capability, net->cudnn_half, device_name);
  } else
    CLOGI(" GPU isn't used");
#endif  // GPU
  if (net->policy == STEP) {
  ...
  CLOGI("images names file: %s", net->names);
  CLOGI("train images file: %s", net->train_images);
  CLOGI("validate images file: %s", net->valid_images);
  CLOGI("backup directory: %s", net->backup_directory);
...
network parse_network_cfg_custom(char *filename, int batch, int time_steps)
{
  ...
  if (sections == NULL) {
    CLOGE("Failure to read cfg sections: %s", filename);
    return net;
  }
  ...
  if (!n) {
    CLOGEM("Config file has no sections");
  ...
  if (!is_network(s)) {
    CLOGEM("First section must be [net] or [network]");
...
#ifdef GPU
  CLOGI("net.optimized_memory = %d", net.optimized_memory);
  ...
  params.net = net;
  CLOGI("mini_batch = %d, batch = %d, time_steps = %d, train = %d", net.batch,
        net.batch * net.subdivisions, net.time_steps, params.train);
  ...
  if (params.train == 1) {
    while (n_tmp) {
      ...
      if (stopbackward == 1) {
        ...
        CLOGI("last_stop_backward = %d", last_stop_backward);
      }
      n_tmp = n_tmp->next;
      ++count_tmp;
    }
  ...
  CLOGIM("   layer   filters  size/strd(dil)      input                output");
  while (n) {
    ...
    if (lt == CONVOLUTIONAL) {
      ...
      CLOGE("Type not recognized: %s", s->type);
    }
    ...
    if (show_receptive_field) {
      ...
      CLOGI("%4d - receptive field: %d x %d", count, cur_receptive_w,
            cur_receptive_h);
    }
    ...
    if (l.stopbackward == 1)
      CLOGIM(" ------- previous layers are frozen -------");
  ...
  CLOGI("Total BFLOPS %5.3f", bflops);
  CLOGI("avg_outputs = %d", avg_outputs);
#ifdef GPU
  get_cuda_stream();
  ...
  if (gpu_index >= 0) {
    ...
    *net.max_output16_size = 0;
    ...
    if (workspace_size) {
      CLOGI(" Allocate additional workspace_size = %1.2f MB",
...
list *read_cfg(char *filename)
{
  ...
  if (filename == NULL) {
    CLOGEM("No valid options file provided");
    return NULL;
  }

  CLOGT("Trying to read the file: %s", filename);

  FILE *file = fopen(filename, "r");
  if (file == NULL) {
    CLOGE("Failure to open options file: %s", filename);
    file_error(__FILE__, __LINE__, filename);
  }
  ...
  while ((line = fgetl(file)) != 0) {
    ...
    switch (line[0]) {
      ...
      default:
        if (current == NULL) {
          CLOGEM("No cfg file section declared!");
          free(sections);
          fclose(file);
          return NULL;
        }
...
void save_shortcut_weights(layer l, FILE *fp)
{
  ...
  for (i = 0; i < l.nweights; ++i)
    CLOGI(" l.weights[%d] = %f, ", i, l.weights[i]);
  CLOGI(" l.nweights = %d", l.nweights);
...
#endif
  CLOGI("Saving weights to %s", filename);
  FILE *fp = fopen(filename, "wb");
  if (!fp) file_error(__FILE__, __LINE__, filename);
...
void load_convolutional_weights(layer l, FILE *fp)
{
  ...
  if (read_bytes > 0 && read_bytes < l.n)
    CLOGW("Warning: Unexpected end of wights-file! l.biases - l.index = %d",
          l.index);
  // fread(l.weights, sizeof(float), num, fp); // as in connected layer
  if (l.batch_normalize && (!l.dontloadscales)) {
    read_bytes = fread(l.scales, sizeof(float), l.n, fp);
    if (read_bytes > 0 && read_bytes < l.n)
      CLOGW("Warning: Unexpected end of wights-file! l.scales - l.index = %d",
            l.index);
    ...
    if (0) {
      int i;
      for (i = 0; i < l.n; ++i) {
        CLOGT("l.rolling_mean[%d] = %g, ", i, l.rolling_mean[i]);
      }

      for (i = 0; i < l.n; ++i) {
        CLOGT("l.rolling_variance[%d]= %g, ", i, l.rolling_variance[i]);
      }
    }
    if (0) {
...
void load_weights_upto(network *net, char *filename, int cutoff)
{
...
#endif
  CLOGI("Loading weights from %s...", filename);
  ...
  *net->cur_iteration = get_current_batch(*net);
  CLOGI(" trained: %.0f K-images (%.0f Kilo-batches_64)",
  ...
  CLOGI("Done! Loaded %d layers from weights-file", i);
}
...
{
  CLOGI(" Try to load cfg: %s, weights: %s, clear = %d", cfg, weights, clear);

  network *net = (network *)xcalloc(1, sizeof(network));
  *net = parse_network_cfg_custom(cfg, batch, 1);
  if (weights && weights[0] != 0) {
    CLOGI(" Try to load weights: %s", weights);
...
{
  if (cfg == NULL) {
    CLOGEM("Invalid config file pointer provided");
    return NULL;
  }

  if (strlen(cfg) == 0) {
    CLOGEM("Invalid config file path string provided");
    return NULL;
  }

  if (weights == NULL) {
    CLOGEM("Invalid weights file pointer provided");
    return NULL;
  }

  if (strlen(weights) == 0) {
    CLOGEM("Invalid weights file path string provided");
    return NULL;
  }

  CLOGI("Try to load cfg: %s, clear = %d", cfg, clear);
