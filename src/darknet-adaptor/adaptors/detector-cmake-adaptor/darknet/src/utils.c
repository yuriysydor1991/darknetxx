...
void find_replace_extension(char *str, char *orig, char *rep, char *output)
{
  ...
  if (!p ||
      chars_from_end != strlen(orig)) {  // Is 'orig' even in 'str' AND is
...
{
  find_replace(input_path, "/images/train2017/", "/labels/train2017/",
               output_path);  // COCO
  find_replace(output_path, "/images/val2017/", "/labels/val2017/",
               output_path);  // COCO
  find_replace(output_path, "/JPEGImages/", "/labels/",
               output_path);  // PascalVOC
  find_replace(output_path, "\\images\\train2017\\", "\\labels\\train2017\\",
               output_path);  // COCO
  find_replace(output_path, "\\images\\val2017\\", "\\labels\\val2017\\",
               output_path);  // COCO
  ...
  find_replace(output_path, "\\images\\val2014\\", "\\labels\\val2014\\",
               output_path);  // COCO
  find_replace(output_path, "/images/train2014/", "/labels/train2014/",
               output_path);  // COCO
  find_replace(output_path, "/images/val2014/", "/labels/val2014/",
               output_path);  // COCO
  ...
  find_replace_extension(output_path, ".TIFF", ".txt", output_path);
  find_replace_extension(output_path, ".webp", ".txt", output_path);
...
void file_error(const char *const source, const unsigned int line,
                const char *const s)
{
  fprintf(stderr, "Couldn't open file: %s from %s:%d\n", s, source, line);
  log_backtrace();
  exit(EXIT_FAILURE);
}
...
char *fgetl(FILE *fp)
{
  ...
  char *line = (char *)xcalloc(size + 1, sizeof(char));
  ...
  CLOGT("Read the line: %s", line);

  size_t curr = strlen(line);

  while ((line[curr - 1] != '\n') && !feof(fp)) {
    CLOGTM("Rereading file line");
    if (curr == size - 1) {
      size *= 2;
  ...
  CLOGT("returning cleaned line: %s", line);
...
void print_statistics(float *a, int n)
{
  ...
  CLOGI("MSE: %.6f, Mean: %.6f, Variance: %.6f\n", mse_array(a, n), m, v);
}

void print_statistics_w(const char *prefix, float *a, int n)
{
  float m = mean_array(a, n);
  float v = variance_array(a, n);

  CLOGI("%s MSE: %.6f, Mean: %.6f, Variance: %.6f\n", prefix, mse_array(a, n),
        m, v);
}
...
boxabs box_to_boxabs(const box *b, const int img_w, const int img_h,
...
char *relocate_copy_str(const char *const orig)
{
  const int tmpsize = strlen(orig);
  const char *tmp = (char *)xcalloc(tmpsize + 1, sizeof(char));

  memset(tmp, 0, tmpsize + 1);

  strncpy(tmp, orig, tmpsize);
