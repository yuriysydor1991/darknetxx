...
float delta_gaussian_yolo_box(box truth, float *x, float *biases, int n,
                              ...
                              float uc_normalizer, int accumulate,
                              YOLO_POINT yolo_point, float max_delta)
{
  box pred = get_gaussian_yolo_box(x, biases, n, index, i, j, lw, lh, w, h,
                                   stride, yolo_point);
  ...
  float delta_ux = temp_x * (in_exp_x_2 / x[index + 1 * stride] -
                             1. / (x[index + 1 * stride] + sigma_const));
  float delta_uy = temp_y * (in_exp_y_2 / x[index + 3 * stride] -
                             1. / (x[index + 3 * stride] + sigma_const));
  float delta_uw = temp_w * (in_exp_w_2 / x[index + 5 * stride] -
                             1. / (x[index + 5 * stride] + sigma_const));
  float delta_uh = temp_h * (in_exp_h_2 / x[index + 7 * stride] -
...
void averages_gaussian_yolo_deltas(int class_index, int box_index, int stride,
...
void delta_gaussian_yolo_class(float *output, float *delta, int index,
                               int class_id, int classes, int stride,
                               float *avg_cat, float label_smooth_eps,
                               float *classes_multipliers, float cls_normalizer)
{
  ...
  if (delta[index]) {
    ...
    delta[index + stride * class_id] =
        y_true - output[index + stride * class_id];
...
int compare_gaussian_yolo_class(float *output, int classes, int class_index,
...
static int entry_gaussian_index(layer l, int batch, int location, int entry)
{
  ...
  return batch * l.outputs + n * l.w * l.h * (8 + l.classes + 1) +
         entry * l.w * l.h + loc;
}
...
void forward_gaussian_yolo_layer(const layer l, network_state state)
{
  ...
  for (b = 0; b < l.batch; ++b) {
    for (n = 0; n < l.n; ++n) {
      ...
      scal_add_cpu(l.w * l.h, l.scale_x_y, -0.5 * (l.scale_x_y - 1),
                   l.output + index, 1);  // scale x
      // y : mu, sigma
      index = entry_gaussian_index(l, b, n * l.w * l.h, 2);
      activate_array(l.output + index, 2 * l.w * l.h, LOGISTIC);
      scal_add_cpu(l.w * l.h, l.scale_x_y, -0.5 * (l.scale_x_y - 1),
                   l.output + index, 1);  // scale y
      // w : sigma
  ...
  *(l.cost) = 0;
  for (b = 0; b < l.batch; ++b) {
    for (j = 0; j < l.h; ++j) {
      for (i = 0; i < l.w; ++i) {
        for (n = 0; n < l.n; ++n) {
          ...
          const int obj_index =
              entry_gaussian_index(l, b, n * l.w * l.h + j * l.w + i, 8);
          const int box_index =
              entry_gaussian_index(l, b, n * l.w * l.h + j * l.w + i, 0);
          const int stride = l.w * l.h;
          box pred = get_gaussian_yolo_box(
              l.output, l.biases, l.mask[n], box_index, i, j, l.w, l.h,
          ...
          for (t = 0; t < l.max_boxes; ++t) {
            ...
            int class_id_match = compare_gaussian_yolo_class(
                l.output, l.classes, class_index, l.w * l.h, objectness,
                class_id, 0.25f);
          ...
          if (best_match_iou > l.ignore_thresh) {
            const float iou_multiplier =
                best_match_iou *
                best_match_iou;  // (best_match_iou - l.ignore_thresh) / (1.0 -
                                 // l.ignore_thresh);
            if (l.objectness_smooth) {
              ...
              int class_id =
                  state.truth[best_match_t * l.truth_size + b * l.truths + 4];
              if (l.map) class_id = l.map[class_id];
              delta_gaussian_yolo_class(
                  l.output, l.delta, class_index, class_id, l.classes,
                  l.w * l.h, 0, l.label_smooth_eps, l.classes_multipliers,
                  l.cls_normalizer);
            } else
              l.delta[obj_index] = 0;
          ...
          if (best_iou > l.truth_thresh) {
            ...
            if (l.map) class_id = l.map[class_id];
            delta_gaussian_yolo_class(
                l.output, l.delta, class_index, class_id, l.classes, l.w * l.h,
                0, l.label_smooth_eps, l.classes_multipliers, l.cls_normalizer);
            ...
            delta_gaussian_yolo_box(
                truth, l.output, l.biases, l.mask[n], box_index, i, j, l.w, l.h,
    ...
    for (t = 0; t < l.max_boxes; ++t) {
      ...
      if (l.yolo_point == YOLO_CENTER) {
        ...
        i = min_val_cmp(l.w - 1,
                        max_val_cmp(0, ((truth.x - truth.w / 2) * l.w)));
        j = min_val_cmp(l.h - 1,
                        max_val_cmp(0, ((truth.y - truth.h / 2) * l.h)));
      ...
      if (mask_n >= 0) {
        ...
        float iou = delta_gaussian_yolo_box(
        ...
        int obj_index =
            entry_gaussian_index(l, b, mask_n * l.w * l.h + j * l.w + i, 8);
        avg_obj += l.output[obj_index];
        l.delta[obj_index] =
            class_multiplier * l.obj_normalizer * (1 - l.output[obj_index]);

        int class_index =
            entry_gaussian_index(l, b, mask_n * l.w * l.h + j * l.w + i, 9);
    ...
    for (j = 0; j < l.h; ++j) {
      for (i = 0; i < l.w; ++i) {
        for (n = 0; n < l.n; ++n) {
          ...
          averages_gaussian_yolo_deltas(class_index, box_index, stride,
                                        l.classes, l.delta);
        }
      }
    }
  ...
  float class_loss =
      pow(mag_array(classification_lost, l.outputs * l.batch), 2);
  free(classification_lost);

  float *except_uncertainty_lost =
      (float *)calloc(l.batch * l.outputs, sizeof(float));
  ...
  float except_uc_loss =
      pow(mag_array(except_uncertainty_lost, l.outputs * l.batch), 2);
  ...
  *(l.cost) = pow(mag_array(l.delta, l.outputs * l.batch), 2);
  ...
  CLOGI(
      ...
      recall75 / count, count, class_loss, iou_loss, uc_loss, loss);
}
  ...
  for (i = 0; i < l.w * l.h; ++i) {
    ...
    for (n = 0; n < l.n; ++n) {
      ...
      if (objectness > thresh) {
        ...
        dets[count].bbox = get_gaussian_yolo_box(
            predictions, l.biases, l.mask[n], box_index, col, row, l.w, l.h,
            netw, neth, l.w * l.h, l.yolo_point);
        dets[count].objectness = objectness;
        dets[count].classes = l.classes;

        dets[count].uc[0] = predictions[entry_gaussian_index(
            l, 0, n * l.w * l.h + i, 1)];  // tx uncertainty
        dets[count].uc[1] = predictions[entry_gaussian_index(
            l, 0, n * l.w * l.h + i, 3)];  // ty uncertainty
        dets[count].uc[2] = predictions[entry_gaussian_index(
            l, 0, n * l.w * l.h + i, 5)];  // tw uncertainty
        dets[count].uc[3] = predictions[entry_gaussian_index(
        ...
        for (j = 0; j < l.classes; ++j) {
          int class_index =
              entry_gaussian_index(l, 0, n * l.w * l.h + i, 9 + j);
...
void forward_gaussian_yolo_layer_gpu(const layer l, network_state state)
{
  ...
  for (b = 0; b < l.batch; ++b) {
    for (n = 0; n < l.n; ++n) {
      ...
      scal_add_ongpu(l.w * l.h, l.scale_x_y, -0.5 * (l.scale_x_y - 1),
                     l.output_gpu + index, 1);  // scale x
      // y : mu, sigma
      index = entry_gaussian_index(l, b, n * l.w * l.h, 2);
      activate_array_ongpu(l.output_gpu + index, 2 * l.w * l.h, LOGISTIC);
      scal_add_ongpu(l.w * l.h, l.scale_x_y, -0.5 * (l.scale_x_y - 1),
                     l.output_gpu + index, 1);  // scale y
      // w : sigma
      index = entry_gaussian_index(l, b, n * l.w * l.h, 5);
...
{
  axpy_ongpu(l.batch * l.inputs, l.delta_normalizer, l.delta_gpu, 1,
             state.delta, 1);
}
#endif
