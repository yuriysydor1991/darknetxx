...
void free_layer(layer l) { free_layer_custom(l, 0); }

void free_layer_custom(layer l, int keep_cudnn_desc)
{
  CLOGD("Freeing layer type: %d = %s", l.type, layer_type_2_string(l.type));

  // don't free shared layers
  if (l.share_layer != NULL) {
    CLOGDM("Skipping a shared layer");
    return;
  }
  ...
  if (l.delta_gpu &&
      (l.optimized_memory < 1 || l.keep_delta_gpu && l.optimized_memory < 3))
...
char *layer_type_2_string(const LAYER_TYPE type)
{
  ...
  CLOGE("Unknown layer type given: %d", type);
