...
void resize_yolo_layer(layer *l, int w, int h)
{
  ...
  if (l->embedding_output)
    l->embedding_output =
        (float *)xrealloc(l->output, l->batch * l->embedding_size * l->n *
                                         l->h * l->w * sizeof(float));
  if (l->labels)
    l->labels =
        (int *)xrealloc(l->labels, l->batch * l->n * l->h * l->w * sizeof(int));
  if (l->class_ids)
    l->class_ids = (int *)xrealloc(l->class_ids,
                                   l->batch * l->n * l->h * l->w * sizeof(int));
                 ...
                 int lw, int lh, int w, int h, int stride, int new_coords)
{
  ...
  // previous conv-layer w = ln(t.w * net.w / anchors_w); // w - output of
  // previous conv-layer h = ln(t.h * net.h / anchors_h); // h - output of
  // previous conv-layer
  if (new_coords) {
    b.x = (i + x[index + 0 * stride]) / lw;
    b.y = (j + x[index + 1 * stride]) / lh;
    b.w = x[index + 2 * stride] * x[index + 2 * stride] * 4 * biases[2 * n] / w;
    b.h = x[index + 3 * stride] * x[index + 3 * stride] * 4 *
          biases[2 * n + 1] / h;
  } else {
    b.x = (i + x[index + 0 * stride]) / lw;
    b.y = (j + x[index + 1 * stride]) / lh;
...
ious delta_yolo_box(box truth, float *x, float *biases, int n, int index, int i,
                    int j, int lw, int lh, int w, int h, float *delta,
                    float scale, int stride, float iou_normalizer,
                    IOU_LOSS iou_loss, int accumulate, float max_delta,
                    int *rewritten_bbox, int new_coords)
{
  ...
  box pred =
      get_yolo_box(x, biases, n, index, i, j, lw, lh, w, h, stride, new_coords);
  ...
  if (iou_loss == MSE)  // old loss
  {
    ...
    if (new_coords) {
      ...
      // float grad_w = 8 * exp(-x[index + 2 * stride]) / pow(exp(-x[index + 2 *
      // stride]) + 1, 3); float grad_h = 8 * exp(-x[index + 3 * stride]) /
      // pow(exp(-x[index + 3 * stride]) + 1, 3); dw *= grad_w; dh *= grad_h;
    } else {
  ...
  if (focal_loss) {
    ...
    for (n = 0; n < classes; ++n) {
      ...
      if (classes_multipliers && n == class_id)
        delta[index + stride * class_id] *=
            classes_multipliers[class_id] * cls_normalizer;
      if (n == class_id && avg_cat) *avg_cat += output[index + stride * n];
    }
  }
}
...
{
  {
      ...
      for (i = 0; i < l.w; ++i) {
        for (n = 0; n < l.n; ++n) {
          ...
          for (t = 0; t < l.max_boxes; ++t) {
            ...
            int class_id_match =
                compare_yolo_class(l.output, l.classes, class_index, l.w * l.h,
          ...
          if (best_match_iou > l.ignore_thresh) {
            if (l.objectness_smooth) {
              const float delta_obj =
                  l.obj_normalizer * (best_match_iou - l.output[obj_index]);
            ...
            l.delta[obj_index] =
                scale * l.obj_normalizer * (0 - l.output[obj_index]);
            int cl_id;
            int found_object = 0;
            for (cl_id = 0; cl_id < l.classes; ++cl_id) {
              if (l.output[class_index + stride * cl_id] * l.output[obj_index] >
                  0.25) {
            ...
            if (found_object) {
              ...
              l.delta[box_index + 0 * stride] +=
                  scale * (0 - l.output[box_index + 0 * stride]);
              l.delta[box_index + 1 * stride] +=
                  scale * (0 - l.output[box_index + 1 * stride]);
              l.delta[box_index + 2 * stride] +=
                  scale * (0 - l.output[box_index + 2 * stride]);
              l.delta[box_index + 3 * stride] +=
                  scale * (0 - l.output[box_index + 3 * stride]);
            }
          }
          if (best_iou > l.truth_thresh) {
            const float iou_multiplier =
                best_iou * best_iou;  // (best_iou - l.truth_thresh) / (1.0 -
                                      // l.truth_thresh);
            if (l.objectness_smooth)
              l.delta[obj_index] =
                  l.obj_normalizer * (iou_multiplier - l.output[obj_index]);
            else
              l.delta[obj_index] = l.obj_normalizer * (1 - l.output[obj_index]);
            // l.delta[obj_index] = l.obj_normalizer * (1 -
            // l.output[obj_index]);

            int class_id =
                state.truth[best_t * l.truth_size + b * l.truths + 4];
            if (l.map) class_id = l.map[class_id];
            delta_yolo_class(l.output, l.delta, class_index, class_id,
                             l.classes, l.w * l.h, 0, l.focal_loss,
            ...
            box truth = float_to_box_stride(
                state.truth + best_t * l.truth_size + b * l.truths, 1);
            delta_yolo_box(truth, l.output, l.biases, l.mask[n], box_index, i,
                           j, l.w, l.h, state.net.w, state.net.h, l.delta,
                           (2 - truth.w * truth.h), l.w * l.h,
                           l.iou_normalizer * class_multiplier, l.iou_loss, 1,
                           l.max_delta, state.net.rewritten_bbox, l.new_coords);
    ...
    for (t = 0; t < l.max_boxes; ++t) {
      box truth =
          float_to_box_stride(state.truth + t * l.truth_size + b * l.truths, 1);
      if (!truth.x) break;  // continue;
      ...
      if (mask_n >= 0) {
        ...
        ious all_ious = delta_yolo_box(
            truth, l.output, l.biases, best_n, box_index, i, j, l.w, l.h,
        ...
        const int truth_out_index =
            b * l.n * l.w * l.h + mask_n * l.w * l.h + j * l.w + i;
        l.labels[truth_out_index] = track_id;
        l.class_ids[truth_out_index] = class_id;
        // printf(" track_id = %d, t = %d, b = %d, truth_in_index = %d,
        // truth_out_index = %d \n", track_id, t, b, truth_in_index,
        // truth_out_index);
        ...
        if (l.objectness_smooth) {
          float delta_obj =
              class_multiplier * l.obj_normalizer * (1 - l.output[obj_index]);
        ...
        delta_yolo_class(l.output, l.delta, class_index, class_id, l.classes,
                         ...
                         l.classes_multipliers, l.cls_normalizer);

        // printf(" label: class_id = %d, truth.x = %f, truth.y = %f, truth.w =
        // %f, truth.h = %f \n", class_id, truth.x, truth.y, truth.w, truth.h);
        // printf(" mask_n = %d, l.output[obj_index] = %f, l.output[class_index
        // + class_id] = %f \n\n", mask_n, l.output[obj_index],
        // l.output[class_index + class_id]);
      ...
      for (n = 0; n < l.total; ++n) {
        ...
        if (mask_n >= 0 && n != best_n && l.iou_thresh < 1.0f) {
          ...
          if (iou > l.iou_thresh) {
            ...
            avg_obj += l.output[obj_index];
            if (l.objectness_smooth) {
              float delta_obj = class_multiplier * l.obj_normalizer *
                                (1 - l.output[obj_index]);
              if (l.delta[obj_index] == 0) l.delta[obj_index] = delta_obj;
            } else
              l.delta[obj_index] = class_multiplier * l.obj_normalizer *
                                   (1 - l.output[obj_index]);
    ...
    if (l.iou_thresh < 1.0f) {
      // averages the deltas obtained by the function:
      // delta_yolo_box()_accumulate
      for (j = 0; j < l.h; ++j) {
        for (i = 0; i < l.w; ++i) {
          for (n = 0; n < l.n; ++n) {
...
void forward_yolo_layer(const layer l, network_state state)
{
  ...
  *(l.cost) = 0;
  ...
  struct train_yolo_args *yolo_args =
      (train_yolo_args *)xcalloc(l.batch, sizeof(struct train_yolo_args));
  ...
  // Search for an equidistant point from the distant boundaries of the local
  // minimum
  ...
  if ((state.net.badlabels_rejection_percentage &&
       start_point < iteration_num) ||
      (state.net.num_sigmas_reject_badlabels && start_point < iteration_num) ||
      (state.net.equidistant_point &&
       state.net.equidistant_point < iteration_num)) {
    const float progress_it = iteration_num - state.net.equidistant_point;
    const float progress =
        progress_it / (state.net.max_batches - state.net.equidistant_point);
    ...
    for (i = 0; i < l.batch * l.outputs; ++i) {
      if (l.delta[i] != 0) {
        ...
        if (cur_max < fabs(l.delta[i])) cur_max = fabs(l.delta[i]);
      }
    }
    ...
    *state.net.delta_rolling_max =
        *state.net.delta_rolling_max * 0.99 + cur_max * 0.01;
    *state.net.delta_rolling_avg =
        *state.net.delta_rolling_avg * 0.99 + cur_avg * 0.01;
    ...
    if (state.net.num_sigmas_reject_badlabels && start_point < iteration_num) {
      ...
      const float progress_badlabels =
          (float)(iteration_num - start_point) / (start_point);
      ...
      *state.net.delta_rolling_std =
          *state.net.delta_rolling_std * 0.99 + cur_std * 0.01;

      float final_badlebels_threshold =
          rolling_avg + rolling_std * state.net.num_sigmas_reject_badlabels;
      float badlabels_threshold =
          rolling_max -
          progress_badlabels * fabs(rolling_max - final_badlebels_threshold);
      badlabels_threshold =
          max_val_cmp(final_badlebels_threshold, badlabels_threshold);
      for (i = 0; i < l.batch * l.outputs; ++i) {
        if (fabs(l.delta[i]) > badlabels_threshold) l.delta[i] = 0;
      }
      CLOGI(" rolling_std = %f, rolling_max = %f, rolling_avg = %f",
            rolling_std, rolling_max, rolling_avg);
      CLOGI(" badlabels loss_threshold = %f, start_it = %d, progress = %f",
            badlabels_threshold, start_point, progress_badlabels * 100);

      ep_loss_threshold =
          min_val_cmp(final_badlebels_threshold, rolling_avg) * progress;
    }
    ...
    if (state.net.badlabels_rejection_percentage &&
      ...
      CLOGI(" badlabels_reject_threshold = %f",
      ...
      float cur_percent =
          100 * (counter_reject * num_deltas_per_anchor / counter_all);
      ...
      CLOGI(
          " badlabels_reject_threshold = %f, cur_percent = %f, "
          "badlabels_rejection_percentage = %f, delta_rolling_max = %f",
          *state.net.badlabels_reject_threshold, cur_percent,
          state.net.badlabels_rejection_percentage,
          *state.net.delta_rolling_max);
    }

    // reject low loss to find equidistant point
    if (state.net.equidistant_point &&
        state.net.equidistant_point < iteration_num) {
      CLOGI(
          " equidistant_point loss_threshold = %f, start_it = %d, progress = "
          "%3.1f %%",
          ep_loss_threshold, state.net.equidistant_point, progress * 100);
      for (i = 0; i < l.batch * l.outputs; ++i) {
        if (fabs(l.delta[i]) < ep_loss_threshold) l.delta[i] = 0;
      }
    }
  }
  ...
  if (l.show_details == 0) {
    ...
    *(l.cost) = loss;
    ...
    const char *const ioulossstr =
        (l.iou_loss == MSE ? "mse" : (l.iou_loss == GIOU ? "giou" : "iou"));
    CLOGI(
        "v3 (%s loss, Normalizer: (iou: %.2f, obj: %.2f, cls: %.2f) Region "
        "%d Avg (IOU: %f), count: %d, total_loss = %f",
        ioulossstr, l.iou_normalizer, l.obj_normalizer, l.cls_normalizer,
        state.index, tot_iou / count, count, loss);
  } else {
    ...
    *(l.cost) = loss;
    ...
    if (l.iou_loss == MSE) {
      *(l.cost) = pow(mag_array(l.delta, l.outputs * l.batch), 2);
      ...
      if (l.iou_loss == GIOU) {
        ...
        avg_iou_loss =
            count > 0 ? l.iou_normalizer * (tot_iou_loss / count) : 0;
      }
    ...
    CLOGI(
        "v3 (%s loss, Normalizer: (iou: %.2f, obj: %.2f, cls: %.2f) Region "
        "%d Avg (IOU: %f), count: %d, class_loss = %f, iou_loss = %f, "
        "total_loss = %f",
        ioulossstr, l.iou_normalizer, l.obj_normalizer, l.cls_normalizer,
        state.index, tot_iou / count, count, classification_loss, iou_loss,
        loss);

    // fprintf(stderr, "v3 (%s loss, Normalizer: (iou: %.2f, cls: %.2f) Region
    // %d Avg (IOU: %f, GIOU: %f), Class: %f, Obj: %f, No Obj: %f, .5R: %f,
    // .75R: %f, count: %d, class_loss = %f, iou_loss = %f, total_loss = %f \n",
    //     (l.iou_loss == MSE ? "mse" : (l.iou_loss == GIOU ? "giou" : "iou")),
    //     l.iou_normalizer, l.obj_normalizer, state.index, tot_iou / count,
    //     tot_giou / count, avg_cat / class_count, avg_obj / count, avg_anyobj
    //     / (l.w*l.h*l.n*l.batch), recall / count, recall75 / count, count,
  ...
  for (i = 0; i < l.w * l.h; ++i) {
    ...
    for (n = 0; n < l.n; ++n) {
      ...
      if (objectness > thresh) {
        ...
        if (l.embedding_output) {
          get_embedding(l.embedding_output, l.w, l.h, l.n * l.embedding_size,
                        l.embedding_size, col, row, n, 0,
...
int get_yolo_detections_batch(layer l, int w, int h, int netw, int neth,
                              float thresh, int *map, int relative,
  ...
  for (i = 0; i < l.w * l.h; ++i) {
    ...
    for (n = 0; n < l.n; ++n) {
      ...
      if (objectness > thresh) {
        ...
        if (l.embedding_output) {
          get_embedding(l.embedding_output, l.w, l.h, l.n * l.embedding_size,
                        l.embedding_size, col, row, n, batch,
                        dets[count].embeddings);
        }
...
void forward_yolo_layer_gpu(const layer l, network_state state)
{
  if (l.embedding_output) {
    ...
    cuda_pull_array_async(le.output_gpu, l.embedding_output,
                          le.batch * le.outputs);
  }
  ...
  for (b = 0; b < l.batch; ++b) {
    for (n = 0; n < l.n; ++n) {
      ...
      // if(y->0) x -> -inf
      if (l.new_coords) {
        // activate_array_ongpu(l.output_gpu + bbox_index, 4 * l.w*l.h,
        // LOGISTIC);    // x,y,w,h
      } else {
        activate_array_ongpu(l.output_gpu + bbox_index, 2 * l.w * l.h,
                             LOGISTIC);  // x,y

        int obj_index = entry_index(l, b, n * l.w * l.h, 4);
        activate_array_ongpu(l.output_gpu + obj_index,
                             (1 + l.classes) * l.w * l.h,
                             LOGISTIC);  // classes and objectness
      }
      if (l.scale_x_y != 1)
        scal_add_ongpu(2 * l.w * l.h, l.scale_x_y, -0.5 * (l.scale_x_y - 1),
                       l.output_gpu + bbox_index, 1);  // scale x,y
    }
  }
  if (!state.train || l.onlyforward) {
    // cuda_pull_array(l.output_gpu, l.output, l.batch*l.outputs);
    if (l.mean_alpha && l.output_avg_gpu)
      mean_array_gpu(l.output_gpu, l.batch * l.outputs, l.mean_alpha,
                     l.output_avg_gpu);
