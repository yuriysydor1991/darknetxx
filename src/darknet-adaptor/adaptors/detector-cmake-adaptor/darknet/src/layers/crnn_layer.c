...
layer make_crnn_layer(int batch, int h, int w, int c, int hidden_filters,
                      int output_filters, int groups, int steps, int size,
                      int stride, int dilation, int pad, ACTIVATION activation,
                      int batch_normalize, int xnor, int train)
{
  CLOGI("CRNN Layer: %d x %d x %d image, %d filters", h, w, c, output_filters);
  ...
  *(l.input_layer) = make_convolutional_layer(
      batch, steps, h, w, c, hidden_filters, groups, size, stride, stride,
      dilation, pad, activation, batch_normalize, 0, xnor, 0, 0, 0, 0, NULL, 0,
      0, train);
  ...
  *(l.self_layer) = make_convolutional_layer(
      batch, steps, h, w, hidden_filters, hidden_filters, groups, size, stride,
  ...
  *(l.output_layer) = make_convolutional_layer(
      batch, steps, h, w, hidden_filters, output_filters, groups, size, stride,
      stride, dilation, pad, activation, batch_normalize, 0, xnor, 0, 0, 0, 0,
      NULL, 0, 0, train);
  ...
  l.bflops =
      l.input_layer->bflops + l.self_layer->bflops + l.output_layer->bflops;
...
#ifdef GPU
  cuda_push_array(l.self_layer->output_gpu, l.self_layer->output,
                  l.outputs * l.batch);
#endif  // GPU
}
                       ...
                       float decay)
{
  ...
  update_convolutional_layer(*(l.self_layer), batch, learning_rate, momentum,
                             decay);
  update_convolutional_layer(*(l.output_layer), batch, learning_rate, momentum,
                             decay);
}
...
void forward_crnn_layer_gpu(layer l, network_state state)
{
  ...
  input_layer.c*input_layer.n*input_layer.size*input_layer.size,
  input_layer.weights_gpu16); cuda_convert_f32_to_f16(self_layer.weights_gpu,
  self_layer.c*self_layer.n*self_layer.size*self_layer.size,
  self_layer.weights_gpu16); cuda_convert_f32_to_f16(output_layer.weights_gpu,
  output_layer.c*output_layer.n*output_layer.size*output_layer.size,
  output_layer.weights_gpu16);
      }
  #endif  //CUDNN_HALF
  */
  ...
  for (i = 0; i < l.steps; ++i) {
    ...
    axpy_ongpu(l.hidden * l.batch, 1, input_layer.output_gpu, 1, l.state_gpu,
               1);
...
void backward_crnn_layer_gpu(layer l, network_state state)
{
  ...
  for (i = l.steps - 1; i >= 0; --i) {
    ...
    // 1);   // commented in RNN axpy_ongpu(l.hidden * l.batch, 1,
    // self_layer.output_gpu, 1, l.state_gpu, 1); // commented in RNN
    ...
    if (state.net.try_fix_nan) {
      fix_nan_and_inf(output_layer.delta_gpu,
                      output_layer.inputs * output_layer.batch);
      fix_nan_and_inf(self_layer.delta_gpu,
                      self_layer.inputs * self_layer.batch);
      fix_nan_and_inf(input_layer.delta_gpu,
                      input_layer.inputs * input_layer.batch);
    }
