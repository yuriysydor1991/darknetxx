...
{
  CLOGI("Batch Normalization Layer: %d x %d x %d image", w, h, c);
...
void update_batchnorm_layer(layer l, int batch, float learning_rate,
...
void forward_batchnorm_layer_gpu(layer l, network_state state)
{
  ...
  if (state.train) {
    ...
    if (l.batch_normalize == 2) {
      ...
      CLOGT("minibatch_index = %d, max_minibatch_index = %d", minibatch_index,
            max_minibatch_index);
      ...
      fast_v_cbn_gpu(l.output_gpu, l.mean_gpu, l.batch, l.out_c,
                     l.out_h * l.out_w, minibatch_index, max_minibatch_index,
                     l.m_cbn_avg_gpu, l.v_cbn_avg_gpu, l.variance_gpu, alpha,
                     l.rolling_mean_gpu, l.rolling_variance_gpu,
                     inverse_variance, .00001);

      normalize_scale_bias_gpu(l.output_gpu, l.mean_gpu, l.variance_gpu,
                               l.scales_gpu, l.biases_gpu, l.batch, l.out_c,
      ...
      CLOGT("CBN, minibatch_index = %d", minibatch_index);
    } else {
#ifdef CUDNN
      float one = 1;
      float zero = 0;
...
{
  if (state.net.adversarial) {
    inverse_variance_ongpu(l.out_c, l.rolling_variance_gpu, l.variance_gpu,
                           0.00001);

    scale_bias_gpu(l.delta_gpu, l.variance_gpu, l.batch, l.out_c,
                   l.out_h * l.out_w);
                                ...
                                float momentum, float decay, float loss_scale)
{
  ...
  axpy_ongpu(l.c, learning_rate / batch, l.bias_updates_gpu, 1, l.biases_gpu,
             1);
  scal_ongpu(l.c, momentum, l.bias_updates_gpu, 1);

  axpy_ongpu(l.c, learning_rate / batch, l.scale_updates_gpu, 1, l.scales_gpu,
             1);
