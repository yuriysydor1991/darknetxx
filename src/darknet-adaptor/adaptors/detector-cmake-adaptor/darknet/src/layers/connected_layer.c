...
connected_layer make_connected_layer(int batch, int steps, int inputs,
                                     ...
                                     int batch_normalize)
{
  ...
  cudnn_convolutional_setup(&l, cudnn_fastest,
                            0);  // cudnn_fastest, cudnn_smallest
  l.workspace_size = get_connected_workspace_size(l);
#endif  // CUDNN
#endif  // GPU
  CLOGI("connected                            %4d  ->  %4d\n", inputs, outputs);
  return l;
}
...
void backward_connected_layer(connected_layer l, network_state state)
{
  ...
  if (l.batch_normalize) {
    ...
    variance_delta_cpu(l.x, l.delta, l.mean, l.variance, l.batch, l.outputs, 1,
                       l.variance_delta);
    normalize_delta_cpu(l.x, l.mean, l.variance, l.mean_delta, l.variance_delta,
                        l.batch, l.outputs, 1, l.delta);
  }
...
{
  if (l.batch_normalize) {
    print_statistics_w("Scales ", l.scales, l.outputs);
    /*
    print_statistics_w("Rolling Mean ", l.rolling_mean, l.outputs);
    print_statistics_w("Rolling Variance ", l.rolling_variance, l.outputs);
    */
  }
  print_statistics_w("Biases ", l.biases, l.outputs);
  print_statistics_w("Weights ", l.weights, l.outputs);
}
                                ...
                                float decay, float loss_scale)
{
  ...
  axpy_ongpu(l.inputs * l.outputs, learning_rate / batch, l.weight_updates_gpu,
             1, l.weights_gpu, 1);
...
void backward_connected_layer_gpu(connected_layer l, network_state state)
{
  ...
  gradient_array_ongpu(l.output_gpu, l.outputs * l.batch, l.activation,
                       l.delta_gpu);
  for (i = 0; i < l.batch; ++i) {
    axpy_ongpu(l.outputs, 1, l.delta_gpu + i * l.outputs, 1, l.bias_updates_gpu,
               1);
  }
