...
void delta_region_class(float *output, float *delta, int index, int class_id,
                        int classes, tree *hier, float scale, float *avg_cat,
                        int focal_loss)
{
  int i, n;
  if (hier) {
...
void forward_region_layer(const region_layer l, network_state state)
{
  ...
  if (l.softmax_tree) {
    for (b = 0; b < l.batch; ++b) {
      for (i = 0; i < l.h * l.w * l.n; ++i) {
        int index = size * i + b * l.outputs;
        softmax_tree(l.output + index + 5, 1, 0, 1, l.softmax_tree,
                     l.output + index + 5);
      }
    }
  } else if (l.softmax) {
  ...
  *(l.cost) = 0;
  for (b = 0; b < l.batch; ++b) {
        ...
        if (truth.x > 100000 && truth.y > 100000) {
          for (n = 0; n < l.n * l.w * l.h; ++n) {
            ...
            float p = scale * get_hierarchy_probability(
                                  l.output + index, l.softmax_tree, class_id);
            if (p > maxp) {
              maxp = p;
              maxi = n;
            }
          ...
          delta_region_class(l.output, l.delta, index, class_id, l.classes,
                             l.softmax_tree, l.class_scale, &avg_cat,
                             l.focal_loss);
          ++class_count;
          onlyclass_id = 1;
          break;
    ...
    for (j = 0; j < l.h; ++j) {
      for (i = 0; i < l.w; ++i) {
        for (n = 0; n < l.n; ++n) {
          ...
          for (t = 0; t < l.max_boxes; ++t) {
            box truth =
                float_to_box(state.truth + t * l.truth_size + b * l.truths);
            int class_id = state.truth[t * l.truth_size + b * l.truths + 4];
          ...
          if (l.classfix == -1)
            l.delta[index + 4] =
                l.noobject_scale * ((best_iou - l.output[index + 4]) *
                                    logistic_gradient(l.output[index + 4]));
          else {
            if (best_iou > l.thresh) {
              ...
              if (l.classfix > 0) {
                delta_region_class(
                    l.output, l.delta, index + 5, best_class_id, l.classes,
                    l.softmax_tree,
          ...
          if (*(state.net.seen) < 12800) {
            ...
            delta_region_box(truth, l.output, l.biases, n, index, i, j, l.w,
                             l.h, l.delta, .01);
          }
        }
      }
    ...
    for (t = 0; t < l.max_boxes; ++t) {
      ...
      i = (truth.x * l.w);
      j = (truth.y * l.h);
      CLOGT("%d %f %d %f", i, truth.x * l.w, j, truth.y * l.h);
      ...
      for (n = 0; n < l.n; ++n) {
        ...
        CLOGT("pred: (%f, %f) %f x %f", pred.x, pred.y, pred.w, pred.h);
        pred.x = 0;
        pred.y = 0;
      ...
      CLOGT("%d %f (%f, %f) %f x %f", best_n, best_iou, truth.x, truth.y,
            truth.w, truth.h);

      float iou =
          delta_region_box(truth, l.output, l.biases, best_n, best_index, i, j,
                           l.w, l.h, l.delta, l.coord_scale);
      if (iou > .5) recall += 1;
      avg_iou += iou;
...
void forward_region_layer_gpu(const region_layer l, network_state state)
{
   ...
   */
  flatten_ongpu(state.input, l.h * l.w, l.n * (l.coords + l.classes + 1),
                l.batch, 1, l.output_gpu);
  if (l.softmax_tree) {
    int i;
    int count = 5;
    for (i = 0; i < l.softmax_tree->groups; ++i) {
      int group_size = l.softmax_tree->group_size[i];
      softmax_gpu(l.output_gpu + count, group_size, l.classes + 5,
                  l.w * l.h * l.n * l.batch, 1, l.output_gpu + count);
      count += group_size;
    }
  } else if (l.softmax) {
    softmax_gpu(l.output_gpu + 5, l.classes, l.classes + 5,
                l.w * l.h * l.n * l.batch, 1, l.output_gpu + 5);
  }
...
{
  flatten_ongpu(l.delta_gpu, l.h * l.w, l.n * (l.coords + l.classes + 1),
                l.batch, 0, state.delta);
}
#endif
