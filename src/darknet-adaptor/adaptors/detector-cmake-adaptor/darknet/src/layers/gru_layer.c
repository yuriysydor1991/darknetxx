...
layer make_gru_layer(int batch, int inputs, int outputs, int steps,
                     int batch_normalize)
{
  CLOGI("GRU Layer: %d inputs, %d outputs", inputs, outputs);
...
void backward_gru_layer(__attribute__((unused)) layer l,
                        __attribute__((unused)) network_state state)
{
}
...
void update_gru_layer_gpu(layer l, int batch, float learning_rate,
                          float momentum, float decay, float loss_scale)
{
  update_connected_layer_gpu(*(l.input_r_layer), batch, learning_rate, momentum,
                             decay, loss_scale);
  update_connected_layer_gpu(*(l.input_z_layer), batch, learning_rate, momentum,
                             decay, loss_scale);
  update_connected_layer_gpu(*(l.input_h_layer), batch, learning_rate, momentum,
                             decay, loss_scale);
  update_connected_layer_gpu(*(l.state_r_layer), batch, learning_rate, momentum,
                             decay, loss_scale);
  update_connected_layer_gpu(*(l.state_z_layer), batch, learning_rate, momentum,
                             decay, loss_scale);
  update_connected_layer_gpu(*(l.state_h_layer), batch, learning_rate, momentum,
                             decay, loss_scale);
}
...
void backward_gru_layer_gpu(layer l, network_state state)
{
  ...
  for (i = l.steps - 1; i >= 0; --i) {
    if (i != 0)
      copy_ongpu(l.outputs * l.batch, l.output_gpu - l.outputs * l.batch, 1,
                 l.prev_state_gpu, 1);
    ...
    if (prev_delta_gpu)
      mult_add_into_gpu(l.outputs * l.batch, l.forgot_delta_gpu, l.r_gpu,
                        prev_delta_gpu);
    mult_add_into_gpu(l.outputs * l.batch, l.forgot_delta_gpu, l.prev_state_gpu,
