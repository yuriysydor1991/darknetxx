...
void forward_softmax_layer_gpu(const softmax_layer l, network_state net)
{
  if (l.softmax_tree) {
    ...
    for (i = 0; i < l.softmax_tree->groups; ++i) {
    int group_size = l.softmax_tree->group_size[i];
    softmax_gpu(net.input_gpu + count, group_size, l.batch, l.inputs, 1, 0, 1,
    l.temperature, l.output_gpu + count); count += group_size;
    }
    */
  } else {
    if (l.spatial) {
  ...
  if (net.truth && !l.noloss) {
    softmax_x_ent_gpu(l.batch * l.inputs, l.output_gpu, net.truth, l.delta_gpu,
                      l.loss_gpu);
    if (l.softmax_tree) {
      mask_gpu_new_api(l.batch * l.inputs, l.delta_gpu, SECRET_NUM, net.truth,
                       0);
      mask_gpu_new_api(l.batch * l.inputs, l.loss_gpu, SECRET_NUM, net.truth,
                       0);
    }
...
{
  axpy_ongpu(layer.batch * layer.inputs, state.net.loss_scale, layer.delta_gpu,
             1, state.delta, 1);
}
...
contrastive_layer make_contrastive_layer(int batch, int w, int h, int c,
                                         int classes, int inputs,
                                         layer *yolo_layer)
{
  ...
  if (yolo_layer) {
    ...
    if (l.embedding_size != yolo_layer->embedding_size) {
      CLOGE(
          " Error: [contrastive] embedding_size=%d isn't equal to [yolo] "
          "embedding_size=%d. They should use the same [convolutional] layer",
          l.embedding_size, yolo_layer->embedding_size);
      error("Error!", DARKNET_LOC);
    }
    if (l.inputs % (l.n * l.h * l.w) != 0) {
      CLOGW(
          " Warning: filters= number in the previous (embedding) layer isn't "
          "divisable by number of anchors %d",
          l.n);
    }
  } else {
    l.detection = 0;
  ...
  // l.contrast_p = (contrastive_params*)xcalloc(*l.contrast_p_size,
  ...
  const int max_contr_size = (l.max_boxes * l.batch) * (l.max_boxes * l.batch) *
                             sizeof(contrastive_params) / 4;
  CLOGI(" max_contr_size = %d MB", max_contr_size / (1024 * 1024));
  l.contrast_p_gpu =
      (contrastive_params *)cuda_make_array(NULL, max_contr_size);
#endif
  CLOGI(
      "contrastive %4d x%4d x%4d x emb_size %4d x batch: %4d  classes = "
      "%4d, step = %4zu",
      w, h, l.n, l.embedding_size, batch, l.classes, step);
  if (l.detection) CLOGIM(" detection");
  return l;
}
...
void forward_contrastive_layer(contrastive_layer l, network_state state)
{
        ...
        for (int w = 0; w < l.w; ++w) {
          ...
          for (int n = 0; n < l.classes; ++n) {
            ...
            if (truth_prob > truth_thresh) {
              // printf(" truth_prob = %f, max_truth = %f, n = %d; ",
              // truth_prob, max_truth, n);
    ...
    for (int n = 0; n < l.n; ++n) {
      for (int h = 0; h < l.h; ++h) {
        for (int w = 0; w < l.w; ++w) {
          ...
          // const int input_index = b*l.inputs + n*l.embedding_size*l.h*l.w +
          // h*l.w + w; float *ptr = state.input + input_index; z[z_index] =
          // ptr;

          z[z_index] = (float *)xcalloc(l.embedding_size, sizeof(float));
          get_embedding(state.input, l.w, l.h, l.c, l.embedding_size, w, h, n,
                        b, z[z_index]);
        }
      }
    }
  ...
  size_t contrast_p_index = 0;

  const size_t gstep = l.batch * l.n * l.h * l.w;
  size_t contrast_p_size = gstep;
  if (!l.detection) contrast_p_size = l.batch * l.batch;
  contrastive_params *contrast_p = (contrastive_params *)xcalloc(
      contrast_p_size, sizeof(contrastive_params));
            ...
            for (int n2 = 0; n2 < l.n; ++n2) {
              for (int h2 = 0; h2 < l.h; ++h2) {
                for (int w2 = 0; w2 < l.w; ++w2) {
                  const int z_index2 =
                      b2 * l.n * l.h * l.w + n2 * l.h * l.w + h2 * l.w + w2;
                  ...
                  const float sim = cosine_similarity(z[z_index], z[z_index2],
                                                      l.embedding_size);
                  ...
                  if ((contrast_p_index + 1) >= contrast_p_size) {
                    ...
                    // printf(" contrast_p_size = %d, z_index = %d, z_index2 =
                    // %d \n", contrast_p_size, z_index, z_index2);
                    contrast_p = (contrastive_params *)xrealloc(
                        contrast_p,
                        contrast_p_size * sizeof(contrastive_params));
                  }
  ...
  if (all_sims > 0) {
    ...
    *l.loss = -1;
  CLOGI(" Contrast accuracy = %f %%, all = %d, good = %d, same = %d, diff = %d",
  ...
  for (b = 0; b < l.batch; b += 2) {
      ...
      //printf(" l.labels[b] = %d, l.labels[b+1] = %d, l.labels[b+2] = %d, b =
  ...
  */
  ...
  for (b = 0; b < l.batch; ++b) {
      ...
      for (b2 = 0; b2 < l.batch; ++b2) {
          if (b != b2) {
              const float P = P_constrastive(b, b2, l.labels, l.batch, z,
  l.embedding_size, l.temperature, l.cos_sim); l.p_constrastive[b*l.batch + b2]
  = P; if (P > 1 || P < -1) { printf(" p = %f, ", P);
              }
          }
      }
  }
  */
  ...
  if (l.detection) {
#ifdef GPU
    const int max_contr_size =
        (l.max_boxes * l.batch) * (l.max_boxes * l.batch);
    if (max_contr_size < contr_size) {
      CLOGE(
          " Error: too large number of bboxes: contr_size = %d > "
          "max_contr_size  = %d",
    ...
    if (contr_size > 2) {
      ...
      P_constrastive_f_det_gpu(labels, l.embedding_size, l.temperature,
                               l.contrast_p_gpu, contr_size);
    ...
    for (int b = 0; b < l.batch; ++b) {
      for (int n = 0; n < l.n; ++n) {
        for (int h = 0; h < l.h; ++h) {
          for (int w = 0; w < l.w; ++w) {
            const int z_index =
                b * l.n * l.h * l.w + n * l.h * l.w + h * l.w + w;
            if (l.labels[z_index] < 0) continue;
              ...
              for (int n2 = 0; n2 < l.n; ++n2) {
                for (int h2 = 0; h2 < l.h; ++h2) {
                  for (int w2 = 0; w2 < l.w; ++w2) {
                    ...
                    float P = -10;
                    if (l.detection) {
                      P = P_constrastive_f(z_index, z_index2, l.labels, z,
                                           l.embedding_size, l.temperature,
                                           contrast_p, contr_size);
                    } else {
                      P = P_constrastive(z_index, z_index2, l.labels, step, z,
                                         l.embedding_size, l.temperature,
                    ...
                    for (q = 0; q < contr_size; ++q)
                      if (contrast_p[q].i == z_index &&
                          contrast_p[q].j == z_index2) {
                        contrast_p[q].P = P;
                        break;
                      }

                    // if (P > 1 || P < -1) {
                    //     printf(" p = %f, z_index = %d, z_index2 = %d ", P,
                    //     z_index, z_index2);
                    // }
                  }
                }
    ...
    for (int nd = 0; nd < l.n; ++nd) {
      for (int hd = 0; hd < l.h; ++hd) {
        for (int wd = 0; wd < l.w; ++wd) {
          const int z_index =
              bd * l.n * l.h * l.w + nd * l.h * l.w + hd * l.w + wd;
          const size_t step = l.batch * l.n * l.h * l.w;
          if (l.labels[z_index] < 0) continue;

          const int delta_index = bd * l.embedding_size * l.n * l.h * l.w +
                                  nd * l.embedding_size * l.h * l.w + hd * l.w +
                                  wd;
          ...
          if (l.detection) {
            ...
            grad_contrastive_loss_positive_f(
                z_index, l.class_ids, l.labels, step, z, l.embedding_size,
            ...
            grad_contrastive_loss_positive(
                z_index, l.labels, step, z, l.embedding_size, l.temperature,
  ...
  *(l.cost) = pow(mag_array(l.delta, l.inputs * l.batch), 2);
  if (state.net.adversarial) {
    CLOGI(" adversarial contrastive loss = %f", *(l.cost));
  } else {
    CLOGI(" contrastive loss = %f", *(l.cost));
  }
...
void backward_contrastive_layer_gpu(contrastive_layer layer,
                                    network_state state)
{
  axpy_ongpu(layer.batch * layer.inputs, state.net.loss_scale, layer.delta_gpu,
             1, state.delta, 1);
