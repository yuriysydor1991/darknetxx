...
#define POPCNT64(x) \
  (popcnt((unsigned)(x)) + popcnt((unsigned)((uint64_t)(x) >> 32)))
#else
#include <intrin.h>
#ifdef _WIN64
...
static inline uint32_t xnor_int32(uint32_t a, uint32_t b) { return ~(a ^ b); }

static inline uint64_t xnor_int64(uint64_t a, uint64_t b) { return ~(a ^ b); }
    ...
    float *C, int ldc, float *mean_arr)
{
    ...
    for (i = 0; i < M; ++i) {   // l.n - filters [16 - 55 - 1024]
        for (k = 0; k < K; ++k) {   // l.size*l.size*l.c - one filter size [27 -
9216] char a_bit = get_bit(A, i*lda + k);

            for (j = 0; j < N; ++j) { // out_h*out_w - one channel output size
[169 - 173056] char b_bit = get_bit(B, k*ldb + j); count_arr[i*ldc + j] +=
xnor(a_bit, b_bit);
            }
        }
    }
    ...
    float *C, int ldc, float *mean_arr)
{
    ...
    int i, j, k;
    for (i = 0; i < M; ++i) {   // l.n - filters [16 - 55 - 1024]
        for (j = 0; j < N; ++j) { // out_h*out_w - one channel output size [169
- 173056] for (k = 0; k < K; ++k) {   // l.size*l.size*l.c - one filter size [27
- 9216] char a_bit = get_bit(A, i*lda + k); char b_bit = get_bit(B, j*ldb + k);
...
// #pragma omp parallel for
    for (i = 0; i < M; ++i) {   // l.n - filters [16 - 55 - 1024]
        int j, k, h;
        for (k = 0; k < K; ++k) {   // l.size*l.size*l.c - one filter size [27 -
9216] const char a_bit = get_bit(A, i*lda + k); uint64_t a_bit64 =
fill_bit_int64(a_bit); int  k_ldb = k*ldb;

            for (j = 0; j < N; j += 64) { // out_h*out_w - one channel output
size [169 - 173056] if ((N - j > 64) && (k_ldb % 8 == 0)) { uint64_t b_bit64 =
*((uint64_t *)(B + (k_ldb + j) / 8)); uint64_t c_bit64 = xnor_int64(a_bit64,
b_bit64);
                    ...
                    for (; j < N; ++j) { // out_h*out_w - one channel output
size [169 - 173056] char b_bit = get_bit(B, k_ldb + j); if (xnor(a_bit, b_bit))
count_arr[i*ldc + j] += 1;
                    }
                }
...
size [27 - 9216] uint64_t a_bit64 = *((uint64_t *)(A + (i*lda + k) / 8));
...
uint32_t reverse_32_bit(uint32_t a)
{
  ...
  // %1\n" : "=r"(output) : "r"(input));
  return (reverse_8_bit(a >> 24) << 0) | (reverse_8_bit(a >> 16) << 8) |
         (reverse_8_bit(a >> 8) << 16) | (reverse_8_bit(a >> 0) << 24);
}
...
void transpose_32x32_bits_reversed_diagonale(uint32_t *A, uint32_t *B, int m,
                                             int n)
{
  unsigned A_tmp[32];
  int i;
...
void transpose8rS32_reversed_diagonale(unsigned char *A, unsigned char *B,
                                       int m, int n)
{
  unsigned x, y, t;
                   ...
                   const int lda, const int ldb, const int block_size)
{
  ...
  // m, m % 32); printf("\n lda = %d (lda mod 32 = %d), ldb = %d (ldb mod 32 =
  ...
  for (i = 0; i < n; i += 32) {
    ...
    for (j = 0; j < m; j += 32) {
      ...
      // transpose_32x32_bits_my(&A[a_index/32], &B[b_index/32], lda/32,
      // ldb/32);
    }
    for (; j < m; ++j) {
    ...
    (defined(_WIN64) && !defined(__MINGW32__) && !defined(_M_ARM64))
...
static inline float _dn_castu32_f32(uint32_t a) { return *((float *)&a); }
...
//  SIMD: 128-bit
static int HW_SSE, HW_SSE2, HW_SSE3, HW_SSSE3, HW_SSE41, HW_SSE42, HW_SSE4a,
    HW_AES, HW_SHA;
...
void gemm_nn_fast(int M, int N, int K, float ALPHA, float *A, int lda, float *B,
                  int ldb, float *C, int ldc)
{
  int i;
...
void gemm_nn_bin_32bit_packed(int M, int N, int K, float ALPHA, uint32_t *A,
                              int lda, uint32_t *B, int ldb, float *C, int ldc,
                              float *mean_arr)
{
  int i;
  // #pragma omp parallel for
  for (i = 0; i < M; ++i) {  // l.n
    ...
    for (s = 0; s < K; ++s)  // l.size*l.size*l.c/32  or (l.size*l.size*l.c)
    {
      ...
      for (j = 0; j < N - 8; j += 8) {
        ...
        __m256i xnor256 =
            _mm256_andnot_si256(xor256, all_1);  // xnor = not(xor(a,b))

        // waiting for - CPUID Flags: AVX512VPOPCNTDQ: __m512i
        // _mm512_popcnt_epi32(__m512i a)
        __m256 count = _mm256_setr_ps(POPCNT(_mm256_extract_epi32(xnor256, 0)),
...
void convolution_2d_old(int w, int h, int ksize, int n, int c, int pad,
      ...
      for (y = 0; y < h; ++y)
        ...
        for (x = 0; x < w; ++x) {
          ...
          int const weights_pre_index =
              fil * c * ksize * ksize + chan * ksize * ksize;
                    ...
                    float *weights, float *input, float *output, float *mean)
{
  ...
  for (i = 0; i < ksize * ksize * n * c; i += 8) {
    *((__m256 *)&weights[i]) = _mm256_and_ps(*((__m256 *)&weights[i]),
                                             _mm256_castsi256_ps(all256_sing1));
  }

  // for (i = 0; i < w*h*c; i += 8) {
  //(*(__m256*)&input[i]) = _mm256_and_ps(*((__m256*)&input[i]),
  //_mm256_castsi256_ps(all256_sing1));
  //}
      ...
      for (x = 0; x < w - 8; x += 8) {
        ...
        for (chan = 0; chan < c; ++chan) {
          ...
          for (f_y = 0; f_y < ksize; ++f_y) {
            ...
            //__m256 in = _mm256_loadu_ps(&input[input_pre_index + input_y*w + x
            //- pad]);
            ...
            for (f_x = 0; f_x < ksize; ++f_x) {
              ...
              // %f, 7 = %f \n", in.m256_f32[0], in.m256_f32[1], in.m256_f32[2],
              // in.m256_f32[3], in.m256_f32[4], in.m256_f32[5], in.m256_f32[6],
              // in.m256_f32[7]);

              if (f_x < ksize - 1) {
                // in = _mm256_permutevar8x32_ps(in, idx256);
                // in = _mm256_and_ps(in,
                // _mm256_castsi256_ps(all256_last_zero));
              }
            }
          }
        }
        ...
        //     cur_mean, sum256.m256_f32[0], sum256.m256_f32[1],
...
{
  return popcnt128(_mm256_extractf128_si256(n, 0)) +
         popcnt128(_mm256_extractf128_si256(n, 1));
}
...
static inline int popcnt256_custom(__m256i n)
{
  ...
  // val.m256i_i64[2] +
  // val.m256i_i64[3];
  return _mm256_extract_epi64(val, 0) + _mm256_extract_epi64(val, 1) +
         _mm256_extract_epi64(val, 2) + _mm256_extract_epi64(val, 3);
}

static inline void xnor_avx2_popcnt(__m256i a_bit256, __m256i b_bit256,
                                    __m256i *count_sum)
{
  __m256i c_bit256 = _mm256_set1_epi8((char)255);

  __m256i xor256 =
      _mm256_xor_si256(a_bit256, b_bit256);  // xnor = not(xor(a,b))
  c_bit256 = _mm256_andnot_si256(
      xor256, c_bit256);  // can be optimized - we can do other NOT for wegihts
                          // once and do not do this NOT

  *count_sum = _mm256_add_epi64(
      count256(c_bit256), *count_sum);  //  1st part - popcnt Mula's algorithm
}
...
void gemm_nn_custom_bin_mean_transposed(int M, int N, int K, float ALPHA_UNUSED,
                                        unsigned char *A, int lda,
                                        unsigned char *B, int ldb, float *C,
                                        int ldc, float *mean_arr)
{
  ...
  for (i = 0; i < (M / 2) * 2; i += 2) {  // l.n - filters [16 - 55 - 1024]
    ...
    // for (j = 0; j < N; ++j)
    for (j = 0; j < (N / 2) * 2;
         j += 2) {  // out_h*out_w - one channel output size [169 - 173056]
      // int count = 0;
      ...
      for (k = 0; k < K;
           k += bit_step) {  // l.size*l.size*l.c - one filter size [27 - 9216]

        __m256i a_bit256_0 =
            _mm256_loadu_si256((__m256i *)(A + ((i + 0) * lda + k) / 8));
        __m256i b_bit256_0 =
            _mm256_loadu_si256((__m256i *)(B + ((j + 0) * ldb + k) / 8));

        __m256i a_bit256_1 =
            _mm256_loadu_si256((__m256i *)(A + ((i + 1) * lda + k) / 8));
        __m256i b_bit256_1 =
            _mm256_loadu_si256((__m256i *)(B + ((j + 1) * ldb + k) / 8));
      ...
      count_0 =
          count_0 - f1;  // remove extra bits (from empty space for align only)
      count_1 = count_1 - f1;
      count_2 = count_2 - f1;
      count_3 = count_3 - f1;
    ...
    for (i_d = 0; i_d < 2; ++i_d) {
      ...
      for (j = (N / 2) * 2; j < N;
           j += 1) {  // out_h*out_w - one channel output size [169 - 173056]
        const int bit_step = 256;
        __m256i count_sum = _mm256_set1_epi8(0);

        for (k = 0; k < K;
             k +=
             bit_step) {  // l.size*l.size*l.c - one filter size [27 - 9216]
          __m256i a_bit256_0 = _mm256_loadu_si256(
              (__m256i *)(A + ((i + i_d + 0) * lda + k) / 8));
          __m256i b_bit256_0 =
              _mm256_loadu_si256((__m256i *)(B + ((j + 0) * ldb + k) / 8));
...
void im2col_cpu_custom_transpose(float *data_im, int channels, int height,
    ...
    for (c = 0; c < channels_col; ++c) {
      ...
      for (h = pad; h < height_col - pad; ++h) {
        for (w = pad; w < width_col - pad - 4; w += 8) {
          ...
          int col_index =
              (h * width_col + w) * ldb_align + c;  // transposed & aligned

          // data_col[col_index] = data_im[im_col + width*(im_row +
          // height*c_im)];
          __m256 src256 = _mm256_loadu_ps(
              (float *)(&data_im[im_col + width * (im_row + height * c_im)]));
          data_col[col_index + ldb_align * 0] =
              _mm256_extract_float32(src256, 0);  // src256.m256_f32[0];
          data_col[col_index + ldb_align * 1] =
              _mm256_extract_float32(src256, 1);  // src256.m256_f32[1];
          data_col[col_index + ldb_align * 2] =
              _mm256_extract_float32(src256, 2);  // src256.m256_f32[2];
          data_col[col_index + ldb_align * 3] =
              _mm256_extract_float32(src256, 3);  // src256.m256_f32[3];
          data_col[col_index + ldb_align * 4] =
              _mm256_extract_float32(src256, 4);  // src256.m256_f32[4];
          data_col[col_index + ldb_align * 5] =
              _mm256_extract_float32(src256, 5);  // src256.m256_f32[5];
          data_col[col_index + ldb_align * 6] =
              _mm256_extract_float32(src256, 6);  // src256.m256_f32[6];
          data_col[col_index + ldb_align * 7] =
              _mm256_extract_float32(src256, 7);  // src256.m256_f32[7];
        ...
        for (; w < width_col - pad; ++w) {
          ...
          int col_index =
              (h * width_col + w) * ldb_align + c;  // transposed & aligned
          data_col[col_index] =
              data_im[im_col + width * (im_row + height * c_im)];
        }
      }
...
// From Berkeley Vision's Caffe!
// https://github.com/BVLC/caffe/blob/master/LICENSE
void im2col_cpu_custom(float *data_im, int channels, int height, int width,
...
// From Berkeley Vision's Caffe!
// https://github.com/BVLC/caffe/blob/master/LICENSE
void im2col_cpu_custom_align(float *data_im, int channels, int height,
                             int width, int ksize, int stride, int pad,
                             float *data_col, int bit_align)
{
  ...
  if (height_col == height && width_col == width && stride == 1 && pad == 1 &&
    ...
    printf("\n Error: is no non-optimized version \n");
    // im2col_cpu(data_im, channels, height, width, ksize, stride, pad,
    // data_col); // must be aligned for transpose after float_to_bin
...
// From Berkeley Vision's Caffe!
// https://github.com/BVLC/caffe/blob/master/LICENSE
void im2col_cpu_custom_bin(float *data_im, int channels, int height, int width,
                           int ksize, int stride, int pad, float *data_col,
                           int bit_align)
{
  int c;
    ...
    for (c = 0; c < channels_col; ++c) {
...

      {
        ...
        for (h = 0; h < height_col; ++h) {
          ...
          float val = im2col_get_pixel(data_im, height, width, channels, im_row,
                                       im_col, c_im, pad);
          if (val > 0) set_bit((unsigned char *const)data_col, col_index);
        }
      }
  ...
  if (a == LINEAR) {
    ...
    if (is_fma_avx2()) {
      ...
      for (i = 0; i < n - 8; i += 8) {
        ...
        __m256i sign256 =
            _mm256_and_si256(_mm256_castps_si256(src256),
                             all256_sing1);  // check sign in 8 x 32-bit floats

        __m256 result256 = _mm256_blendv_ps(
            src256, mult256,
...
static inline void transpose4x4_SSE(float *A, float *B, const int lda,
                                    const int ldb)
{
...
void forward_maxpool_layer_avx(float *src, float *dst, int *indexes, int size,
          ...
          for (j = 0; j < out_w - 8 - (size - 1); j += 8) {
            ...
            for (n = 0; n < size; ++n) {
              for (m = 0; m < size; ++m) {
                ...
                int valid =
                    (cur_h >= 0 && cur_h < h && cur_w >= 0 && cur_w < w);
                if (!valid) continue;
          ...
          for (j = 0; j < out_w - 4; j += 4) {
            ...
            for (n = 0; n < size; ++n) {
              ...
              m = 0;
              {
                ...
                __m256 src256 = _mm256_loadu_ps(&src[index]);
                __m256 src256_2 =
                    _mm256_permute_ps(src256, (1 << 0) | (3 << 4));
                ...
                __m128 src128 =
                    _mm_shuffle_ps(src128_0, src128_1, (2 << 2) | (2 << 6));
                                        ...
                                        int ldc, float *mean_arr)
{
  ...
  for (i = 0; i < M; ++i) {  // l.n - filters [16 - 55 - 1024]
    ...
    for (j = 0; j < N;
         ++j) {  // out_h*out_w - one channel output size [169 - 173056]
      int count = 0;
...
static inline void transpose_scalar_block(float *A, float *B, const int lda,
...
void transpose_uint32(uint32_t *src, uint32_t *dst, int src_h, int src_w,
...
void convolution_repacked(uint32_t *packed_input, uint32_t *packed_weights,
                          float *output, int w, int h, int c, int n, int size,
                          int pad, int new_lda, float *mean_arr)
{
  int fil;
  // filter index
  // #pragma omp parallel for
        ...
        for (x = 0; x < w; ++x) {
          ...
          for (f_y = 0; f_y < size; ++f_y) {
            ...
            for (f_x = 0; f_x < size; ++f_x) {
              ...
              // normal
              // float input = state.input[(chan + c_pack)*l.w*l.h + input_y*l.w
              // + input_x]; float weight = l.weights[fil*l.c*l.size*l.size +
              // (chan + c_pack)*l.size*l.size + f_y*l.size + f_x];

              // packed
              // float input = re_packed_input[chan*l.w*l.h + (input_y*l.w +
              // input_x) * 32 + c_pack]; float weight =
              // l.weights[fil*l.c*l.size*l.size + chan*l.size*l.size +
              // (f_y*l.size + f_x) * 32 + c_pack]; sum += input * weight;

              // float input = re_packed_input[chan*l.w*l.h + (input_y*l.w +
              // input_x) * 32 + c_pack]; float weight =
              // l.weights[fil*l.c*l.size*l.size + chan*l.size*l.size +
              // (f_y*l.size + f_x) * 32 + c_pack]; uint32_t bit1 = input > 0;
              // uint32_t bit2 = weight > 0;
              // uint32_t count = (~(bit1 ^ bit2)) & 1;
              // float result = (2 * (float)count - 1) * mean_val;
              // printf("\n mul = %f, bit1 = %d, bit2 = %d, count = %d, mean =
              // %f, result = %f  ", input*weight, bit1, bit2, count, mean_val,
              // result); sum += result;

              uint32_t input =
                  ((uint32_t *)
                       packed_input)[chan * w * h + input_y * w + input_x];
              // uint32_t weight = ((uint32_t
              // *)l.align_bit_weights)[fil*l.c*l.size*l.size/32 +
              // chan*l.size*l.size + f_y*l.size + f_x];
              uint32_t weight =
                  ((uint32_t *)
                       packed_weights)[fil * new_lda / 32 + chan * size * size +
                                       f_y * size + f_x];
...
{
  CLOGT("cpu: %d %d %d %d %d %f %d %d %f %d", TA, TB, M, N, K, ALPHA, lda, ldb,
        BETA, ldc);
                ...
                int ldc)
{
  ...
  cudaError_t stream_status =
      (cudaError_t)cublasSetStream(handle, get_cuda_stream());
  CHECK_CUDA(stream_status);
  cudaError_t status = (cudaError_t)cublasSgemm(
      handle, (TB ? CUBLAS_OP_T : CUBLAS_OP_N),
      (TA ? CUBLAS_OP_T : CUBLAS_OP_N), N, M, K, &ALPHA, B_gpu, ldb, A_gpu, lda,
      &BETA, C_gpu, ldc);
  CHECK_CUDA(status);
}
