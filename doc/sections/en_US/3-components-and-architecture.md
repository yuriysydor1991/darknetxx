...
1. [Application runtime components](/doc/sections/en_US/3-1-application-runtime-components.md) - the infrastructure that parses the command line, builds the run context and selects the application to execute (the `app` namespace).
1. [Darknet adaptor components](/doc/sections/en_US/3-2-darknet-adaptor-components.md) - the bridge that drives the original Darknet code to train or to detect (the `darknet_adaptor` namespace).
1. [Supporting components](/doc/sections/en_US/3-3-supporting-components.md) - the OpenMP controller, the logging subsystem and the helper utilities.
