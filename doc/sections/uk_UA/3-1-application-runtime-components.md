...
- **`ApplicationHelpPrinter`** ([`ApplicationHelpPrinter.h`](/src/app/ApplicationHelpPrinter.h)) - друкує довідкове повідомлення, яке представляє усі доступні параметри командного рядка. Дивись секцію [Запуск головного виконуваного файлу](/doc/sections/uk_UA/5-run-the-executable.md#доступні-параметри-командного-рядка) для повного переліку.
...
- **`CommandLineParser`** ([`CommandLineParser.h`](/src/app/CommandLineParser.h)) - розбирає масив `argv` у поля `ApplicationContext` і перевіряє, що параметри, які потребують значення, його отримали. Дивись [Впровадження нових параметрів командного рядка](/doc/sections/uk_UA/2-project-structure.md#впровадження-нових-параметрів-командного-рядка).
